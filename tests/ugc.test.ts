...
describe("native UGC domain", () => {
  ...
  it("bounds deliverables, usage rights, and eligibility in the accepted brief", () => {
    const result = ugcOpportunityInputSchema.parse({
      ...
      description: "Produce a tested vertical product story.",
      category: "Wellness",
      platforms: ["Instagram"],
      deliverables: [
        { title: "Vertical video", quantity: 2, format: "vertical_video" },
      ],
      compensationModel: "hybrid",
      ...
      eligibility: { requiresPortfolio: true },
      revisionLimit: 2,
    });
    ...
    expect(result.sampleTerms).toMatchObject({ required: false, items: [] });
  });
  ...
  it("enforces terminal states and the revision loop", () => {
    ...
    expect(
      canTransitionUgcCollaboration("submitted", "revision_requested"),
    ).toBe(true);
    expect(
      canTransitionUgcCollaboration("revision_requested", "submitted"),
    ).toBe(true);
    expect(canTransitionUgcCollaboration("completed", "in_progress")).toBe(
      false,
    );
    expect(canTransitionUgcSampleShipment("requested", "approved")).toBe(true);
    expect(canTransitionUgcSampleShipment("shipped", "approved")).toBe(false);
    expect(
      canTransitionUgcSampleShipment("delivered", "return_requested"),
    ).toBe(true);
  });
