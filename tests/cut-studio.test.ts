...
describe("CutStudio edit decision list", () => {
  ...
  it("models durable video and audio layers on an absolute multitrack timeline", () => {
    ...
    const result = validateCutEdl({ version: 3, clips: [
      ...
      { id: "broll", assetId: brollId, start: 1, end: 6, track: "v2", timelineStart: 4, transform: { x: .1, y: .1, width: .28, height: .32, opacity: .9 }, motionKeyframes: [{ at: 4, x: .68, y: .62, easing: "ease_in_out" }, { at: 1, x: .2, y: .2 }], chromaKey: { enabled: true, color: "#00ff00", similarity: .12, blend: .05 } },
    ...
    expect(result.clips[1]).toMatchObject({ track: "v2", timelineStart: 4, assetId: brollId, transform: expect.objectContaining({ opacity: .9 }), motionKeyframes: [{ at: 1, x: .2, y: .2 }, { at: 4, x: .68, y: .62, easing: "ease_in_out" }], chromaKey: expect.objectContaining({ enabled: true }) });
