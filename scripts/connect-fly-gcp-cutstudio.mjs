...
const secretResult = spawnSync(gcloud, ["secrets", "versions", "access", "latest", "--secret", "creativesos-cut-dispatch-secret", "--project", "creativesos-504623"], { encoding: null, windowsHide: true });
if (secretResult.status !== 0 || !secretResult.stdout?.length) throw new Error("Unable to read the CutStudio dispatcher secret from Google Secret Manager");
...
try {
  ...
  const result = spawnSync(flyctl, args, { input, stdio: ["pipe", "inherit", "inherit"], windowsHide: true });
