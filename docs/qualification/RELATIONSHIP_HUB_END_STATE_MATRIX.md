...
| --- | --- | --- |
| Standalone unified inbox | Native adapter, legacy DM synchronization, canonical conversations/messages, queue/search/assignment/status controls | `native_complete` |
...
| Durable delivery | Transactional job, idempotency, cross-machine claim, retry, dead letter, stale recovery, receipts, audit, quota reservation | `native_complete`; external delivery is provider-specific |
...
- Full tests, TypeScript, production build, and clean PostgreSQL migration
  qualification.
- Authenticated native field test for inbox initialization, send/receipt,
