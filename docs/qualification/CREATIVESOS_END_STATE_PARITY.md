...
| Shared creator platform | Media Cloud, DAM, planning, analytics/attribution, audience/notification, discovery and rights/trust foundations | `verified_complete` (native); clean production release verified, authenticated refresh pending | Isolated contracts, migrations and two-viewport lifecycle journeys exist for every family. The current clean commit and complete migration parity are resolved through `/api/release`; provider-scale packaging, delivery and production-volume evidence remain separate. |
...
| Developer platform | Scoped APIs, OpenAPI, opaque pagination, key custody, delegated OAuth, typed SDKs, sandbox tenants, reviewed app marketplace, signed webhooks, retry/dead letter and distributed rate limits | `verified_complete` (native); clean production release verified, authenticated refresh pending | Mobile/desktop authorization, cross-tenant denial, atomic code exchange and refresh rotation, SSRF rejection, signed delivery, app review, sandbox expiry/revocation and database-backed limit evidence pass. Current clean production identity is resolved through `/api/release`; public registry publication and independently controlled third-party adoption remain release evidence. |
| Data portability | Versioned exports, dry-run validation, atomic/idempotent imports, durable source mappings and specialized audience/podcast/media migration paths | `verified_complete` (native); clean production release verified, authenticated refresh pending | Products, nested courses, contacts and inactive automation definitions import transactionally with tenant isolation, advisory locking, replay/conflict behavior and secret rejection. Imported offers remain draft; private media URLs never appear in export manifests. Current clean production identity is resolved through `/api/release`; representative competitor-export and production-volume reconciliation remain field evidence. |
...
| Operations | Health/readiness, exact release identity, SLOs, error budgets, usage/cost boundaries, provider state, alerts, recovery, backup/restore and migration parity | `verified_complete` (native); production identity is release-specific | The control plane publishes nine SLOs, preserves `unmeasured`, meters tenant usage/cost, persists budgets, and reports coarse Media Cloud/CutStudio worker capacity. Independent worker entrypoints use capability-aware compare-and-set claims, renewable leases, cancellation, drain state and expired-lease recovery. Readiness fails closed unless source commit, immutable archive fingerprint, dirty state, build identity and the exact migration ledger agree. `/api/release` is canonical for the currently deployed runtime identity; paid regional loss and production-volume evidence remain separate scale gates. |
...
- [x] The exact release replays all 106 migrations from empty, and protected deployment workflow `32664016977` applied and independently exposed exact 106/106 production parity.
- [x] Secret scan, dependency audit, backup/restore and local capacity checks pass.
- [x] The 202-execution isolated PostgreSQL browser matrix covers every provider-independent capability and material local role transition above on mobile and desktop: 196 pass, six are intentionally skipped, and none fail.
...
- [x] Production field evidence retains the earlier safe signed-in LiveKit, AI-quota, Stripe, creation-studio, Distribution, automation, profile, marketplace and community runs described below. Protected workflow `32664016977` deployed exact merge commit `e0f6409d1a0645d41fe920893e4c164b51ab20d6` only after the 202-execution release matrix passed; it completed a private backup, applied all 106 migrations, preserved the compact topology, and passed chained exact-commit public smoke. Independent checks then proved `sourceDirty: false`, fingerprint `a630ae460daa78451420d308d3000f2706caa83e56047798f2491e525dee3053`, exact 106/106 migration parity, ready database/private R2/production authentication, and an empty readiness blocker set. `/api/release` remains canonical for the current runtime after later commits. A fresh authenticated current-release browser repeat is not claimed until an authorized signed-in session is available. Physical camera publication is not claimed because the automated in-app browser had no granted camera device. The unchanged anonymous auth entry retains its Lighthouse evidence at 96 performance and 100 accessibility.
- [x] Provider-disabled states are honest: mixed native/external distribution remains `needs_connection`, retries preserve one native receipt, unconfigured realtime/AI/channel surfaces fail closed, and the activation control plane cannot qualify a provider without current passing evidence for all 14 stages.
