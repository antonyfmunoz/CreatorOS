...
| CutStudio | Conventional multitrack editing plus programmable motion graphics and cinematic generative production | `verified_complete` (existing bounded editor); `locally_qualified` (programmable/cinematic control plane); `not_benchmarked` (Remotion/Higgsfield parity) | The existing creator workflow retains Broadcast handoff, multicam, private proxies, review and source lineage. The expanded runtime now has native schemas, persistence, authorization routes, parameterized starters, a shared-evaluator frame scrubber, deterministic keyframes/transitions, safe EDL compilation, briefs, continuity, camera craft, consent/disclosure, a persisted visual workflow graph and honest provider-pending jobs. Migration 0114 and the mobile/desktop lifecycle, persistence, reviewer denial and cross-tenant asset rejection pass locally. It is not production-qualified or competitively complete until protected deployment evidence, complete advanced-layer preview/render, general graph/layer authoring, an isolated code executor, an approved model round trip and locked direct benchmarks pass. See `CUT_STUDIO_PROGRAMMABLE_CINEMA_STANDARD.md`. |
