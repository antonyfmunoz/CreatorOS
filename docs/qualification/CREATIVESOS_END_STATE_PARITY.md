...
| CutStudio | Durable multitrack editing, captions, audio/color/brand controls, review, rendering and reusable asset lineage | `implemented_unqualified` | Known specialist-depth gaps in the creation-studio scorecard and same-source quality benchmark still prevent a parity claim |
| Broadcast | Multi-studio live production, scenes/sources/audio, guests, collaboration, recording, resilient delivery and operator evidence | `implemented_unqualified` | Known source, guest, audio and operator-depth gaps plus the same-show benchmark still prevent a parity claim |
| Connected creation loop | Completed Broadcast programs, isolated sources and markers open directly as lineage-preserving CutStudio projects without export/re-upload | `implemented_unqualified` | Native handoff is field-tested and idempotent; downstream caption/highlight, distribution, relationship and performance continuity still needs one complete benchmarked golden journey |
