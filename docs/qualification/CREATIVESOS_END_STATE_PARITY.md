...
- [x] The current candidate replays all 106 migrations from empty; the currently verified production runtime remains at 105/105 until this exact candidate is deployed.
...
- [x] Production field evidence retains the earlier safe signed-in LiveKit, AI-quota, Stripe, creation-studio, Distribution, automation, profile, marketplace and community runs described below. Dated workflow `32603964955` deployed exact merge commit `6b8e5d0aadb3da8668a2d1bb73c39d7fb9ef63b9` after 112 files / 432 tests / 186 browser executions in deployment order, all 105 production migrations, a completed private backup, healthy/readiness/release identity, and a two-machine rolling release. Independent checks proved `sourceDirty: false`, exact 105/105 migration parity, protected-API `401` boundaries and an empty readiness blocker set. `/api/release` remains canonical for the current runtime after later commits. A fresh authenticated current-release browser repeat is not claimed until an authorized signed-in session is available. Physical camera publication is not claimed because the automated in-app browser had no granted camera device. The unchanged anonymous auth entry retains its Lighthouse evidence at 96 performance and 100 accessibility.
