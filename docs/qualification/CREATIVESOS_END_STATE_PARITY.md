...
| CutStudio | Conventional multitrack editing plus programmable motion graphics and cinematic generative production | `verified_complete` (existing bounded editor); `locally_qualified` (programmable/cinematic runtime); `not_benchmarked` (Remotion/Higgsfield parity) | The existing creator workflow retains Broadcast handoff, multicam, private proxies, review and source lineage. The expanded runtime has native schemas, persistence, authorization routes, parameterized starters and batches, a shared-evaluator frame scrubber, visual layer/graph authoring, safe EDL compilation, sampled 2D/3D motion, bounded cube/pyramid/plane geometry, geometric/private-mask transitions, allowlisted stylized effects, private TTF/OTF selection and final rendering, validated private self-contained Lottie and Rive browser playback plus isolated final-render frame sequences, briefs, continuity, camera craft, consent/disclosure, honest provider-pending jobs and a private-video candidate review-to-edit handoff that retains shot/variant/job lineage. Fresh isolated qualification proves final private MP4 output, bounded 3D pixels, persisted Lottie SVG preview, real Rive canvas pixels through a pinned same-origin WASM runtime, expression/external-resource and invalid-Rive rejection, final Lottie/Rive RGBA timeline advancement, selected-variant EDL handoff, reviewer denial and cross-tenant asset rejection. Bounded animation final rendering is now implemented and included in the production renderer; exact-release and individual artifact evidence are tracked in the dated programmable-cinema checkpoint. The separate native TSX prototype has actual PNG/JPEG/WebP, frame-range video, hashed image sequences and isolation/cancellation evidence, but public code execution still needs hardened runtime admission, durable dispatch, private artifact exchange, preview integration and recovery. Imported programmable Three scenes, code audio/codec breadth, an approved model round trip and locked direct benchmarks remain internal, provider and evidence work—not merely provider setup. See `CUT_STUDIO_PROGRAMMABLE_CINEMA_STANDARD.md`. |
