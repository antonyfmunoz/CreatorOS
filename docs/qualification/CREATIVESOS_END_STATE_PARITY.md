...
| CutStudio | Conventional multitrack editing plus programmable motion graphics and cinematic generative production | `verified_complete` (existing bounded editor); `locally_qualified` (programmable/cinematic runtime); `not_benchmarked` (Remotion/Higgsfield parity) | The existing creator workflow retains Broadcast handoff, multicam, private proxies, review and source lineage. The expanded runtime has native schemas, persistence, authorization routes, parameterized starters and batches, a shared-evaluator frame scrubber, visual layer/graph authoring, safe EDL compilation, sampled 2D/3D motion, bounded cube/pyramid/plane geometry, geometric/private-mask transitions, allowlisted stylized effects, private TTF/OTF selection and final rendering, validated private self-contained Lottie SVG and Rive Canvas2D browser playback, briefs, continuity, camera craft, consent/disclosure, honest provider-pending jobs and a private-video candidate review-to-edit handoff that retains shot/variant/job lineage. Fresh isolated browser journeys against all 119 migrations prove final private MP4 output, bounded 3D pixels, persisted Lottie SVG preview, real Rive canvas pixels through a pinned same-origin WASM runtime, expression/external-resource and invalid-Rive rejection, fail-closed interactive-media timeline apply, selected-variant EDL handoff, reviewer denial and cross-tenant asset rejection. It remains only locally qualified until the protected stacked release, exact deployment and independent production smoke complete; isolated Lottie/Rive final rendering, imported programmable Three scenes, isolated code execution, an approved model round trip and locked direct benchmarks remain. See `CUT_STUDIO_PROGRAMMABLE_CINEMA_STANDARD.md`. |
