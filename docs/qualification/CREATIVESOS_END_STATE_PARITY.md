...
| Shared creator platform | Media Cloud, DAM, planning, analytics/attribution, audience/notification, discovery and rights/trust foundations | `verified_complete` (native and authenticated production smoke) | Isolated contracts, migrations and two-viewport lifecycle journeys exist for every family. Protected all-scope smoke `32760646885` traversed all 16 production workspaces on mobile and desktop against exact deployed commit `3fdd0b95b9e8e4243dbe9cfbd8e584c5ae94cf48`; provider-scale packaging, delivery and production-volume evidence remain separate. |
...
| Operations | Health/readiness, exact release identity, SLOs, error budgets, usage/cost boundaries, provider state, alerts, recovery, backup/restore and migration parity | `verified_complete` (native); production identity is release-specific | The control plane publishes nine SLOs, preserves `unmeasured`, meters tenant usage/cost, persists budgets, and reports coarse Media Cloud/CutStudio worker capacity. Independent worker entrypoints use capability-aware compare-and-set claims, renewable leases, cancellation, drain state and expired-lease recovery. Readiness fails closed unless source commit, immutable archive fingerprint, dirty state, build identity and the exact migration ledger agree. `/api/release` is canonical for the currently deployed runtime identity. Weekly and on-demand protected qualification verifies the newest private production backup's manifest hash, archive readability and required-table inventory while retaining secret-free evidence; a real-archive isolated restore, paid regional loss and production-volume evidence remain separate recovery or scale gates. |
...
- [x] Production field evidence retains the earlier safe signed-in LiveKit, AI-quota, Stripe, creation-studio, Distribution, automation, profile, marketplace and community runs described below. Protected workflow `32758515220` deployed exact clean commit `3fdd0b95b9e8e4243dbe9cfbd8e584c5ae94cf48` after the complete release matrix passed; it completed a private backup, applied all 106 migrations, preserved the compact topology, and proved exact source identity. Protected all-scope smoke `32760646885` then passed public boundaries and all 16 authenticated production workspaces on mobile and desktop using a dedicated smoke identity and short-lived Clerk testing state. `/api/release` remains canonical for the current runtime after later commits. Physical camera publication is not claimed because the automated browser had no granted camera device. The unchanged anonymous auth entry retains its Lighthouse evidence at 96 performance and 100 accessibility.
