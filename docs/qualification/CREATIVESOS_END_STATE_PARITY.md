...
| Operations | Health/readiness, usage/capacity, provider state, alerts, recovery, backup/restore and migration parity | `verified_complete` | Local migration/recovery/security/capacity gates and v308 production readiness pass across all 83 migrations; production migrations use a transaction-scoped advisory lock safe for the database transaction pooler |
...
- [x] Empty PostgreSQL migration qualification and the v308 production release ledger both pass across all 83 migrations.
...
This qualification plus the v308 production checks are release evidence. They are not evidence of a
