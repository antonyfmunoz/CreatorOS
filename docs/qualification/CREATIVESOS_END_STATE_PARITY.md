...
| Operations | Health/readiness, usage/capacity, provider state, alerts, recovery, backup/restore and migration parity | `verified_complete` | Local migration/recovery/security/capacity gates and v299 production readiness pass; production migrations now use a transaction-scoped advisory lock safe for the database transaction pooler |
...
- [x] Empty PostgreSQL migration and v299 production migration/release-command parity pass.
...
- [x] Production field tests repeat the safe signed-in application surfaces; LiveKit join/leave, AI quota handling, Stripe connected-account delivery, refund/reversal, dispute recovery/restoration and failed-payout persistence retain their current evidence, and v299 adds live authenticated checks for Broadcast, CutStudio, Distribution, automations, profile, marketplace search/product routing and communities with clean runtime logs.
