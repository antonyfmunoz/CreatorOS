...
| Operations | Health/readiness, usage/capacity, provider state, alerts, recovery, backup/restore and migration parity | `verified_complete` | Local migration/recovery/security/capacity gates and v307 production readiness pass across all 83 migrations; production migrations use a transaction-scoped advisory lock safe for the database transaction pooler |
...
- [x] Empty PostgreSQL migration qualification and the v307 production release ledger both pass across all 83 migrations.
...
This is local release evidence. It is not evidence of a
new production deployment, a native phone-binary or background-capture field test, an authorized
