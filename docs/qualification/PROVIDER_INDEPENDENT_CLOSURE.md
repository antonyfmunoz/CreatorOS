...
| 3 | Production smoke | A read-only production Playwright suite verifies release identity, health/readiness, auth-page navigation boundaries, accessibility, and 16 authenticated workspaces on mobile and desktop. The protected workflow always supports an independently runnable public scope; its `all` scope adds authenticated workspaces when an ephemeral session token is configured. Both scopes retain failure artifacts. | Protected public-smoke run [`32626124784`](https://github.com/antonyfmunoz/CreatorOS/actions/runs/32626124784) passed mobile and desktop checks against the exact deployed commit `004e50c45328c02ceeb8d0d54a623fc6e8e2e586`. An authenticated live run remains a session-secret execution gate, not missing application code. |
