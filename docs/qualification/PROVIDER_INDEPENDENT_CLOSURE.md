...
| 3 | Production smoke | A read-only production Playwright suite verifies release identity, health/readiness, auth-page navigation boundaries, accessibility, and 16 authenticated workspaces on mobile and desktop. Every protected deployment now finishes with exact-commit public smoke, and the separate protected workflow supports independently rerunning that public scope. Its `all` scope adds authenticated workspaces when an ephemeral session token is configured. Failure evidence is retained. | Protected public-smoke run [`32626124784`](https://github.com/antonyfmunoz/CreatorOS/actions/runs/32626124784) passed mobile and desktop checks against the exact deployed commit `004e50c45328c02ceeb8d0d54a623fc6e8e2e586`. The deployment-chain contract is statically enforced; its first live chained execution will be recorded with the next authorized application release. An authenticated live run remains a session-secret execution gate, not missing application code. |
...
- The currently deployed runtime has been independently rechecked as verified,
  clean source commit `004e50c45328c02ceeb8d0d54a623fc6e8e2e586`,
  fingerprint `50b56bff60f2f4df67691477738e2e7854bca93ea1da3246ac15cc3cdf1b4b14`,
  with 105/105 migration parity, healthy readiness, configured private R2 asset
  delivery, and zero release blockers.
