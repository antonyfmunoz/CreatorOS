...
| 2 | Scale, soak and chaos | A mixed authenticated HTTP profile covers nine read/write paths and two isolated users; a sustained readiness soak and forced web-process replacement are part of the same disposable-database gate. Worker claim serialization, stale recovery, active-lease preservation and deterministic rate-limit behavior are separately tested. | Final local run: 1,600 mixed requests at concurrency 32, zero failures, 172.4 requests/second, p95 272.3 ms and p99 350.1 ms. The 30.1-second soak completed 8,596 requests at concurrency 24 with zero failures, 285.8 requests/second, p95 173.5 ms and p99 202.5 ms. Readiness recovered and authenticated writes survived process replacement. The limiter admits 240 requests and throttles the following 20 with `Retry-After`. |
...
| 4 | Stitch-only visual regression | All and only the 74 `attached_assets/stitch_creatoros/stitch_creatoros/*/screen.png` references have path, PNG, dimension and SHA-256 custody. Twenty-four canonical provider-independent states are compared against selected Stitch frames using visual-signature and state-specific structural bounds; temporary candidate captures are never promoted to reference images. | The 74-reference manifest gate passes. Feed/story, commerce, native messaging, profile, notifications, settings, discovery, event and joined-community interaction states pass together on the mobile portrait canvas. Iterative variants are grouped as one state, provider-gated voice-lobby media remains explicitly pending, and desktop behavior remains covered by the broader browser matrix. |
...
- Unit, typecheck, production build, bundle budget and Worker dry-deployment
  gates pass.
- The complete authenticated browser matrix schedules 262 executions on
  disposable 114-migration databases across mobile and desktop: 238 pass, 24
  viewport-conditional Stitch cases are intentional skips, and none fail. All
  24 canonical Stitch states execute once on the mobile portrait canvas while
  the behavioral suites retain their configured mobile and desktop coverage.
  It includes a machine-checked census of all 110 registered client routes,
  malformed-identifier containment,
