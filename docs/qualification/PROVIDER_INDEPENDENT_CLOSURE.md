...
| 3 | Production smoke | A read-only production Playwright suite verifies release identity, health/readiness, auth-page navigation boundaries, accessibility, and 16 authenticated workspaces on mobile and desktop. Every protected deployment now finishes with exact-commit public smoke, and the separate protected workflow supports independently rerunning that public scope. Its `all` scope adds authenticated workspaces when an ephemeral session token is configured. Failure evidence is retained. | Protected deployment run [`32664016977`](https://github.com/antonyfmunoz/CreatorOS/actions/runs/32664016977) passed its chained mobile/desktop public smoke against exact deployed commit `e0f6409d1a0645d41fe920893e4c164b51ab20d6`; independent public-smoke run [`32665173137`](https://github.com/antonyfmunoz/CreatorOS/actions/runs/32665173137) passed the same exact-commit boundary again. Its authenticated job was intentionally excluded because no short-lived production session token was supplied; that remains an external execution gate, not missing application code. |
...
- The currently deployed runtime has been independently rechecked as verified,
  clean source commit `e0f6409d1a0645d41fe920893e4c164b51ab20d6`,
  fingerprint `a630ae460daa78451420d308d3000f2706caa83e56047798f2491e525dee3053`,
  with 106/106 migration parity, healthy readiness, configured private R2 asset
  delivery and production authentication, and zero release blockers.
