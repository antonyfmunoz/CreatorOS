...
| Application regression | Passed | 60 test files and 203 tests passed, followed by a clean TypeScript check, production build, and bundle-budget gate. |
...
- `npm run verify`: passed (203 tests, TypeScript, production build, bundle budgets).
...
- `npm run verify:secrets`: passed (508 source files scanned).
...
- Fly release: `v237`; both production machines converged on the same image with passing health checks after the release migration completed successfully. Production migrations use a transaction-scoped advisory lock, and the deployment runner completed without leaving a pooled session lock.
