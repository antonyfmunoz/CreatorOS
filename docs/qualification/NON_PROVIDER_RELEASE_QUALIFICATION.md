...
| Application regression | Passed | 71 test files and 269 tests passed, followed by a clean TypeScript check, production build, and bundle-budget gate. |
...
- `npm run verify`: passed (269 tests across 71 files, TypeScript, production build, bundle budgets and Worker dry run).
...
- `npm run verify:secrets`: passed (568 source files scanned).
...
- Fly release: `v299`; the serving machine is healthy and the stopped machine is the expected standby. The release migration confirmed all 79 migrations. Production migrations use a transaction-scoped advisory lock, and the deployment runner completed without leaving a pooled session lock.
