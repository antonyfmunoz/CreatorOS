...
- Fly release: `v304`, including performance/race hardening commit `8514e15` and stock-fallback removal commit `b3a0241`; the serving machine is healthy and the stopped machine is the expected standby. The release migration confirmed all 80 migrations. Production migrations use a transaction-scoped advisory lock, and the deployment runner completed without leaving a pooled session lock.
