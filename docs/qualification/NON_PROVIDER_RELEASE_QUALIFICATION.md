...
| Application regression | Passed | 73 test files and 289 tests passed, followed by a clean TypeScript check, production build, and bundle-budget gate. |
...
- `npm run verify`: passed (289 tests across 73 files, TypeScript, production build, bundle budgets and Worker dry run).
...
- Fly release: `v305`, including route-family navigation correction commit `22d7cb9`; the serving machine is healthy and the stopped machine is the expected standby. The release migration confirmed all 80 migrations before, during, and after rollout. Production migrations use a transaction-scoped advisory lock, and the deployment runner completed without leaving a pooled session lock.
...
- Anonymous entry performance: server-side `/` to `/auth/login` redirect; Lighthouse 96 performance and 100 accessibility, 1.70 s FCP/LCP, 209 ms total blocking time, 0.019 CLS and 99 ms server response. Application-only overlays are deferred outside authentication, reducing enforced initial JavaScript to 109,391 gzip bytes from the earlier 208,446-byte entry.
- Signed-in browser field test: all safe application workspaces rendered without route or console failures; authenticated production evidence covers Broadcast, a real CutStudio project, Distribution and its queue, all automation views, the six profile tabs, Marketplace filtering and stable product details, communities, and the Create hub. The v305 sweep directly verified exactly one active bottom-navigation destination on Explore, Marketplace, Broadcast, Communities, Profile, Privacy settings, and CutStudio, with Broadcast and CutStudio correctly retaining Create. LiveKit joined with camera and microphone off and reached a connected participant state; OpenAI agent creation/chat reached the provider and returned the explicit quota-exhausted state; Stripe processed signed refund and dispute lifecycles, reversed and restored the creator transfer, drained recovery residue to zero, and persisted a connected-account payout failure; external channels remain honestly provider-disabled.
