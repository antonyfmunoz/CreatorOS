...
| `relationship.inbox.messenger` | provider-backed native capability | connected Facebook Page owner | adapter, OAuth, signed webhooks, durable delivery, and page enumeration implemented; production activation pending Meta credentials and review | OAuth connection + Page task authority | Messenger Platform | signed page event + normalized message + provider receipt |
| `relationship.inbox.whatsapp` | provider-backed native capability | connected WhatsApp Business owner | adapter, encrypted system-user connection, signed webhooks, media delivery, and receipts implemented; production activation pending Meta credentials and business account | verified phone-number connection + customer-service-window policy | WhatsApp Cloud API | signed phone-number event + normalized message + provider receipt |
| `relationship.inbox.x` | provider-backed native capability | connected X account owner | OAuth PKCE, DM adapter, signed webhook support, reconciliation fallback, delivery, and token renewal implemented; production activation pending X credentials and paid API access | OAuth connection + provider policy | X API v2 | normalized DM event + durable cursor + provider receipt |
| `relationship.inbox.provider_policy_blocked` | provider availability declaration | provider platform | TikTok and LinkedIn creator DMs intentionally unavailable until an approved, generally available messaging API or partnership exists | provider approval required; no unofficial session scraping | TikTok / LinkedIn | explicit unavailable state, never a simulated connection |
