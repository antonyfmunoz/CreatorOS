...
| Motion | Ordered keyframes, deterministic interpolation, linear/ease/spring/step behavior, geometric wipe/iris/clock and 3D flip transitions, animated blur/brightness/saturation, bounded effects and an exact-frame scrubber; normal-user keyframe, transition and effect authoring; sampled title, shape and path position/opacity plus rounded shape geometry compile into bounded FFmpeg expressions | Final-render graphic scale, rotation, advanced transitions/effects and interactive-media parity remain renderer work |
...
11. Activate and qualify at least one approved model adapter and one isolated
   code executor, with secrets, rate limits, cancellation, timeout, artifact
   lineage, privacy export/deletion and failure recovery.
12. Run the direct substitution benchmark using locked briefs/assets and human
