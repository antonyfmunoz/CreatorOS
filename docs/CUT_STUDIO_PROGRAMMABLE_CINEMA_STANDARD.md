...
| Layers | Video, audio, image, text, shape, allowlisted SVG/path, caption, Lottie, Rive, 3D and typed data layers; absolute timing, transforms, perspective, blend modes and bindings; visual add, duplicate, delete, reorder, timing, transform, content and style controls; normal-user project video/audio/image/font/Lottie/Rive selection; safe vector/path/SVG, data, bounded 3D plus validated private Lottie SVG and Rive Canvas2D browser preview; private image, fonts, shapes, paths, SVG, Lottie, Rive and typed data text compile into the private final-render graph | Isolated TSX has a pinned Three core, `SvgScene`, and bounded `WebGLScene` under a software renderer. The released field proof covers core geometry, materials, lights and shader-capable rendering in the sealed local runtime. Host GPU/WebGPU, arbitrary addon/dependency compatibility, broad texture workflows and general production-GPU qualification remain separate gaps. |
