...
| Audio repair | Per-clip volume/fades, media-derived audio waveforms, realtime RMS signal meter, audio beds, automatic voice-over ducking, denoise, master gain, and voice/music/broadcast EQ, compression, loudness normalization and true-peak limiting | Add calibrated realtime LUFS/true-peak metering and keyframed mixing automation | Product code and generated-media browser qualification |
| Multitrack edit | Private project media library plus real B-roll/PIP, audio beds, synchronized clip grouping, durable named compounds, render-effective track lock/visibility/mute/solo/gain controls, and timed native title/lower-third/callout graphics rendered by FFmpeg | Add named audio buses and reusable track presets when project-template workflows require them | Product code complete for individual track authority and mixing; team preset work remains |
| Visual finishing | Per-clip position/size/opacity, aspect render, cut/fade-through-black transitions, grade presets, exposure/contrast/saturation/temperature controls with editor preview, and chroma key for overlays | Add LUT import, dissolves and keyframes | Product code |
...
| Source control | Transform, crop, layer, blend, basic visual filters, adjustable chroma key, mute/volume, reusable render-effective source presets, high/low-pass audio filters, dynamics compression, local monitoring, and per-source device echo cancellation, noise suppression and automatic gain | LUT and advanced bus routing | Product code; browser/device capability limitations are disclosed at the control |
| Brand production | Lower thirds, tickers, countdowns, asset logos, persistent studio colors, reusable account brand library and adjustable fade/slide/pulse overlay motion | Role-shared team libraries and expanded motion presets | Product code |
