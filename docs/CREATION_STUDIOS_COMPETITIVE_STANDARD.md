...
| Programmable compositions | Versioned parameter, layer, keyframe, easing, transition, effects, font, data-binding, 3D/vector/interactive-media and audio-reactive contracts; deterministic evaluator; compilation of the safe media/image/text/shape/path/SVG subset into EDL v3; starter templates; visual authoring with owned project video/audio/image selection and shape color; allowlisted vector/path/SVG, 3D, geometric-transition, animated-filter and bounded-effect browser preview; typed parameter resolution and idempotent batches of up to 20 durable named variants; sampled image/text/shape/path/SVG position, opacity, scale, Z/X/Y rotation, perspective, blur, brightness and saturation plus final-render flip, directional wipe, iris, clock, private custom-mask, shadow, glow, grain, noise, vignette, color-matrix, chroma-key, displacement, motion-blur and light-leak treatments | Private font selection, native Lottie/Rive/Three media playback/final rendering and isolated code execution | Native persistence, expanded browser renderer, batch variants, private project-media selection and transformed/filtered/3D-animated/masked/stylized image/text/rounded shape/path/sanitized-SVG output are locally qualified; protected stacked releases and interactive-media rendering remain; executable code additionally requires isolated compute |
