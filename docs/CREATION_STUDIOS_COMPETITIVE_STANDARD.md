...
| Programmable compositions | Versioned parameter, layer, keyframe, easing, transition, effects, font, data-binding, 3D/vector/interactive-media and audio-reactive contracts; deterministic evaluator; compilation of the safe media/image/text/shape/path/SVG/3D-primitive subset into EDL v3; starter templates; visual authoring with owned project video/audio/image/private-TTF-or-OTF/private-Lottie/private-Rive selection and shape/primitive controls; allowlisted vector/path/SVG, bounded cube/pyramid/plane geometry, geometric-transition, animated-filter, bounded-effect, validated self-contained Lottie SVG preview and pinned same-origin Rive Canvas2D/WASM preview; typed parameter resolution and idempotent batches of up to 20 durable named variants; sampled image/text/shape/path/SVG/3D-primitive position, opacity, scale, Z/X/Y rotation, perspective, blur, brightness and saturation plus final-render flip, directional wipe, iris, clock, private custom-mask, shadow, glow, grain, noise, vignette, color-matrix, chroma-key, displacement, motion-blur and light-leak treatments; exact private fonts load in browser preview and materialize into native FFmpeg text rendering | Isolated Lottie/Rive final rendering, imported programmable Three scenes and isolated code execution | Native persistence, expanded browser renderer, batch variants, private project-media/font/Lottie/Rive selection and transformed/filtered/3D-animated/masked/stylized image/text/rounded shape/path/sanitized-SVG/bounded-primitive output are locally qualified; Lottie/Rive timeline apply fails closed until their dedicated renderer exists; protected stacked releases and richer interactive-media final rendering remain; executable code additionally requires isolated compute |
| Cinematic generation | Durable brief, cast/location/prop/wardrobe/product/style/sound continuity, consent/disclosure, shot/lens/camera/lighting/color/audio control, normalized model operations, idempotent jobs, retry/cancel, variants and portable workflow graphs with general node/edge/output authoring; project video candidates have native review, select/reject/supersede state and direct EDL v3 handoff with variant/job lineage | Execute and benchmark a same-brief multi-shot sequence against the approved Higgsfield workflow | Provider-independent control plane, authoring and review-to-edit handoff are implemented; model execution, callback ingest and quality proof require approved compute |
