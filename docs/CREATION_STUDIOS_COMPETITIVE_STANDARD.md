...
| Visual finishing | Per-clip position/size/opacity, render-effective linear position keyframes, aspect render, cut/fade-through-black/cross-dissolve transitions, grade presets, exposure/contrast/saturation/temperature controls with editor preview, private validated `.cube` LUT import with render-time ownership enforcement, and chroma key for overlays | Expand keyframes to scale, opacity, easing curves, and reusable motion presets | Product code and generated-media browser qualification |
