...
| Programmable compositions | Versioned parameter, layer, keyframe, easing, transition, effects, font, data-binding, 3D/vector/interactive-media and audio-reactive contracts; deterministic evaluator; compilation of the safe media/text subset into EDL v3; starter templates; visual layer ordering/timing/transform/style/keyframe/transition/effect authoring | Browser and final rendering of the complete declarative contract | Native persistence and expanded UI are locally qualified; complete renderer and protected release remain; executable code additionally requires isolated compute |
