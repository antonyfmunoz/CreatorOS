...
| Visual finishing | Per-clip position/size/opacity, aspect render, cut/fade-through-black/cross-dissolve transitions, grade presets, exposure/contrast/saturation/temperature controls with editor preview, and chroma key for overlays | Add LUT import and keyframes | Product code |
...
| Capability | Current baseline | Competitive target | Gate |
| --- | --- | --- | --- |
| Scene composer | Reusable solo, interview, presentation and countdown scenes, complete custom scene presets, and owner/editor/viewer team studio collaboration implemented | Cross-studio organization template catalogs | Product code and owner-scoped collaboration field tests |
...
| Brand production | Lower thirds, tickers, countdowns, asset logos, persistent studio colors, reusable account brand library, role-shared studio configuration and adjustable fade/slide/pulse overlay motion | Organization-wide brand catalogs and expanded motion presets | Product code |
