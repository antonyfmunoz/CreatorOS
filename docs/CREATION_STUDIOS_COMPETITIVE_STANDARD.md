...
| Visual finishing | Per-clip position/size/opacity, aspect render, cut/fade-through-black/cross-dissolve transitions, grade presets, exposure/contrast/saturation/temperature controls with editor preview, private validated `.cube` LUT import with render-time ownership enforcement, and chroma key for overlays | Add render-effective keyframes | Product code |
