...
export type CutSourceStarter = "motion" | "three_svg" | "three_webgl";
...
export function starterCutSource(starter: CutSourceStarter = "motion"): CutSourceFile[] {
  if (starter === "three_webgl") return [
    { path: "package.json", content: JSON.stringify({ name: "cut-three-webgl-composition", private: true, type: "module", dependencies: { react: "18.3.1", three: "0.185.1" } }, null, 2) + "\n" },
    { path: "src/index.tsx", content: "import React, { useMemo } from 'react';\nimport * as THREE from 'three';\nimport { FullFrame, WebGLScene, useFrame, useInputs } from '@creativesos/cut';\n\ntype Inputs = { rotationSpeed: number; accent: string };\n\nexport default function Composition() {\n  const frame = useFrame();\n  const inputs = useInputs<Inputs>();\n  const scene = useMemo(() => {\n    const next = new THREE.Scene();\n    next.background = new THREE.Color('#09090b');\n    next.add(new THREE.AmbientLight('#ffffff', 1.2));\n    const light = new THREE.DirectionalLight(inputs.accent, 2.4);\n    light.position.set(2, 3, 4);\n    next.add(light);\n    const sphere = new THREE.Mesh(new THREE.SphereGeometry(1.15, 48, 32), new THREE.MeshStandardMaterial({ color: inputs.accent, metalness: 0.45, roughness: 0.24 }));\n    sphere.name = 'hero-sphere';\n    next.add(sphere);\n    return next;\n  }, [inputs.accent]);\n  const camera = useMemo(() => {\n    const next = new THREE.PerspectiveCamera(42, 1, 0.1, 100);\n    next.position.z = 4.3;\n    return next;\n  }, []);\n  const sphere = scene.getObjectByName('hero-sphere');\n  if (sphere) sphere.rotation.set(frame / (46 / inputs.rotationSpeed), frame / (58 / inputs.rotationSpeed), frame / (72 / inputs.rotationSpeed));\n  return <FullFrame><WebGLScene scene={scene} camera={camera} width={1080} height={1080} style={{ width: '100%', height: '100%' }} /></FullFrame>;\n}\n" },
  ];
