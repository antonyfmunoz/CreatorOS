...
export function productTypeFromLegacyCategory(category: unknown): MvpProductType {
...
export function normalizeProductCommercialTerms(input: {
  ...
  const productType = mvpProductTypes.includes(input.productType as MvpProductType)
    ? input.productType as MvpProductType
    : productTypeFromLegacyCategory(input.category);
  const billingModel = productBillingModels.includes(input.billingModel as ProductBillingModel)
    ? input.billingModel as ProductBillingModel
    : "one_time";
  const billingInterval = billingModel === "recurring"
    ? productBillingIntervals.includes(input.billingInterval as ProductBillingInterval)
      ? input.billingInterval as ProductBillingInterval
      : "month"
    : null;

  if (billingModel === "recurring" && productType !== "membership" && productType !== "community") {
    throw new Error("Recurring billing is available for memberships and communities");
  }

  return { productType, billingModel, billingInterval };
}

export function checkoutBillingTerms(items: Array<{
  billingModel: string;
  billingInterval: string | null;
}>) {
  if (!items.length) throw new Error("Checkout requires at least one offer");
  const billingModel = items[0].billingModel === "recurring" ? "recurring" : "one_time";
  const billingInterval = billingModel === "recurring" ? items[0].billingInterval : null;
  const incompatible = items.some((item) =>
    (item.billingModel === "recurring" ? "recurring" : "one_time") !== billingModel
    || (billingModel === "recurring" && item.billingInterval !== billingInterval),
  );
  if (incompatible) {
