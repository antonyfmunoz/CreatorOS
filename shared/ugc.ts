...
export const ugcCompensationModels = ["fixed", "commission", "hybrid", "gifted"] as const;
export const ugcOpportunityStatuses = ["draft", "open", "paused", "closed", "completed", "archived"] as const;
export const ugcApplicationStatuses = ["submitted", "shortlisted", "accepted", "rejected", "withdrawn"] as const;
export const ugcCollaborationStatuses = ["in_progress", "submitted", "revision_requested", "approved", "live", "completed", "cancelled", "disputed"] as const;
export const ugcSubmissionStatuses = ["submitted", "revision_requested", "approved", "rejected"] as const;

export const ugcDeliverableSchema = z.object({
  title: z.string().trim().min(1).max(160),
  quantity: z.number().int().min(1).max(100).default(1),
  format: z.enum(["vertical_video", "landscape_video", "square_video", "photo", "raw_footage", "script", "other"]),
...
export const ugcUsageRightsSchema = z.object({
  ...
  territories: z.array(z.string().trim().min(1).max(80)).max(50).default(["Worldwide"]),
...
export const ugcCreatorProfileInputSchema = z.object({
  ...
  currency: z.string().trim().regex(/^[a-zA-Z]{3}$/).transform((value) => value.toLowerCase()).default("usd"),
...
export const ugcPortfolioInputSchema = z.object({
  ...
  format: z.enum(["vertical_video", "landscape_video", "square_video", "photo", "audio", "other"]),
...
export const ugcOpportunityInputSchema = z.object({
  ...
  disclosure: z.string().trim().max(2_000).default("Paid partnership disclosure required where applicable."),
});
...
export const ugcApplicationInputSchema = z.object({
  ...
  proposedFeeCents: z.number().int().min(0).max(100_000_000).nullable().optional(),
});
...
export function validateUgcCompensation(input: { compensationModel: typeof ugcCompensationModels[number]; fixedFeeCents: number; commissionBps: number }) {
  if (["fixed", "hybrid"].includes(input.compensationModel) && input.fixedFeeCents <= 0)
    throw new Error("Fixed and hybrid opportunities require a fixed fee");
  if (["commission", "hybrid"].includes(input.compensationModel) && input.commissionBps <= 0)
    throw new Error("Commission and hybrid opportunities require a commission rate");
  if (input.compensationModel === "gifted" && (input.fixedFeeCents > 0 || input.commissionBps > 0))
...
const collaborationTransitions: Record<string, readonly string[]> = {
  ...
  completed: [], cancelled: [], disputed: ["in_progress", "cancelled", "completed"],
};
...
export function ugcCommissionAmount(attributedRevenueCents: number, commissionBps: number) {
  return Math.round(attributedRevenueCents * commissionBps / 10_000);
}

export function ugcEarningsSummary(entries: Array<{ amountCents: number; status: string }>) {
  return entries.reduce((summary, entry) => {
    summary.totalCents += entry.amountCents;
    if (entry.status === "paid") summary.paidCents += entry.amountCents;
    else if (entry.status === "approved") summary.approvedCents += entry.amountCents;
