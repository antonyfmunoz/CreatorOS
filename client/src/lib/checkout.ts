...
export async function startStripeCheckout(productIds: number[], idempotencyKey = crypto.randomUUID()) {
  const order = await apiRequest("POST", "/api/orders", { productIds, idempotencyKey });
  const prepared = await order.json() as PreparedOrder;
  const session = await apiRequest("POST", `/api/orders/${prepared.id}/checkout`, {});
  const checkout = await session.json() as CheckoutSession;
  if (!checkout.checkoutUrl) throw new Error("Secure checkout could not be started");
...
export async function resumeStripeCheckout(orderId: string) {
  ...
  if (!checkout.checkoutUrl) throw new Error("Secure checkout could not be resumed");
  window.location.assign(checkout.checkoutUrl);
}
