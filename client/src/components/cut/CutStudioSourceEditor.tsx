...
export function CutStudioSourceEditor({ draft, busy, onChange, onSave }: { draft: CutSourceDraft; busy: boolean; onChange: (draft: CutSourceDraft) => void; onSave: () => void }) {
  ...
  return <div aria-label="Composition source editor" className="mt-3 space-y-3 rounded-xl border border-zinc-700 p-3">
    ...
    <div className="flex flex-wrap gap-2"><Button size="sm" disabled={busy || Boolean(invalid) || !sourceDraftDirty(draft)} onClick={onSave}>Save new private source ZIP</Button><Button size="sm" variant="outline" disabled={busy || Boolean(invalid)} onClick={exportZip}>Download source ZIP</Button></div>
