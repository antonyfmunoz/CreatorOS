...
function WorkflowEditor({ row, busy, onChange, onSave }: { row: WorkflowRow; busy: boolean; onChange: (workflow: CutGenerativeWorkflow) => void; onSave: () => void }) {
  const width = Math.max(700, ...row.workflow.nodes.map((node) => node.position.x + 210));
  const height = Math.max(150, ...row.workflow.nodes.map((node) => node.position.y + 120));
  const nodes = new Map(row.workflow.nodes.map((node) => [node.id, node]));
  const addFinish = () => {
    const previous = row.workflow.nodes.at(-1)!;
    const id = `finish_${crypto.randomUUID().slice(0, 8)}`;
    onChange({ ...row.workflow, nodes: [...row.workflow.nodes, { id, operation: "video_to_video", provider: "auto", model: "auto", prompt: "Apply the final look and delivery treatment", parameters: {}, inputs: [{ slot: "source_video", sourceNodeId: previous.id, sourceOutput: "video", assetIds: [] }], position: { x: previous.position.x + 280, y: previous.position.y } }], outputs: [{ nodeId: id, output: "video", label: "Finished video" }] });
  };
  return <div className="rounded-xl border border-zinc-800 bg-black p-3" aria-label={`Workflow ${row.workflow.name}`}>
    <div><p className="text-xs font-bold">{row.workflow.name}</p><p className="mt-1 text-[10px] text-zinc-600">{row.workflow.outputs.length} outputs · revision {row.revision}</p></div>
    <div className="mt-3 overflow-x-auto rounded-lg border border-zinc-800 bg-zinc-950"><div className="relative" style={{ width, height }}>
      <svg className="absolute inset-0 h-full w-full" viewBox={`0 0 ${width} ${height}`} aria-hidden="true">{row.workflow.nodes.flatMap((node) => node.inputs.flatMap((input, inputIndex) => { const source = input.sourceNodeId ? nodes.get(input.sourceNodeId) : null; return source ? [<path key={`${node.id}:${inputIndex}`} d={`M ${source.position.x + 190} ${source.position.y + 58} C ${source.position.x + 225} ${source.position.y + 58}, ${node.position.x - 35} ${node.position.y + 58}, ${node.position.x} ${node.position.y + 58}`} fill="none" stroke="#1d9bf0" strokeOpacity=".65" strokeWidth="2"/>] : []; }))}</svg>
      {row.workflow.nodes.map((node) => <div key={node.id} className="absolute w-[190px] rounded-lg border border-[#1d9bf0]/45 bg-black p-2 shadow-lg" style={{ left: node.position.x, top: node.position.y }}><p className="truncate text-[9px] font-black uppercase tracking-wide text-[#1d9bf0]">{node.operation.replaceAll("_", " ")}</p><textarea aria-label={`${node.id} prompt`} className="mt-1 h-12 w-full resize-none rounded border border-zinc-800 bg-zinc-950 p-1.5 text-[9px] leading-3 text-zinc-300 outline-none focus:border-[#1d9bf0]" value={node.prompt} onChange={(event) => onChange({ ...row.workflow, nodes: row.workflow.nodes.map((candidate) => candidate.id === node.id ? { ...candidate, prompt: event.target.value } : candidate) })}/><p className="mt-1 truncate text-[8px] text-zinc-600">{node.provider} · {node.model}</p></div>)}
    </div></div>
    <div className="mt-2 flex gap-2"><Button size="sm" variant="ghost" disabled={busy || row.workflow.nodes.length >= 200} onClick={addFinish}><Plus className="mr-1 h-3.5 w-3.5"/>Finish stage</Button><p className="self-center text-[9px] text-zinc-600">Connections are durable typed inputs, not visual-only lines.</p></div>
    <Button className="mt-2 w-full scroll-mt-20" size="sm" variant="outline" disabled={busy} onClick={onSave}><Check className="mr-1 h-3.5 w-3.5"/>Save graph</Button>
  </div>;
}
  ...
  return <div className="rounded-2xl border border-[#1d9bf0]/35 bg-zinc-950 p-4" aria-label="CutStudio creative runtime">
    ...
    {expanded && <>
      ...
      {!runtime ? <div className="flex justify-center py-10"><Loader2 className="h-5 w-5 animate-spin text-[#1d9bf0]"/></div> : section === "motion" ? <div className="mt-4 space-y-3">
        ...
        {runtime.compositions.map((composition) => { const title = composition.manifest.layers.find((layer) => layer.kind === "text"); const titleIndex = composition.manifest.layers.findIndex((layer) => layer.id === title?.id); return <div key={composition.id} className="rounded-xl border border-zinc-800 bg-black p-3"><div className="flex items-center justify-between gap-2"><div><p className="text-xs font-bold">{composition.name}</p><p className="mt-1 text-[10px] text-zinc-600">{composition.manifest.layers.length} layers · {composition.manifest.fps} fps · revision {composition.revision}</p></div><Button size="sm" disabled={Boolean(busy)} onClick={() => void applyComposition(composition)}>{busy === `apply:${composition.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <><Play className="mr-1 h-3.5 w-3.5"/>Apply</>}</Button></div><CompositionPreview manifest={composition.manifest}/>{title && <><label className="mt-2 block text-[10px] text-zinc-500">Title<input className={field} value={title.text ?? ""} onChange={(event) => updateCompositionDraft(composition.id, (manifest) => ({ ...manifest, parameters: manifest.parameters.map((parameter) => parameter.key === title.dataBindings.text ? { ...parameter, defaultValue: event.target.value } : parameter), layers: manifest.layers.map((layer, index) => index === titleIndex ? { ...layer, text: event.target.value } : layer) }))}/></label><div className="grid grid-cols-3 gap-2"><label className="text-[10px] text-zinc-500">X<input className={field} type="number" min={0} max={.85} step={.05} value={title.x} onChange={(event) => updateCompositionDraft(composition.id, (manifest) => ({ ...manifest, layers: manifest.layers.map((layer, index) => index === titleIndex ? { ...layer, x: Number(event.target.value) } : layer) }))}/></label><label className="text-[10px] text-zinc-500">Y<input className={field} type="number" min={0} max={.85} step={.05} value={title.y} onChange={(event) => updateCompositionDraft(composition.id, (manifest) => ({ ...manifest, layers: manifest.layers.map((layer, index) => index === titleIndex ? { ...layer, y: Number(event.target.value) } : layer) }))}/></label><label className="text-[10px] text-zinc-500">Size<input className={field} type="number" min={12} max={160} step={2} value={Number(title.style.fontSize ?? 48)} onChange={(event) => updateCompositionDraft(composition.id, (manifest) => ({ ...manifest, layers: manifest.layers.map((layer, index) => index === titleIndex ? { ...layer, style: { ...layer.style, fontSize: Number(event.target.value) } } : layer) }))}/></label></div><Button className="mt-2 w-full" size="sm" variant="outline" disabled={Boolean(busy)} onClick={() => void saveComposition(composition)}>{busy === `save:${composition.id}` ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin"/> : <Check className="mr-1 h-3.5 w-3.5"/>}Save controls</Button></>}</div>; })}
      ...
      </div> : <div className="mt-4 space-y-3"><p className="text-xs leading-5 text-zinc-400">Build reusable capability-driven graphs instead of hard-coding one vendor. Inputs, outputs, model choice, prompts, and lineage remain portable.</p><Button className="w-full" size="sm" variant="outline" disabled={Boolean(busy)} onClick={() => void createWorkflow()}>{busy === "workflow" ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin"/> : <Plus className="mr-1 h-3.5 w-3.5"/>}Starter campaign flow</Button>{runtime.workflows.map((row) => <WorkflowEditor key={row.id} row={row} busy={Boolean(busy)} onChange={(workflow) => updateWorkflowDraft(row.id, workflow)} onSave={() => void saveWorkflow(row)}/>)}</div>}
