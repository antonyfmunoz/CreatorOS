...
function CodeRenderControls({ composition, busy, readiness, onQueue, onQueueBatch }: { composition: CompositionRow; busy: boolean; readiness: LocalExecutionReadiness; onQueue: (request: CutCodeRenderRequest) => void; onQueueBatch: (requests: CutCodeRenderRequest[]) => void }) {
  ...
  return <div className="mt-3 space-y-2 border-t border-[#1d9bf0]/20 pt-3" aria-label={label}>
    ...
    {!readiness.ready && <p role="status" className="text-[9px] leading-4 text-amber-200">{readiness.message}</p>}
...
export function CutStudioCreativeRuntime({ project, media, onSaveCodeSource, onTimelineApplied: applyTimeline, onRenderBatchQueued: renderBatchQueued, onProjectMediaChanged, onUseCodeRenderOutput, onTimelineBusyChange, onUnsavedChange }: { project: ProjectInput; media: ProjectMediaInput[]; onSaveCodeSource: (file: File, lockfile?: File) => Promise<{ assetId: string; lockfileAssetId?: string }>; onTimelineApplied: (result: { edl: CutEdl; duration: number; revision: number }) => void; onRenderBatchQueued: () => void; onProjectMediaChanged?: () => void; onUseCodeRenderOutput?: (media: ProjectMediaInput) => void; onTimelineBusyChange?: (busy: boolean) => void; onUnsavedChange?: (dirty: boolean) => void }) {
  ...
  const declarativeCompositionManifests = new Map((runtime?.compositions ?? []).filter((composition) => composition.mode === "declarative").map((composition) => [composition.id, composition.manifest]));
  ...
  const restoreCompositionRevision = (composition: CompositionRow, historicalRevision: number) => act(`composition:restore:${composition.id}:${historicalRevision}`, async () => {
    const restored = await (await apiRequest("POST", `/api/cut/projects/${project.id}/compositions/${composition.id}/history/${historicalRevision}/restore`, {}, { "If-Match": String(composition.revision) })).json() as CompositionRow;
    if (alive.current) {
      setRuntime((current) => current ? { ...current, compositions: current.compositions.map((row) => row.id === restored.id ? restored : row) } : current);
      setCompositionHistories((current) => { const next = { ...current }; delete next[composition.id]; return next; });
    }
    await refresh();
    setMessage(`Restored revision ${historicalRevision} as revision ${restored.revision}. The source history remains immutable.`);
  });
    ...
    {expanded && <>
      ...
      {!runtime ? <div className="flex justify-center py-10"><Loader2 className="h-5 w-5 animate-spin text-[#1d9bf0]"/></div> : section === "motion" ? <div className="mt-4 space-y-3">
        ...
        {runtime.compositions.map((composition) => <div key={composition.id} aria-label={`Composition ${composition.name}`} className="rounded-xl border border-zinc-800 bg-black p-3">
          <div className="flex items-center justify-between gap-2"><div><p className="text-xs font-bold">{composition.name}</p><p className="mt-1 text-[10px] text-zinc-600">{composition.mode === "sandboxed_tsx" ? "isolated TSX" : `${composition.manifest.layers.length} layers`} · {composition.manifest.fps} fps · revision {composition.revision}</p></div><div className="flex gap-1"><Button size="sm" variant="ghost" aria-label={`Source history for ${composition.name}`} disabled={Boolean(busy)} onClick={() => void loadCompositionHistory(composition)}><History className="mr-1 h-3.5 w-3.5"/>History</Button>{composition.mode === "declarative" ? <Button size="sm" disabled={Boolean(busy) || compositions.current.has(composition.id)} onClick={() => void applyComposition(composition)}>{busy === `apply:${composition.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <><Play className="mr-1 h-3.5 w-3.5"/>Apply</>}</Button> : <Button size="sm" variant="outline" disabled={Boolean(busy)} onClick={() => beginCodeCompositionRevision(composition)}>Revise source</Button>}</div></div>
          {compositionHistories[composition.id] && <div aria-label={`Source history for ${composition.name}`} className="mt-2 rounded-md border border-zinc-800 bg-zinc-950 px-2 py-1.5 text-[9px] text-zinc-400"><p className="font-bold uppercase tracking-[.12em] text-zinc-500">Pinned source history</p>{compositionHistories[composition.id].map((revision) => {
