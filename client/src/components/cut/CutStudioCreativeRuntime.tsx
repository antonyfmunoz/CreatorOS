    ...
    {expanded && <>
      ...
      {!runtime ? <div className="flex justify-center py-10"><Loader2 className="h-5 w-5 animate-spin text-[#1d9bf0]"/></div> : section === "motion" ? <div className="mt-4 space-y-3">
        ...
        {runtime.compositions.map((composition) => <div key={composition.id} aria-label={`Composition ${composition.name}`} className="rounded-xl border border-zinc-800 bg-black p-3">
          ...
          {compositionHistories[composition.id] && <div aria-label={`Source history for ${composition.name}`} className="mt-2 rounded-md border border-zinc-800 bg-zinc-950 px-2 py-1.5 text-[9px] text-zinc-400"><p className="font-bold uppercase tracking-[.12em] text-zinc-500">Pinned source history</p>{compositionHistories[composition.id].map((revision) => <div key={`${composition.id}:${revision.revision}`} className="mt-1 flex items-center justify-between gap-2"><p>Revision {revision.revision} · {new Date(revision.createdAt).toLocaleString()} · {revision.codeCapsule?.entrypoint ?? "declarative manifest"}</p>{revision.revision !== composition.revision && <Button size="sm" variant="ghost" aria-label={`Restore revision ${revision.revision} for ${composition.name}`} disabled={Boolean(busy)} onClick={() => void restoreCompositionRevision(composition, revision.revision)}>Restore</Button>}</div>)}</div>}
