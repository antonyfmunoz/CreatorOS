...
function CodeRenderControls({ composition, busy, ready, onQueue, onQueueBatch }: { composition: CompositionRow; busy: boolean; ready: boolean; onQueue: (request: CutCodeRenderRequest) => void; onQueueBatch: (requests: CutCodeRenderRequest[]) => void }) {
  ...
  const setExportMode = (next: CutCodeRenderMode) => {
    setMode(next); setFormat(defaultCutCodeRenderFormat(next)); setEncodingMode("default");
  };
  ...
  return <div className="mt-3 space-y-2 border-t border-[#1d9bf0]/20 pt-3" aria-label={label}>
    <div className="grid grid-cols-3 gap-2"><label className="text-[9px] text-zinc-500">Output<select aria-label={`${label} output`} className={field} value={mode} disabled={busy} onChange={(event) => setExportMode(event.target.value as CutCodeRenderMode)}>{(["still", "video", "sequence", "audio"] as const).map((value) => <option key={value} value={value}>{value === "still" ? "Still" : value === "video" ? "Video" : value === "audio" ? "Audio" : "Frame sequence"}</option>)}</select></label><label className="text-[9px] text-zinc-500">Format<select aria-label={`${label} format`} className={field} value={format} disabled={busy} onChange={(event) => { const next = event.target.value; setFormat(next); if (next !== "mp4" && next !== "webm") setEncodingMode("default"); if (next === "webm" && encodingMode === "lossless") setEncodingMode("default"); }}>{cutCodeRenderFormats[mode].map((value) => <option key={value} value={value}>{value === "mov" ? "MOV · ProRes" : value.toUpperCase()}</option>)}</select></label>{format === "mov" ? <label className="text-[9px] text-zinc-500">ProRes profile<select aria-label={`${label} ProRes profile`} className={field} value={proresProfile} disabled={busy} onChange={(event) => setProresProfile(event.target.value as typeof proresProfile)}><option value="422hq">422 HQ</option><option value="4444">4444</option><option value="4444xq">4444 XQ</option></select></label> : <label className="text-[9px] text-zinc-500">Quality<input aria-label={`${label} quality`} className={field} type="number" min="1" max="100" value={quality} disabled={busy || !["jpeg", "webp"].includes(format)} onChange={(event) => readNumber(event, setQuality)}/></label>}</div>
