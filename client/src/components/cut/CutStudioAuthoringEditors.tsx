...
import { CutStudioTextLayoutControls } from "./CutStudioTextLayoutControls";
import { CUT_NATIVE_TEXT_MAX_CHARACTERS } from "@shared/cut-text-layout";
...
const animationEasings = ["linear", "ease_in", "ease_out", "ease_in_out", "spring", "step"] as const;
...
export function CompositionAuthoringControls({ composition, assets, compositions, busy, onChange, onSave }: {
  ...
  const addKeyframe = () => updateLayer((layer) => {
    ...
    const keyframes = [...(existing?.keyframes ?? []).filter((item) => item.frame !== boundedFrame), { frame: boundedFrame, value: keyframeValue, easing: keyframeEasing }].sort((left, right) => left.frame - right.frame);
  ...
  const selectLayerFont = (assetId: string) => {
    if (!selected) return;
    if (!assetId) {
      const { fontFamily: _priorFamily, ...style } = selected.style;
      onChange({ ...manifest, layers: replaceAt(manifest.layers, selectedIndex, { ...selected, style }) });
      return;
    }
    const asset = assets.find((item) => item.mediaKind === "font" && item.assetId === assetId);
    if (!asset) return;
    const family = cutStudioPrivateFontFamily(asset.assetId);
    const registered = manifest.fonts.find((font) => font.family === family && font.assetId === asset.assetId);
    const fonts = registered ? manifest.fonts : [...manifest.fonts.filter((font) => font.family !== family), { family, assetId: asset.assetId, weight: 400, style: "normal" as const }];
    onChange({ ...manifest, fonts, layers: replaceAt(manifest.layers, selectedIndex, { ...selected, style: { ...selected.style, fontFamily: family } }) });
  };
  ...
  return <div className="mt-3 rounded-xl border border-zinc-800 bg-zinc-950 p-3" aria-label="Composition authoring controls">
    ...
    {selected && <div className="mt-3 space-y-3">
      ...
      {selected.kind === "shape" && <div className="space-y-2 rounded-lg border border-zinc-800 p-2"><label className="block text-[10px] text-zinc-500">Fill color<input aria-label="Layer fill color" type="color" className={`${field} h-9 p-1`} value={String(selected.style.fill ?? "#1d9bf0")} onChange={(event) => updateLayer((layer) => ({ ...layer, style: { ...layer.style, fill: event.target.value } }))}/></label><label className="flex items-center gap-2 text-[10px] text-zinc-400"><input aria-label="Use shape gradient" type="checkbox" checked={typeof selected.style.gradientStartColor === "string" && typeof selected.style.gradientEndColor === "string"} onChange={(event) => updateLayer((layer) => event.target.checked ? { ...layer, style: { ...layer.style, gradientStartColor: typeof layer.style.fill === "string" ? layer.style.fill : "#1d9bf0", gradientEndColor: "#a855f7", gradientDirection: "horizontal" } } : (() => { const { gradientStartColor: _start, gradientEndColor: _end, gradientDirection: _direction, ...style } = layer.style; return { ...layer, style }; })())}/>Use two-color gradient</label>{typeof selected.style.gradientStartColor === "string" && typeof selected.style.gradientEndColor === "string" && <div className="grid grid-cols-3 gap-2"><label className="text-[10px] text-zinc-500">Start<input aria-label="Gradient start color" type="color" className={`${field} h-9 p-1`} value={selected.style.gradientStartColor} onChange={(event) => updateLayer((layer) => ({ ...layer, style: { ...layer.style, gradientStartColor: event.target.value } }))}/></label><label className="text-[10px] text-zinc-500">End<input aria-label="Gradient end color" type="color" className={`${field} h-9 p-1`} value={selected.style.gradientEndColor} onChange={(event) => updateLayer((layer) => ({ ...layer, style: { ...layer.style, gradientEndColor: event.target.value } }))}/></label><label className="text-[10px] text-zinc-500">Direction<select aria-label="Gradient direction" className={field} value={String(selected.style.gradientDirection ?? "horizontal")} onChange={(event) => updateLayer((layer) => ({ ...layer, style: { ...layer.style, gradientDirection: event.target.value } }))}><option value="horizontal">Horizontal</option><option value="vertical">Vertical</option></select></label></div>}</div>}
      ...
      <div className="rounded-lg border border-zinc-800 p-2"><div className="flex items-center gap-2"><KeyRound className="h-3.5 w-3.5 text-[#1d9bf0]"/><p className="text-[10px] font-bold">Keyframes</p></div><div className="mt-2 grid grid-cols-2 gap-1"><select aria-label="Keyframe property" className={compactField} value={keyframeProperty} onChange={(event) => setKeyframeProperty(event.target.value as typeof keyframeProperty)}>{animationProperties.map((property) => <option key={property}>{property}</option>)}</select><select aria-label="Keyframe easing" className={compactField} value={keyframeEasing} onChange={(event) => setKeyframeEasing(event.target.value as typeof keyframeEasing)}>{animationEasings.map((easing) => <option key={easing}>{easing.replaceAll("_", " ")}</option>)}</select><input aria-label="Keyframe frame" className={compactField} type="number" min={0} max={selected.durationInFrames - 1} value={keyframeFrame} onChange={(event) => setKeyframeFrame(numberValue(event.target.value, 0))}/><input aria-label="Keyframe value" className={compactField} type="number" step="0.01" value={keyframeValue} onChange={(event) => setKeyframeValue(numberValue(event.target.value, 0))}/></div><Button className="mt-2 w-full" size="sm" variant="ghost" onClick={addKeyframe}><Plus className="mr-1 h-3.5 w-3.5"/>Add or replace keyframe</Button><div className="mt-1 flex flex-wrap gap-1">{selected.animations.flatMap((animation) => animation.keyframes.map((keyframe) => <button key={`${animation.property}:${keyframe.frame}`} className="rounded bg-zinc-900 px-1.5 py-1 text-[8px] text-zinc-400" title="Remove keyframe" onClick={() => updateLayer((layer) => ({ ...layer, animations: layer.animations.map((item) => item.property === animation.property ? { ...item, keyframes: item.keyframes.filter((candidate) => candidate.frame !== keyframe.frame) } : item).filter((item) => item.keyframes.length) }))}>{animation.property}@{keyframe.frame}: {String(keyframe.value)} · {keyframe.easing} <X className="inline h-2.5 w-2.5"/></button>))}</div></div>
