      ...
      <div className="rounded-lg border border-zinc-800 p-2">
        <div className="flex items-center justify-between">
          ...
          <select aria-label="Add layer effect" className="rounded border border-zinc-800 bg-zinc-950 px-2 py-1 text-[9px] text-zinc-300" defaultValue="" onChange={(event) => {
            ...
            if (kind) updateLayer((layer) => ({ ...layer, effects: [...layer.effects, { id: `${kind}_${crypto.randomUUID().slice(0, 8)}`, kind, enabled: true, parameters: {} }] }));
            event.currentTarget.value = "";
          }}><option value="">Add effect…</option>{effectKinds.map((kind) => <option key={kind}>{kind}</option>)}</select>
        </div>
        <div className="mt-2 flex flex-wrap gap-1">{selected.effects.map((effect) => <button key={effect.id} className={effect.enabled ? "rounded bg-[#1d9bf0]/15 px-2 py-1 text-[9px] text-[#1d9bf0]" : "rounded bg-zinc-900 px-2 py-1 text-[9px] text-zinc-600"} onClick={() => toggleEffect(effect.id)} onDoubleClick={() => removeEffect(effect.id)}>{effect.kind.replaceAll("_", " ")}</button>)}</div>
        {selected.effects.length > 0 && <p className="mt-1 text-[8px] text-zinc-700">Click to enable/disable · double-click to remove</p>}
      </div>
    </div>}
