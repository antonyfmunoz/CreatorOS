      ...
      <div className="rounded-lg border border-zinc-800 p-2">
        <div className="flex items-center justify-between">
          ...
          <select aria-label="Add layer effect" className="rounded border border-zinc-800 bg-zinc-950 px-2 py-1 text-[9px] text-zinc-300" defaultValue="" onChange={(event) => {
            ...
            if (kind) updateLayer((layer) => ({ ...layer, effects: [...layer.effects, { id: `${kind}_${crypto.randomUUID().slice(0, 8)}`, kind, enabled: true, parameters: effectParameters(kind) }] }));
            event.currentTarget.value = "";
          }}><option value="">Add effect…</option>{effectKinds.map((kind) => <option key={kind}>{kind}</option>)}</select>
        </div>
        <div className="mt-2 space-y-1">{selected.effects.map((effect) => <div key={effect.id} className="grid grid-cols-[minmax(0,1fr)_5rem_auto] items-center gap-1 rounded bg-zinc-900 p-1"><button className={effect.enabled ? "truncate text-left text-[9px] text-[#1d9bf0]" : "truncate text-left text-[9px] text-zinc-600"} onClick={() => toggleEffect(effect.id)}>{effect.kind.replaceAll("_", " ")}</button><input aria-label={`${effect.kind} amount`} className={compactField} type="number" min={0} max={80} step={.05} value={Number(effect.parameters.amount ?? effect.parameters.radius ?? effect.parameters.intensity ?? .5)} onChange={(event) => updateLayer((layer) => ({ ...layer, effects: layer.effects.map((candidate) => candidate.id === effect.id ? { ...candidate, parameters: { ...candidate.parameters, ["radius" in candidate.parameters ? "radius" : "amount"]: Math.max(0, Math.min(80, numberValue(event.target.value, .5))) } } : candidate) }))}/><button aria-label={`Remove ${effect.kind}`} className="p-1 text-rose-400" onClick={() => removeEffect(effect.id)}><Trash2 className="h-3 w-3"/></button></div>)}</div>
