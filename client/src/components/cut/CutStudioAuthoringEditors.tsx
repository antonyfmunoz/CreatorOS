...
type CompositionAsset = { id: string; assetId: string; name: string; duration: number; mediaKind: "video" | "audio" | "image" | "font" | "lottie" };
...
export function CompositionAuthoringControls({ composition, assets, busy, onChange, onSave }: {
  ...
  const selectLayerFont = (assetId: string) => {
    ...
    const fonts = [...manifest.fonts.filter((font) => font.family !== family), { family, assetId: asset.assetId, weight: 400, style: "normal" as const }];
  ...
  return <div className="mt-3 rounded-xl border border-zinc-800 bg-zinc-950 p-3" aria-label="Composition authoring controls">
    ...
    {selected && <div className="mt-3 space-y-3">
      ...
      {["video", "audio", "image", "lottie"].includes(selected.kind) && <label className="block text-[10px] text-zinc-500">Project media<select aria-label="Layer media asset" className={field} value={selected.assetId ?? ""} onChange={(event) => updateLayer((layer) => ({ ...layer, assetId: event.target.value }))}>{assets.filter((asset) => asset.mediaKind === selected.kind).map((asset) => <option key={asset.id} value={asset.assetId}>{asset.name} · {asset.duration.toFixed(1)}s</option>)}</select></label>}
