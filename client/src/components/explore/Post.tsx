...
const Post = ({ post, surface = "light", onDeleted }: PostProps) => {
  ...
  const [filteredGroupConversations, setFilteredGroupConversations] = useState<
    Conversation[]
  >([]);
  ...
  const isOwnRepost = Boolean(
    user && post.userId === user.id && post.repostOfId,
  );
  ...
  const { data: safetyControls = [] } = useQuery<SafetyControl[]>({
    queryKey: ["/api/discovery/safety-controls"],
    enabled: !!user,
  });
  ...
  const isSaved = persistedSavedPosts.some(
    (savedPost) => savedPost.id === post.id,
  );
  const isLiked = persistedLikedPosts.some(
    (likedPost) => likedPost.id === post.id,
  );
  const repostContent = `Reposted @${post.user.username}: ${post.content}`;
  const isReposted = myPosts.some(
    (candidate) => candidate.repostOfId === post.id,
  );
  ...
  const mentionedUsernames = Array.from(
    new Set(
      (post.content.match(/@[A-Za-z0-9_]+/g) ?? []).map((mention) =>
  ...
  useEffect(() => {
    ...
    void apiRequest("POST", `/api/posts/${post.id}/view`).catch(
      () => undefined,
    );
  ...
  const { data: mentionedUsers = [] } = useQuery<User[]>({
    ...
    queryFn: async () => {
      ...
      for (const candidate of resultSets.flat()) {
        if (mentionedUsernames.includes(candidate.username.toLowerCase()))
  ...
  const { data: taggedUsers = [] } = useQuery<TaggedUser[]>({
    ...
    enabled:
      !!post.imageUrl && ((post.taggedUsers?.length ?? 0) > 0 || showTags),
  });
  ...
  const repostMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Reposted",
        description: "The post is now on your feed.",
      });
    },
    onError: () => {
      toast({
  ...
  const votePostPollMutation = useMutation({
    mutationFn: async (optionId: number) =>
      (
        await apiRequest("POST", `/api/posts/${post.id}/poll/vote`, {
          optionId,
        })
      ).json() as Promise<PostPoll>,
    onSuccess: (poll) =>
      queryClient.setQueryData(["/api/posts", post.id, "poll"], poll),
    onError: () =>
      toast({
  ...
  const reportPostMutation = useMutation({
    ...
    onSuccess: () =>
      toast({
        ...
        description: "Thanks. Our moderation team can now review this post.",
      }),
    onError: () =>
      toast({
  ...
  const safetyControlMutation = useMutation({
    mutationFn: async (
      change: Partial<Pick<SafetyControl, "muted" | "restricted">>,
    ) => {
      if (!user) throw new Error("User not authenticated");
      const current = safetyControls.find(
        (control) => control.targetUserId === post.userId,
      );
      return (
        await apiRequest(
          "PUT",
          `/api/discovery/safety-controls/${post.userId}`,
          {
            muted: change.muted ?? current?.muted ?? false,
            restricted: change.restricted ?? current?.restricted ?? false,
          },
        )
      ).json();
    },
    onSuccess: (_, change) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/discovery/feed"] });
      toast({
        title: change.muted ? "Creator muted" : "Creator restricted",
        description: change.muted
          ? "Their posts will no longer appear in your feed."
          : "Their new comments on your posts will be held for your review.",
      });
    },
    onError: () =>
      toast({
        title: "Safety setting was not saved",
        description: "Please try again.",
        variant: "destructive",
      }),
  });

  const blockCreatorMutation = useMutation({
    mutationFn: async () => {
      if (!user) throw new Error("User not authenticated");
      return apiRequest("POST", `/api/discovery/blocks/${post.userId}`, {});
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/discovery/feed"] });
      toast({
        title: "Creator blocked",
        description: "You will no longer see or interact with each other.",
      });
    },
    onError: () =>
      toast({
        title: "Creator was not blocked",
        description: "Please try again.",
        variant: "destructive",
      }),
  });
  ...
  return (
    <Card
      id={`post-${post.id}`}
      className={`mb-0 overflow-hidden rounded-none border-x-0 border-t-0 shadow-none ${isDark ? "border-zinc-800 bg-black text-white" : "border-zinc-100 bg-white text-black"}`}
    >
      <AlertDialog
      ...
      <CardContent className="p-4">
        {post.discovery?.explanation?.length ? (
          <details className="mb-3 text-[10px] text-zinc-500">
            <summary className="cursor-pointer font-semibold uppercase tracking-wide text-zinc-400">
              Why this post
            </summary>
            <ul className="mt-2 list-disc space-y-1 pl-4">
              {post.discovery.explanation.map((reason) => (
            ...
            <DropdownMenuContent align="end" className="w-48">
              {user && user.id === post.userId ? (
                ...
                <>
                  ...
                  <DropdownMenuItem
                    className="cursor-pointer"
                    disabled={safetyControlMutation.isPending}
                    onClick={() =>
                      safetyControlMutation.mutate({ muted: true })
                    }
                  >
                  ...
                  <DropdownMenuItem
                    ...
                    disabled={safetyControlMutation.isPending}
                    onClick={() =>
                      safetyControlMutation.mutate({ restricted: true })
                    }
                  >
                    <ShieldAlert className="mr-2 h-4 w-4" />
                    <span>Restrict creator</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    className="cursor-pointer text-destructive"
                    disabled={blockCreatorMutation.isPending}
                    onClick={() => blockCreatorMutation.mutate()}
                  >
        ...
        {postPoll && (
          <section
            className="mb-4 rounded-xl border border-zinc-800 bg-zinc-950 p-3"
            aria-label={`Poll: ${postPoll.question}`}
          >
            <h3 className="text-sm font-bold text-white">
              {postPoll.question}
            </h3>
            <div className="mt-3 space-y-2">
              {postPoll.options.map((option) => {
                const percentage = postPoll.totalVotes
                  ? Math.round((option.votes / postPoll.totalVotes) * 100)
                  : 0;
                const selected = postPoll.viewerOptionId === option.id;
                return (
                  <button
                    key={option.id}
                    type="button"
                    aria-pressed={selected}
                    disabled={votePostPollMutation.isPending}
                    onClick={() => votePostPollMutation.mutate(option.id)}
                    className={`relative flex w-full overflow-hidden rounded-lg border px-3 py-2.5 text-left text-sm transition-colors ${selected ? "border-[#1d9bf0] text-white" : "border-zinc-700 text-zinc-200 hover:border-zinc-500"}`}
                  >
                    <span
                      className="absolute inset-y-0 left-0 bg-[#1d9bf0]/15"
                      style={{ width: `${percentage}%` }}
                    />
                    <span className="relative z-10 flex-1 font-medium">
                      {option.body}
                    </span>
                    <span className="relative z-10 ml-3 text-xs text-zinc-400">
                      {percentage}%
                    </span>
                  </button>
                );
            ...
            <p className="mt-3 text-xs text-zinc-500">
              {postPoll.totalVotes.toLocaleString()} vote
              {postPoll.totalVotes === 1 ? "" : "s"} · Select an option to vote
              or change your vote
            </p>
          </section>
        )}
            ...
            {taggedUsers.length > 0 && !showTags && (
              <div className="absolute bottom-2 left-2 flex items-center">
                ...
                <div className="ml-2 text-xs bg-black bg-opacity-75 text-white py-1 px-2 rounded">
                  Tap to view {taggedUsers.length} tagged{" "}
        ...
        <Dialog open={mediaViewerOpen} onOpenChange={setMediaViewerOpen}>
          <DialogContent className="h-dvh max-w-none overflow-hidden border-0 bg-black p-0 text-white sm:rounded-none">
            <DialogTitle className="sr-only">
              {isVideoPost ? "Video post" : "Image post"} by{" "}
              {post.user.displayName}
            </DialogTitle>
            <button
              type="button"
              onClick={() => setMediaViewerOpen(false)}
              className="absolute right-4 top-4 z-30 rounded-full bg-black/45 p-2 text-white transition-colors hover:bg-black/70"
            ...
            <button
              type="button"
              className="absolute left-4 top-4 z-30 rounded-full bg-black/45 p-2 text-white transition-colors hover:bg-black/70"
            ...
            <div className="relative flex h-full items-center justify-center bg-black">
              {isVideoPost ? (
                <MediaPlayer
                  ...
                  telemetryContext={{ surface: "feed-viewer", postId: post.id }}
                  playsInline
                  muted={isMediaMuted}
            ...
            <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black via-black/75 to-transparent px-4 pb-5 pt-20">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="ml-auto rounded-full border border-white/80 px-3 py-1.5 text-xs font-bold text-white hover:bg-white hover:text-black"
                >
                  View profile
                </button>
              </div>
              ...
              <div className="mt-4 flex items-center gap-4 text-sm text-white">
                ...
                <button
                  ...
                  disabled={isReposted || isOwnRepost}
              ...
              {isVideoPost && (
                <div className="mt-4 flex items-center justify-between border-t border-white/20 pt-3">
                  <button
                    ...
                    onClick={() =>
                      isMediaPlaying
                        ? mediaViewerRef.current?.pause()
                        : void mediaViewerRef.current?.play()
                    }
                  ...
                  <button
                    type="button"
                    onClick={() => setIsMediaMuted((muted) => !muted)}
                    className="rounded-full p-1.5 hover:bg-white/10"
                    aria-label={isMediaMuted ? "Unmute video" : "Mute video"}
                  >
                    {isMediaMuted ? (
                  ...
                  <span className="flex-1 text-center text-xs text-zinc-300">
                    Video post
                  </span>
                  <button
                    type="button"
                    onClick={() =>
                      void mediaViewerRef.current?.requestPictureInPicture?.()
                    }
                    className="rounded-full p-1.5 hover:bg-white/10"
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      void mediaViewerRef.current?.requestFullscreen?.()
                    }
                    className="rounded-full p-1.5 hover:bg-white/10"
        ...
        <div className="flex items-center justify-between text-zinc-500">
          <div className="flex flex-1 items-center justify-between">
            ...
            >
              <MessageSquare
                className={`h-5 w-5 ${showComments ? "text-blue-500" : ""}`}
              />
              <span>{totalCommentCount}</span>
            </Button>
            ...
            <Button
              ...
              aria-label={
                isOwnRepost ? "Your repost" : isReposted ? "Reposted" : "Repost"
              }
              title={
                isOwnRepost ? "You cannot repost your own repost" : undefined
              }
            >
              <Repeat2
                className={`h-5 w-5 ${isReposted ? "fill-[#1d9bf0]" : ""}`}
              />
            </Button>
            ...
            <Button
              ...
              size="sm"
              className="flex items-center gap-1 px-2"
              onClick={() => setLocation(`/posts/${post.id}/analytics`)}
          ...
          >
            <Button
              variant="ghost"
              size="sm"
              className="ml-1 flex items-center px-2"
              onClick={handleShare}
              aria-label="Share post"
            >
              <Share2 className="h-5 w-5" />
                      ...
                      {userSearchResults.map((foundUser) => (
                        ...
                        >
                          <Avatar className="h-8 w-8 mr-2">
                            <AvatarImage
                              src={foundUser.profileImageUrl || undefined}
