import { ArrowLeft, BookOpen, Bot, CalendarPlus, Camera, FileText, Film, FolderOpen, Image, LayoutDashboard, Megaphone, Mic, PackagePlus, Radio, Repeat2, Send, Sparkles, Video } from "lucide-react";
  ...
  return (
    <main className="min-h-[calc(100dvh-3.5rem)] bg-black pb-20 text-white">
      ...
      <section className="px-4 pt-8">
        ...
        <button onClick={() => setLocation("/vision")} className="mt-3 flex w-full items-center gap-4 rounded-2xl border border-zinc-800 bg-zinc-950 p-4 text-left transition-colors hover:bg-zinc-900">
          <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-cyan-400 text-black"><Camera className="h-5 w-5" /></span>
          <span className="min-w-0 flex-1"><span className="block text-sm font-bold">Open Vision Studio</span><span className="mt-1 block text-xs leading-5 text-zinc-500">Frame camera or screen capture, inspect grounded scene metrics, and control explicit tracking without hidden recording.</span></span>
        </button>
