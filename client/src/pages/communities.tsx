...
type CommunityEvent = {
  ...
  description: string;
  attendanceStatus: "going" | "interested" | "declined" | null;
  goingCount: number;
};
type CommunityRoom = {
  id: string;
...
const Communities = () => {
  const { activeCommunityId, activeChannelId, setActiveCommunity } =
    useCommunitiesStore();
  ...
  const { data: allCommunities = [], isLoading: isLoadingCommunities } =
    useQuery<CommunityType[]>({ queryKey: ["/api/communities"] });
  ...
  useEffect(() => {
    ...
    const reconciledId = reconcileCommunitySelection(
      activeCommunityId,
      allCommunities,
    );
  ...
  const { data: membership, isLoading: isLoadingMembership } = useQuery<{
    isMember: boolean;
    membership: { role: string; status: "active" | "muted" | "banned" } | null;
  }>({
    queryKey: ["/api/communities", activeCommunityId, "membership"],
    enabled: activeCommunityId !== null,
    queryFn: async () => {
      const response = await fetch(
        `/api/communities/${activeCommunityId}/membership`,
      );
  ...
  >({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/communities/${activeCommunityId}/channels`,
      );
  ...
  const { data: communityEvents = [] } = useQuery<CommunityEvent[]>({
    ...
    queryFn: async () =>
      (
        await apiRequest("GET", `/api/communities/${activeCommunityId}/events`)
      ).json(),
  });
  const { data: communityRooms = [] } = useQuery<CommunityRoom[]>({
    ...
    queryFn: async () =>
      (
        await apiRequest("GET", `/api/communities/${activeCommunityId}/rooms`)
      ).json(),
  });
  const { data: roomProviders } = useQuery<{
  ...
  const activeRoom =
    communityRooms.find((room) => room.id === activeRoomId) ?? null;
  useEffect(() => {
    if (!activeRoomId && communityRooms[0])
  ...
  const { data: roomNotes = [] } = useQuery<CommunityRoomNote[]>({
    ...
    queryFn: async () =>
      (
        await apiRequest("GET", `/api/community-rooms/${activeRoomId}/notes`)
      ).json(),
  });
  const { data: roomActionItems = [] } = useQuery<CommunityRoomActionItem[]>({
    ...
    queryFn: async () =>
      (
        await apiRequest(
          "GET",
          `/api/community-rooms/${activeRoomId}/action-items`,
        )
      ).json(),
  });
  const { data: communityMembers = [] } = useQuery<CommunityMember[]>({
    ...
    queryFn: async () =>
      (
        await apiRequest("GET", `/api/communities/${activeCommunityId}/members`)
      ).json(),
  });
  const { data: onboarding } = useQuery<CommunityOnboarding>({
    queryKey: ["/api/communities", activeCommunityId, "onboarding"],
    enabled: activeCommunityId !== null && isMember,
    queryFn: async () =>
      (
        await apiRequest(
          "GET",
          `/api/communities/${activeCommunityId}/onboarding`,
        )
      ).json(),
  });
  const { data: leaderboard } = useQuery<{
    ...
    queryKey: ["/api/communities", activeCommunityId, "leaderboard"],
    enabled: activeCommunityId !== null && isMember,
    queryFn: async () =>
      (
        await apiRequest(
          "GET",
          `/api/communities/${activeCommunityId}/leaderboard`,
        )
      ).json(),
  });
  ...
  useEffect(() => {
    ...
    setOnboardingAnswers(
      Object.fromEntries(
        onboarding.responses.map((response) => [
          response.questionId,
          response.answer,
        ]),
      ),
  ...
  useEffect(() => {
    if (!onboardingSettingsOpen || !onboarding) return;
    setOnboardingDrafts(
  ...
  }, [onboarding, onboardingSettingsOpen]);
  ...
  const joinCommunityMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await apiRequest(
        "POST",
        `/api/communities/${activeCommunityId}/join`,
        {},
      );
      return response.json();
    },
  ...
  const archiveCommunityMutation = useMutation({
    mutationFn: async () => {
      ...
      await apiRequest(
        "POST",
        `/api/communities/${activeCommunityId}/archive`,
        {},
      );
    },
    onSuccess: () => {
  ...
  const rsvpMutation = useMutation({
    mutationFn: async ({
      ...
      eventId: string;
      status: string;
    }) => apiRequest("PUT", `/api/events/${eventId}/rsvp`, { status }),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["/api/communities", activeCommunityId, "events"],
      }),
  ...
  const saveRoomMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        title: roomTitle,
        startsAt: new Date(roomStartsAt).toISOString(),
        provider: roomProvider,
        joinUrl: roomJoinUrl || null,
      };
      const response = editingRoomId
        ? await apiRequest(
            "PATCH",
            `/api/community-rooms/${editingRoomId}`,
            payload,
          )
        : await apiRequest(
            "POST",
            `/api/communities/${activeCommunityId}/rooms`,
            payload,
          );
  ...
  const roomStatusMutation = useMutation({
    ...
    }) => apiRequest("PATCH", `/api/community-rooms/${roomId}`, { status }),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["/api/communities", activeCommunityId, "rooms"],
      }),
  ...
  const roomRsvpMutation = useMutation({
    ...
    }) => apiRequest("PUT", `/api/community-rooms/${roomId}/rsvp`, { status }),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["/api/communities", activeCommunityId, "rooms"],
      }),
  ...
  const roomCheckInMutation = useMutation({
    mutationFn: async (roomId: string) =>
      apiRequest("POST", `/api/community-rooms/${roomId}/check-in`, {}),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["/api/communities", activeCommunityId, "rooms"],
      }),
  ...
  const createRoomNoteMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiRequest("POST", `/api/community-rooms/${activeRoomId}/notes`, {
        content: roomNote,
      });
    },
    onSuccess: () => {
  ...
  const completeRoomActionMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest(
        "PATCH",
        `/api/community-rooms/${activeRoomId}/action-items/${id}`,
        { completed },
      );
    },
    onSuccess: () =>
  ...
  const updateMemberMutation = useMutation({
    mutationFn: async ({
      ...
      return apiRequest(
        "PATCH",
        `/api/communities/${activeCommunityId}/members/${userId}`,
        body,
      );
    },
    onSuccess: () =>
  ...
  const completeOnboardingMutation = useMutation({
    mutationFn: async () => {
      if (!activeCommunityId || !onboarding)
        throw new Error("Onboarding is not ready");
      return apiRequest(
        "POST",
        `/api/communities/${activeCommunityId}/onboarding/complete`,
        {
          answers: onboarding.questions
            .filter((question) => onboardingAnswers[question.id] !== undefined)
            .map((question) => ({
              questionId: question.id,
              value: onboardingAnswers[question.id],
            })),
        },
      );
    },
  ...
  const saveOnboardingQuestionsMutation = useMutation({
    mutationFn: async () => {
      if (!activeCommunityId) throw new Error("Choose a community first");
      const questions = onboardingDrafts.map((draft) => {
        const labels =
          draft.kind === "text"
            ? []
            : draft.options
      ...
      return apiRequest(
        "PUT",
        `/api/communities/${activeCommunityId}/onboarding/questions`,
        { questions },
      );
    },
    onSuccess: () => {
  ...
  const { data: channelPolls = [] } = useQuery<ChannelPoll[]>({
    ...
    queryFn: async () =>
      (
        await apiRequest("GET", `/api/channels/${activeChannelId}/polls`)
      ).json(),
  });
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async () => {
      if (!currentUser || !activeChannelId)
        throw new Error("Not ready to send message");
  ...
  const createPollMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiRequest("POST", `/api/channels/${activeChannelId}/polls`, {
  ...
  const votePollMutation = useMutation({
    mutationFn: async ({
      ...
      pollId: number;
      optionId: number;
    }) => apiRequest("POST", `/api/channel-polls/${pollId}/vote`, { optionId }),
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["/api/channels", activeChannelId, "polls"],
      }),
  ...
  const searchMessages = (
    communitySearch.trim() ? visibleMessages : (messages ?? [])
  )
    .filter((message) => message.parentMessageId === null)
    .slice(0, 6);
  const pinnedMessages = visibleMessages.filter(
    (message) => message.isPinned && message.parentMessageId === null,
  );
  const rootMessages = visibleMessages.filter(
  ...
  const canPinMessages = ["owner", "admin"].includes(
    membership?.membership?.role ?? "",
  );
  const canManageCommunity = ["owner", "admin"].includes(
    membership?.membership?.role ?? "",
  );
  const onboardingReady = Boolean(
    onboarding?.questions.every((question) => {
  ...
  if (isLoadingCommunities) {
    return (
      <main className="flex min-h-dvh items-center justify-center bg-black px-6 text-center text-sm text-zinc-500">
    ...
    return (
      <main className="flex min-h-dvh flex-col bg-black px-5 pb-24 pt-5 text-white">
        ...
        <section className="m-auto w-full max-w-sm rounded-3xl border border-zinc-800 bg-zinc-950 p-7 text-center shadow-2xl shadow-black">
          <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-zinc-900">
            <Hash className="h-7 w-7 text-zinc-300" />
          </span>
          <h1 className="mt-6 text-2xl font-bold">
            Communities are coming together
          </h1>
          <p className="mt-3 text-sm leading-6 text-zinc-500">
            Open your own community for conversations, rooms, and creator
            programs—or explore once other creators launch theirs.
          </p>
          {communityComposerOpen ? (
            ...
            >
              <Input
                ...
                className="border-zinc-800 bg-black text-white placeholder:text-zinc-600"
              />
              <textarea
                required
                maxLength={1_000}
                ...
                className="min-h-24 w-full rounded-md border border-zinc-800 bg-black px-3 py-2 text-sm text-white placeholder:text-zinc-600 outline-none focus:border-zinc-600"
              />
            ...
            <>
              <Button
                className="mt-7 h-11 w-full rounded-full bg-white font-bold text-black hover:bg-zinc-200"
                onClick={() =>
                  currentUser
  ...
  if (!isMember) {
    return (
      <main className="flex min-h-dvh flex-col bg-black px-5 pb-24 pt-5 text-white">
        ...
        <section className="m-auto w-full max-w-sm rounded-3xl border border-zinc-800 bg-zinc-950 p-7 text-center shadow-2xl shadow-black">
          <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-zinc-900">
            <LockKeyhole className="h-7 w-7 text-zinc-300" />
          </span>
          <h1 className="mt-6 text-2xl font-bold">
          ...
          <p className="mt-3 text-sm leading-6 text-zinc-500">
            {community?.description ??
              "Community conversations, channels, and live rooms are available to members."}
          </p>
          <Button
            className="mt-7 h-11 w-full rounded-full bg-white font-bold text-black hover:bg-zinc-200"
            disabled={joinCommunityMutation.isPending || !activeCommunityId}
            onClick={() =>
              community?.accessProductId
                ? setLocation(
                    `/marketplace/product/${community.accessProductId}`,
                  )
  ...
  return (
    <>
      <Dialog
        ...
        onOpenChange={(open) =>
          open ? setCommunityComposerOpen(true) : closeCommunityComposer()
        }
      >
        <DialogContent className="border-zinc-800 bg-zinc-950 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-zinc-400">
              Create a member space for conversations, rooms, events, and
              offers.
            </DialogDescription>
          </DialogHeader>
          <form
          ...
          >
            ...
            >
              ...
              <Input
                ...
                value={communityName}
                onChange={(event) => setCommunityName(event.target.value)}
                placeholder="Creative Operators"
                className="mt-1 border-zinc-700 bg-black text-white placeholder:text-zinc-600"
              />
            </label>
            <label
              className="block text-xs font-semibold text-zinc-400"
            ...
            >
              ...
              <textarea
                ...
                className="mt-1 min-h-28 w-full rounded-md border border-zinc-700 bg-black px-3 py-2 text-sm text-white placeholder:text-zinc-600 outline-none focus:border-zinc-500"
              />
            </label>
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          open ? setChannelComposerOpen(true) : closeChannelComposer()
        }
      >
        <DialogContent className="border-zinc-800 bg-zinc-950 text-white">
          <DialogHeader>
      ...
      >
        <DialogContent className="max-h-[90dvh] overflow-y-auto border-zinc-800 bg-zinc-950 text-white sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Member onboarding</DialogTitle>
            <DialogDescription className="text-zinc-400">
              Ask only what improves the member experience. Saving a changed
              flow asks active members to complete it again.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3" data-testid="onboarding-question-editor">
            {onboardingDrafts.map((draft, index) => (
              <div
                key={draft.id ?? `new-${index}`}
                className="rounded-xl border border-zinc-800 bg-black p-4"
              >
                <div className="flex items-start gap-2">
                  <label className="min-w-0 flex-1 text-xs font-semibold text-zinc-400">
                    Question
                    <Input
                      value={draft.prompt}
                      maxLength={500}
                  ...
                  <Button
                    ...
                    className="mt-5 text-zinc-500 hover:bg-zinc-900 hover:text-red-300"
                    onClick={() =>
                      setOnboardingDrafts((current) =>
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <label className="text-xs font-semibold text-zinc-400">
                          ...
                          current.map((item, itemIndex) =>
                            itemIndex === index
                              ? {
                                  ...item,
                                  kind: event.target
                                    .value as OnboardingQuestion["kind"],
                                }
                              : item,
                          ),
                        )
                    ...
                    >
                      ...
                      <option value="multi_select">Choose several</option>
                    </select>
                  </label>
                  <label className="flex items-center gap-2 self-end rounded-md border border-zinc-800 px-3 py-2.5 text-sm text-zinc-300">
                    <input
                      type="checkbox"
                ...
                {draft.kind !== "text" && (
                  <label className="mt-3 block text-xs font-semibold text-zinc-400">
                    ...
                    <Input
                      ...
                      placeholder="Planning, Publishing, Monetization"
                      className="mt-1 border-zinc-700 bg-zinc-950 text-white"
                    />
                  </label>
                )}
            ...
            {onboardingDrafts.length === 0 && (
              <p className="rounded-xl border border-dashed border-zinc-800 px-4 py-8 text-center text-sm text-zinc-500">
                No questions means members enter immediately.
              </p>
            )}
            <Button
              type="button"
              variant="outline"
              disabled={onboardingDrafts.length >= 12}
              className="w-full border-dashed border-zinc-700 bg-black text-zinc-300 hover:bg-zinc-900 hover:text-white"
              onClick={() =>
                setOnboardingDrafts((current) => [
                  ...current,
                  { prompt: "", kind: "text", options: "", required: true },
                ])
              }
            >
            ...
            {saveOnboardingQuestionsMutation.isError && (
              <p role="alert" className="text-sm text-red-300">
                {saveOnboardingQuestionsMutation.error.message}
              </p>
            )}
            <div className="flex justify-end gap-2">
              <Button
              ...
              <Button
                type="button"
                disabled={
                  saveOnboardingQuestionsMutation.isPending ||
                  onboardingDrafts.some(
                    (draft) =>
                ...
                className="bg-white text-black hover:bg-zinc-200"
                onClick={() => saveOnboardingQuestionsMutation.mutate()}
              >
                {saveOnboardingQuestionsMutation.isPending
                  ? "Saving…"
                  : "Save onboarding"}
      ...
      <Dialog open={leaderboardOpen} onOpenChange={setLeaderboardOpen}>
        <DialogContent className="max-h-[88dvh] overflow-y-auto border-zinc-800 bg-zinc-950 text-white sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>Community leaderboard</DialogTitle>
            <DialogDescription className="text-zinc-400">
              Points reward meaningful participation with daily anti-spam caps.
            </DialogDescription>
          </DialogHeader>
          {leaderboard?.current && (
            <section
              className="rounded-xl border border-zinc-700 bg-black p-4"
              data-testid="current-community-rank"
            >
              ...
              <div className="mt-2 flex items-end justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-zinc-400">
                    #{leaderboard.current.rank} · {leaderboard.current.points}{" "}
                    points
                  </p>
                </div>
                <p className="text-xs text-zinc-500">
                  {leaderboard.current.pointsToNext
                    ? `${leaderboard.current.pointsToNext} to next level`
                    : "Top level"}
                </p>
              </div>
            </section>
          )}
          <div className="space-y-2" data-testid="community-leaderboard">
            {leaderboard?.entries.map((entry) => (
              <div
                key={entry.userId}
                className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-black p-3"
              >
                <span className="w-7 text-center text-sm font-bold text-zinc-500">
                  #{entry.rank}
                </span>
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-zinc-800 text-xs font-bold">
                ...
                {entry.rank <= 3 && (
                  <Trophy
                    className={`h-4 w-4 ${entry.rank === 1 ? "text-amber-300" : "text-zinc-500"}`}
                  />
                )}
              </div>
            ))}
      ...
      <Sheet open={membersOpen} onOpenChange={setMembersOpen}>
        <SheetContent
          side="right"
          className="w-full border-l border-zinc-800 bg-black p-0 text-white sm:max-w-md"
        >
          ...
          <div className="h-[calc(100dvh-81px)] space-y-2 overflow-y-auto p-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span
                          className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${member.status === "active" ? "bg-zinc-800 text-zinc-300" : "bg-amber-500/15 text-amber-300"}`}
                        >
                          {member.status}
                        </span>
                      </div>
                      ...
                      {canManageThisMember && (
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              updateMemberMutation.mutate({
                                ...
                                body: {
                                  role: event.target
                                    .value as CommunityMember["role"],
                                },
                              })
                            }
                            disabled={updateMemberMutation.isPending}
                            className="h-9 rounded-md border border-zinc-700 bg-black px-2 text-xs text-white"
                          >
                          ...
                          <label
                            className="sr-only"
                            htmlFor={`status-${member.userId}`}
                          >
                            Access status for {member.displayName}
                          </label>
                          <select
                            id={`status-${member.userId}`}
                            value={member.status}
                            onChange={(event) =>
                              updateMemberMutation.mutate({
                                userId: member.userId,
                                body: {
                                  status: event.target
                                    .value as CommunityMember["status"],
                                },
                              })
                            }
                            disabled={updateMemberMutation.isPending}
            ...
            {["owner", "admin"].includes(
              ...
              <p className="px-1 pt-3 text-xs leading-5 text-zinc-500">
                <Shield className="mr-1 inline h-3.5 w-3.5" />
                Owners can manage roles. Moderation actions are retained in the
      ...
      <AlertDialog open={archiveDialogOpen} onOpenChange={setArchiveDialogOpen}>
        <AlertDialogContent className="border-zinc-800 bg-zinc-950 text-white">
          <AlertDialogHeader>
            <AlertDialogTitle>
              Archive {community?.name ?? "community"}?
            </AlertDialogTitle>
            <AlertDialogDescription className="text-zinc-400">
              This closes access to its channels, rooms, and member space. It
              will no longer appear in discovery, while its history remains
              retained for audit purposes.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-zinc-700 bg-black text-white hover:bg-zinc-900 hover:text-white">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => archiveCommunityMutation.mutate()}
              disabled={archiveCommunityMutation.isPending}
              className="bg-red-600 text-white hover:bg-red-500"
            >
              {archiveCommunityMutation.isPending
                ? "Archiving…"
                : "Archive community"}
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      <div className="flex h-screen overflow-hidden bg-black pb-14">
        ...
        <Sheet>
          ...
          <SheetContent
            side="left"
            className="w-4/5 border-r border-zinc-800 bg-black p-0 text-white"
          >
            <ChannelSidebar
              isMobile
              isMember={isMember}
              canManage={canManageCommunity}
              onCreateChannel={() => setChannelComposerOpen(true)}
            />
          </SheetContent>
        </Sheet>

        <aside
          className="hidden w-16 shrink-0 flex-col items-center gap-4 border-r border-zinc-800 bg-[#111113] py-4 md:flex"
        ...
        >
          {allCommunities.map((item) => (
            <button
              ...
              className={`flex h-11 w-11 items-center justify-center rounded-full border text-sm font-bold transition-transform hover:scale-105 ${item.id === activeCommunityId ? "border-white text-white" : "border-zinc-700 text-zinc-400"}`}
              style={{
                backgroundColor:
                  item.id === activeCommunityId ? item.iconColor : "#242426",
              }}
          ...
          <button
            ...
            className="flex h-11 w-11 items-center justify-center rounded-full border border-dashed border-zinc-700 text-zinc-500 transition-colors hover:border-zinc-500 hover:bg-zinc-900 hover:text-white"
            aria-label="Open a community"
          >
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-black text-white">
          ...
          <div className="flex h-16 items-center border-b border-zinc-800 px-4">
            ...
            <div className="ml-auto flex shrink-0 items-center space-x-1">
              <Button
                variant="ghost"
                size="icon"
                className="rounded-full text-zinc-400 hover:bg-zinc-900 hover:text-white md:hidden"
                aria-label="Open a community"
                onClick={() => setCommunityComposerOpen(true)}
              >
                <Plus className="h-5 w-5" />
              ...
              {canManageCommunity && (
                <Button
                  ...
                  size="icon"
                  className="rounded-full text-zinc-400 hover:bg-zinc-900 hover:text-white"
                  aria-label="Configure member onboarding"
              ...
              <Button
                ...
                onClick={() => setCommunitySearchOpen((open) => !open)}
              >
              ...
              {membership?.membership?.role === "owner" && (
                <Button
                  variant="ghost"
                  size="icon"
                  className="rounded-full text-zinc-400 hover:bg-zinc-900 hover:text-red-300"
                  aria-label="Archive community"
          ...
          {communitySearchOpen && (
            <section
              ...
              className="border-b border-zinc-800 bg-black px-4 py-3"
            >
              <div className="flex items-center gap-2">
                <Input
                  ...
                  className="border-0 bg-zinc-900 text-white placeholder:text-zinc-500"
                />
                <Button
                  type="button"
                  variant="ghost"
                  ...
                  className="shrink-0 text-zinc-400 hover:bg-zinc-900 hover:text-white"
                  onClick={() => {
              ...
              <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-400">
                {communitySearch.trim() ? "Messages" : "Recent messages"}
              </p>
              <div className="mt-2 grid gap-1">
                {searchMessages.map((message) => (
                  <button
                    key={message.id}
                    type="button"
                    aria-pressed={selectedSearchMessageId === message.id}
                    onClick={() => {
                      setSelectedSearchMessageId(message.id);
                      requestAnimationFrame(() =>
                        document
                          .getElementById(`community-message-${message.id}`)
                          ?.scrollIntoView({
                            behavior: "smooth",
                            block: "center",
                          }),
                    ...
                    className={`rounded-xl px-3 py-2 text-left transition-colors ${selectedSearchMessageId === message.id ? "ring-2 ring-white bg-zinc-900" : "hover:bg-zinc-950"}`}
                  >
                ...
                {searchMessages.length === 0 && (
                  <p className="rounded-xl border border-dashed border-zinc-800 px-3 py-4 text-xs text-zinc-400">
                ...
                {isLoadingChannels
                  ...
                  : channels.map((channel) => (
                      <Button
                        key={channel.id}
                        variant={
                          channel.id === activeChannelId ? "default" : "outline"
                        }
          ...
          <div className="flex-1 space-y-6 overflow-y-auto p-4">
            {onboarding &&
              ...
              onboarding.questions.length > 0 && (
                <section
                  className="rounded-2xl border border-white/20 bg-gradient-to-br from-zinc-900 to-black p-5"
                  data-testid="community-onboarding"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h2 className="font-bold">Make this community yours</h2>
                      <p className="mt-1 text-sm text-zinc-400">
                        Complete {onboarding.progress.requiredCount} onboarding{" "}
                        {onboarding.progress.requiredCount === 1
                          ? "question"
                          : "questions"}{" "}
                        so the experience can match your goals.
                      </p>
                    </div>
                  </div>
                      ...
                      >
                        ...
                        {question.kind === "text" ? (
                          <textarea
                            ...
                            maxLength={2_000}
                            value={
                              typeof onboardingAnswers[question.id] === "string"
                                ? (onboardingAnswers[question.id] as string)
                                : ""
                            }
                            onChange={(event) =>
                            ...
                            className="mt-3 min-h-24 w-full rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-white outline-none focus:border-zinc-600"
                          />
                        ) : (
                          <div className="mt-3 grid gap-2 sm:grid-cols-2">
                            {question.options.map((option) => {
                              const current = onboardingAnswers[question.id];
                              const checked =
                              ...
                              return (
                                <label
                                  key={option.id}
                                  className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2.5 text-sm ${checked ? "border-white bg-zinc-900 text-white" : "border-zinc-800 text-zinc-400"}`}
                                >
                                  <input
                                    type={
                                      question.kind === "multi_select"
                  ...
                  <Button
                    ...
                    disabled={
                      !onboardingReady || completeOnboardingMutation.isPending
                    }
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-zinc-500">
                      {leaderboard.current.points} community points
                      {leaderboard.current.pointsToNext
                        ? ` · ${leaderboard.current.pointsToNext} to next level`
                        : ""}
                    </p>
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="shrink-0 border-zinc-700 bg-black text-white hover:bg-zinc-900"
                  onClick={() => setLeaderboardOpen(true)}
                >
                  Leaderboard
                </Button>
            ...
            <section className="border-b border-zinc-800 pb-6">
              <span className="flex h-16 w-16 items-center justify-center rounded-full bg-zinc-800">
                <Hash className="h-9 w-9" />
              </span>
              <h1 className="mt-5 text-3xl font-bold tracking-tight">
                Welcome to #{activeChannel?.name ?? "the community"}!
              </h1>
              <p className="mt-2 text-base text-zinc-500">
                This is the start of the #{activeChannel?.name ?? "community"}{" "}
                channel.
              </p>
            </section>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start gap-2">
                      ...
                      {["owner", "admin"].includes(
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          className="-mr-2 -mt-2 h-8 px-2 text-zinc-400 hover:bg-zinc-900 hover:text-white"
                          onClick={() =>
                    ...
                    <div className="mt-3 flex gap-2">
                      <Button
                        ...
                        aria-pressed={event.attendanceStatus === "going"}
                        className={
                          event.attendanceStatus === "going"
                      ...
                      >
                        Going
                        {event.goingCount > 0 ? ` · ${event.goingCount}` : ""}
                      </Button>
                      <Button
                        size="sm"
                        variant={
                        ...
                        aria-pressed={event.attendanceStatus === "interested"}
                        className={
                          event.attendanceStatus === "interested"
                            ? "bg-white text-black hover:bg-zinc-200"
                            : "border-zinc-700 bg-black text-white hover:bg-zinc-900"
            ...
            <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-sm font-bold">Live rooms</h2>
                    <p className="mt-0.5 text-xs text-zinc-500">
                      Calls and office hours live beside the conversation.
                    </p>
                  </div>
                </div>
                {["owner", "admin"].includes(
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const shouldOpen =
                        !roomComposerOpen || editingRoomId !== null;
                      setEditingRoomId(null);
                      setRoomTitle("");
                      setRoomStartsAt("");
                      setRoomProvider("manual_link");
                      setRoomJoinUrl("");
                      setRoomComposerOpen(shouldOpen);
                    }}
                  >
              ...
              {roomComposerOpen && (
                ...
                >
                  <p className="text-sm font-bold">
                    {editingRoomId ? "Edit scheduled room" : "Schedule a room"}
                  </p>
                  <Input
                    required
                    maxLength={160}
                  ...
                  <Input
                    ...
                    onInput={(event) =>
                      setRoomStartsAt(event.currentTarget.value)
                    }
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <select
                      value={roomProvider}
                      onChange={(event) => {
                        setRoomProvider(event.target.value);
                        if (event.target.value === "livekit")
                          setRoomJoinUrl("");
                      }}
                      className="h-10 rounded-md border border-zinc-800 bg-black px-3 text-sm text-white"
                    >
                      <option value="manual_link">Room link</option>
                      <option value="google_meet">Google Meet</option>
                      <option value="zoom">Zoom</option>
                      <option value="livekit">
                        Native CreativesOS room
                        {roomProviders?.livekit.configured
                          ? ""
                          : " (setup pending)"}
                      </option>
                    </select>
                    {roomProvider === "livekit" ? (
                      <div className="flex h-10 items-center rounded-md border border-zinc-800 bg-zinc-950 px-3 text-xs text-zinc-500">
                      ...
                      <Input
                        type="url"
                        value={roomJoinUrl}
                        onChange={(event) => setRoomJoinUrl(event.target.value)}
                        placeholder="https://… (optional until provider setup)"
                        className="border-zinc-800 bg-black text-white placeholder:text-zinc-600"
                      />
                    )}
                  </div>
                  <p className="text-xs leading-5 text-zinc-500">
                    Recording, transcription, and AI assistance are off by
                    default. They need explicit consent and a connected
                    provider.
                  </p>
                  <div className="flex justify-end gap-2">
                    <Button
                  ...
                  {[...communityRooms]
                    .sort((a, b) => {
                      ...
                      return (
                        ...
                        new Date(b.startsAt).valueOf() -
                          new Date(a.startsAt).valueOf()
                      );
                    })
                    .map((room) => (
                      <div
                        ...
                        className="w-[min(86vw,28rem)] shrink-0 rounded-xl border border-zinc-800 bg-black p-3"
                      >
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <Radio
                              className={`h-3.5 w-3.5 ${room.status === "live" ? "text-red-400" : "text-zinc-500"}`}
                            />
                            <p className="truncate text-sm font-semibold">
                              {room.title}
                            </p>
                          ...
                          <p className="mt-1 text-xs text-zinc-500">
                            ...
                            {room.provider.replace("_", " ")} ·{" "}
                            {room.recordingEnabled
                              ? "Recording enabled"
                              : "No recording"}
                          </p>
                          <p className="mt-1 text-[11px] text-zinc-500">
                            {room.goingCount} going · {room.checkedInCount}{" "}
                            checked in
                          </p>
                        </div>
                        <div className="mt-3 flex flex-wrap gap-2">
                          <Button
                            ...
                            className="border-zinc-700 bg-black text-white hover:bg-zinc-900"
                            onClick={() => setActiveRoomId(room.id)}
                          >
                            <NotebookPen className="mr-1.5 h-3.5 w-3.5" />
                            Workspace
                          </Button>
                          {["scheduled", "live"].includes(room.status) && (
                            <>
                              <Button
                                size="sm"
                                variant={
                                  room.rsvpStatus === "going"
                                    ? "default"
                                    : "outline"
                                }
                                aria-pressed={room.rsvpStatus === "going"}
                                disabled={
                                  roomRsvpMutation.isPending || isReadOnly
                                }
                                className={
                                  room.rsvpStatus === "going"
                                    ? "bg-white text-black hover:bg-zinc-200"
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                aria-pressed={room.rsvpStatus === "interested"}
                                disabled={
                                  roomRsvpMutation.isPending || isReadOnly
                                }
                                className="border-zinc-700 bg-black text-white hover:bg-zinc-900"
                          ...
                          {room.status === "live" && !room.checkedInAt && (
                            <Button
                              size="sm"
                              variant="outline"
                              disabled={
                                roomCheckInMutation.isPending || isReadOnly
                              }
                              className="border-zinc-700 bg-black text-white hover:bg-zinc-900"
                              onClick={() =>
                                roomCheckInMutation.mutate(room.id)
                              }
                            >
                              Check in
                            </Button>
                          ...
                          {room.provider === "livekit" ? (
                            room.status === "live" ? (
                              <Button
                                size="sm"
                                disabled={!roomProviders?.livekit.configured}
                                className="bg-white text-black hover:bg-zinc-200"
                                onClick={() =>
                                  setLocation(
                                    `/communities/${activeCommunityId}/rooms/${room.id}`,
                                  )
                                }
                              >
                              ...
                              <span className="inline-flex h-9 items-center rounded-md border border-zinc-800 px-3 text-xs text-zinc-500">
                                Native room
                              </span>
                            )
                          ) : room.joinUrl ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex h-9 items-center rounded-md bg-white px-3 text-xs font-semibold text-black hover:bg-zinc-200"
                            >
                              <ExternalLink className="mr-1.5 h-3.5 w-3.5" />
                              Join
                            </a>
                          ) : (
                            <span className="inline-flex h-9 items-center rounded-md border border-zinc-800 px-3 text-xs text-zinc-600">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <h2 className="text-sm font-bold">
                        Manage scheduled room
                      </h2>
                      <p className="mt-1 text-xs text-zinc-500">
                        {activeRoom.title} ·{" "}
                        {new Date(activeRoom.startsAt).toLocaleString()}
                      </p>
                    </div>
                    <div className="flex flex-wrap justify-end gap-2">
                      <Button
                        ...
                        className="border-zinc-700 bg-black text-white hover:bg-zinc-900"
                        onClick={() => {
                          setEditingRoomId(activeRoom.id);
                          setRoomTitle(activeRoom.title);
                          setRoomStartsAt(
                            roomDateTimeValue(activeRoom.startsAt),
                          );
                          setRoomProvider(activeRoom.provider);
                          setRoomJoinUrl(activeRoom.joinUrl ?? "");
                          setRoomComposerOpen(true);
                        }}
                      >
                        <Pencil className="mr-1.5 h-3.5 w-3.5" />
                        Edit
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (
                            window.confirm(
                              `Cancel “${activeRoom.title}”? This closes RSVPs and cannot be undone.`,
                            )
                          )
                            roomStatusMutation.mutate({
                              roomId: activeRoom.id,
            ...
            {activeRoom && (
              <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-sm font-bold">
                      {activeRoom.title} workspace
                    </h2>
                    <p className="mt-0.5 text-xs text-zinc-500">
                      Notes and follow-ups stay within this community.
                    </p>
                  </div>
                </div>
                <div className="mt-4 grid gap-4 lg:grid-cols-2">
                  <div>
                    ...
                    <textarea
                      ...
                      maxLength={20_000}
                      disabled={isReadOnly}
                      placeholder="Capture a decision, link, or takeaway…"
                      className="mt-3 min-h-20 w-full rounded-lg border border-zinc-800 bg-black px-3 py-2 text-sm text-white placeholder:text-zinc-600 outline-none focus:border-zinc-600 disabled:opacity-50"
                    />
                    <Button
                      size="sm"
                      ...
                      disabled={
                        isReadOnly ||
                        !roomNote.trim() ||
                        createRoomNoteMutation.isPending
                      }
                      onClick={() => createRoomNoteMutation.mutate()}
                    >
                      {createRoomNoteMutation.isPending
                        ? "Saving…"
                        : "Add note"}
                  ...
                  <div>
                    ...
                    <div className="mt-2 max-h-48 space-y-2 overflow-y-auto pr-1">
                      {roomActionItems.length === 0 ? (
                        ...
                        roomActionItems.map((item) => (
                          <label
                            key={item.id}
                            className="flex cursor-pointer items-start gap-2 rounded-lg bg-black p-3 text-sm"
                          >
                            <input
                              type="checkbox"
                              className="mt-0.5"
                              ...
                              disabled={
                                isReadOnly ||
                                completeRoomActionMutation.isPending
                              }
                              onChange={(event) =>
                                completeRoomActionMutation.mutate({
                                  id: item.id,
                    ...
                    <Input
                      value={roomAction}
                      onChange={(event) => setRoomAction(event.target.value)}
                      maxLength={2000}
                      disabled={isReadOnly}
                      placeholder="Add a follow-up…"
                      className="mt-3 border-zinc-800 bg-black text-white placeholder:text-zinc-600"
                    />
                    <Button
                      size="sm"
                      className="mt-2 bg-white text-black hover:bg-zinc-200"
                      disabled={
                        isReadOnly ||
                        !roomAction.trim() ||
                        createRoomActionMutation.isPending
                      }
                      onClick={() => createRoomActionMutation.mutate()}
                    >
                      {createRoomActionMutation.isPending
                        ? "Adding…"
                        : "Add action item"}
            ...
            {channelPolls.length > 0 && (
              <section className="space-y-3 rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-bold">Channel polls</h2>
                </div>
                      ...
                      <div className="mt-3 space-y-2">
                        {poll.options.map((option) => {
                          const percent = totalVotes
                            ...
                            : 0;
                          const selected = poll.currentOptionId === option.id;
                          return (
                            <button
                              key={option.id}
                              type="button"
                              ...
                              className={`relative flex w-full overflow-hidden rounded-lg border px-3 py-2 text-left text-xs transition-colors ${selected ? "border-white text-white" : "border-zinc-800 text-zinc-300 hover:border-zinc-600"}`}
                            >
                              <span
                                className="absolute inset-y-0 left-0 bg-zinc-800/70"
                                style={{ width: `${percent}%` }}
                              />
                              <span className="relative flex flex-1 justify-between gap-3">
            ...
            {isLoadingMessages
              ...
              : rootMessages.map((message) => (
                  <div
                    ...
                    key={message.id}
                    className={`rounded-xl transition-shadow ${selectedSearchMessageId === message.id ? "ring-2 ring-white ring-offset-4 ring-offset-black" : ""}`}
                  >
                    <ChatMessage
                      message={message}
                    ...
                    {(repliesByParent.get(message.id) ?? []).map((reply) => (
                      <ChatMessage key={reply.id} message={reply} isReply />
                    ))}
                  </div>
                ))}
          ...
          >
            {pollComposerOpen && (
              <div className="mb-3 rounded-xl border border-zinc-800 bg-zinc-950 p-3">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold">New poll</p>
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    className="h-7 w-7 text-zinc-400 hover:bg-zinc-900 hover:text-white"
                ...
                {pollOptions.map((option, index) => (
                  <Input
                    ...
                    className="mt-2 border-zinc-800 bg-black text-white placeholder:text-zinc-600"
                  />
                ))}
            ...
            {replyingTo && (
              <div className="mb-2 flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="ml-3 h-7 shrink-0 px-2 text-zinc-400 hover:bg-zinc-900 hover:text-white"
            ...
            <div className="flex">
              <div className="flex-1 rounded-lg bg-zinc-900 flex items-center p-2">
                <Input
                  ...
                  placeholder={
                    isReadOnly
                      ? "Your community access is read-only"
                      : isMember
                        ? `Message ${activeChannel ? `#${activeChannel.name}` : "channel"}`
                  ...
                  disabled={
                    !activeChannelId || !currentUser || !isMember || isReadOnly
                  }
                />
              </div>
              <Button
                type="button"
                variant="outline"
                className="ml-2 border-zinc-700 bg-black p-2 text-zinc-300 hover:bg-zinc-900 hover:text-white"
                size="icon"
                disabled={!activeChannelId || !isMember || isReadOnly}
                aria-label="Create community poll"
