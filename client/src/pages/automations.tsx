      ...
      {section === "workflows" && (
        <>
          ...
          {selected && <section className="mx-4 mt-6 rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
            ...
            <button onClick={() => runAutomation.mutate(selected)} disabled={selected.status !== "active" || runAutomation.isPending} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-[#1d9bf0] px-4 py-3 text-sm font-bold text-black disabled:bg-zinc-800 disabled:text-zinc-500"><Play className="h-4 w-4" />{selected.status === "active" ? "Run automation" : "Activate to run"}</button>
          </section>}
        </>
      )}
      ...
      {section === "activity" && <section className="px-4 pt-6">
        ...
        {runDetailQuery.data && <article className="mt-5 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
          ...
          <div className="max-h-80 space-y-3 overflow-y-auto p-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {runDetailQuery.data.thread?.messages.map((message) => <div key={message.id} className={`flex ${message.authorType === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[88%] rounded-2xl px-3 py-2.5 text-xs leading-5 ${message.authorType === "user" ? "bg-[#1d9bf0] text-black" : message.kind === "error" ? "bg-red-500/10 text-red-200" : "bg-zinc-900 text-zinc-300"}`}><p>{message.content}</p><p className={`mt-1 text-[9px] ${message.authorType === "user" ? "text-black/70" : "text-zinc-600"}`}>{new Date(message.createdAt).toLocaleTimeString()}</p></div></div>)}
          </div>
          {runDetailQuery.data.thread && <form onSubmit={(event) => { event.preventDefault(); const content = chatInput.trim(); if (content) sendMessage.mutate({ threadId: runDetailQuery.data!.thread!.id, content }); }} className="flex gap-2 border-t border-zinc-800 p-3"><input value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder="Continue this workflow conversation…" className="min-w-0 flex-1 rounded-xl border border-zinc-800 bg-black px-3 text-xs text-white outline-none focus:border-[#1d9bf0]" /><button type="submit" aria-label="Send workflow message" disabled={!chatInput.trim() || sendMessage.isPending} className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#1d9bf0] text-black disabled:bg-zinc-800"><Send className="h-4 w-4" /></button></form>}
        </article>}
      </section>}
    </main>
  );
