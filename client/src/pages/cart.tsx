      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-3 flex items-center justify-between">
          <strong className="text-sm text-white">
            ...
            {item.billingModel === "recurring"
              ? `/${item.billingInterval === "year" ? "year" : "month"}`
              : ""}
          </strong>
          <button
            onClick={onRemove}
            className="inline-flex items-center gap-1 text-xs font-semibold text-zinc-500 hover:text-red-400"
...
export default function CartPage() {
  ...
  const checkoutGroups = useMemo(
    () => groupCartItemsForCheckout(items),
    [items],
  );
      ...
      if (isDemoMode) {
        await Promise.all(
          group.items.map((item) =>
            apiRequest("POST", `/api/products/${item.id}/demo-access`, {}),
          ),
        );
        ...
        toast({
          ...
          description: "Your new access is now in Purchases.",
        });
        ...
        await startStripeCheckout(
          ...
          `${checkoutKey}:${group.key}`,
          promotionCodes[group.key] ?? "",
        );
      }
    } catch (error) {
      toast({
  ...
  return (
    <main className="min-h-[calc(100dvh-3.5rem)] bg-black pb-24 text-white">
      ...
      {isLoading ? (
        <section className="px-4 py-20 text-center text-sm text-zinc-500">
          Loading your cart...
        </section>
      ) : items.length === 0 ? (
        <section className="mx-auto flex max-w-sm flex-col items-center px-6 py-24 text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-zinc-500">
            Discover courses, communities, and digital assets made for your
            creative work.
          </p>
          <Button
        ...
        <div className="space-y-5 px-4 py-5">
          {checkoutGroups.length > 1 && (
            <section className="rounded-2xl border border-[#1d9bf0]/30 bg-[#1d9bf0]/10 p-4">
              <h2 className="text-sm font-bold">
                Your cart has {checkoutGroups.length} secure checkouts
              </h2>
              <p className="mt-1 text-xs leading-5 text-zinc-400">
                Creator payouts and platform purchases are processed separately.
                Your remaining items stay in the cart.
              </p>
            </section>
          )}
          ...
          {checkoutGroups.map((group) => (
            ...
            >
              ...
              <div
                className={
                  removingProductId
                    ? "pointer-events-none px-4 opacity-70"
                    : "px-4"
                }
              >
                {group.items.map((item) => (
              ...
              <div className="border-t border-zinc-800 p-4">
                <Input
                  className="mb-3 border-zinc-800 bg-black uppercase"
                  aria-label={`Promotion code for ${group.label}`}
                  placeholder="Promotion code (optional)"
                  value={promotionCodes[group.key] ?? ""}
                  onChange={(event) =>
                ...
                <div className="mb-3 flex items-center justify-between text-sm font-bold">
                  <span>
                    {group.items[0]?.billingModel === "recurring"
                      ? "Recurring total"
                      : "Group total"}
                  </span>
                  <span>
                    ...
                    {group.items[0]?.billingModel === "recurring"
                      ? `/${group.items[0].billingInterval === "year" ? "year" : "month"}`
                      : ""}
                  </span>
                </div>
                <Button
                  ...
                  disabled={
                    checkingOutGroup !== null || removingProductId !== null
                  }
                  onClick={() => void checkout(group)}
                >
                  <CheckCircle2 className="mr-2 h-4 w-4" />
                  {checkingOutGroup === group.key
          ...
          <section className="rounded-2xl border border-zinc-800 bg-black p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-zinc-500">
              {isDemoMode
                ? "Demo checkout grants access immediately."
                : "Payments are securely processed by Stripe. Prices are revalidated by the server."}
            </p>
          </section>
        </div>
      )}
