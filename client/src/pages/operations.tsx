...
export default function OperationsPage() {
  ...
  const { data, isLoading } = useQuery<{ windowDays: number; mediaWorkers: { regions: string[]; activeNodes: number; drainingNodes: number; staleNodes: number; activeJobs: number; maxConcurrency: number }; services: Service[] }>({ queryKey: ["/api/operations"], queryFn: async () => (await apiRequest("GET", "/api/operations")).json() });
  ...
  return <main className="min-h-screen bg-black pb-24 text-white">
    ...
    <section className="mx-auto max-w-6xl space-y-5 px-4 py-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Summary icon={ShieldCheck} label="Published objectives" value={String(data?.services.length ?? 9)} /><Summary icon={Activity} label="Measured services" value={String(data?.services.filter((item) => item.observed.total > 0).length ?? 0)} /><Summary icon={Gauge} label="Worker capacity" value={`${data?.mediaWorkers.activeJobs ?? 0}/${data?.mediaWorkers.maxConcurrency ?? 0}`} /><Summary icon={Gauge} label="Active regions" value={String(data?.mediaWorkers.regions.length ?? 0)} /></div>
      {data?.mediaWorkers && <div className={`rounded-2xl border p-4 text-sm ${data.mediaWorkers.staleNodes ? "border-red-500/25 bg-red-500/5 text-red-100" : "border-emerald-500/20 bg-emerald-500/5 text-emerald-100"}`}><div className="flex gap-3"><Activity className="mt-0.5 h-5 w-5 shrink-0" /><p>{data.mediaWorkers.activeNodes} active media worker{data.mediaWorkers.activeNodes === 1 ? "" : "s"} across {data.mediaWorkers.regions.join(", ") || "no registered region"}. {data.mediaWorkers.drainingNodes} draining; {data.mediaWorkers.staleNodes} stale.</p></div></div>}
