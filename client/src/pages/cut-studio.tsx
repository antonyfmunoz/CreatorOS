...
type ProjectMedia = { id: string; assetId: string; name: string; duration: number; mediaKind: "video" | "audio" | "image" | "font"; createdAt: string };
...
export default function CutStudioPage() {
  ...
  const uploadPrivateMedia = async (file: File, mediaKind: "video" | "audio" | "image" | "font") => {
    const assetKind = mediaKind === "image" ? "photo" : mediaKind === "font" ? "cut-font" : mediaKind;
    const mimeType = mediaKind === "font" ? (/\.otf$/i.test(file.name) ? "font/otf" : "font/ttf") : file.type;
  ...
  const uploadProjectMedia = async (file: File) => {
    if (!project) return;
    const mediaKind = file.type.startsWith("audio/") ? "audio" : file.type.startsWith("video/") ? "video" : file.type.startsWith("image/") ? "image" : /\.(ttf|otf)$/i.test(file.name) || file.type.startsWith("font/") ? "font" : null;
    if (!mediaKind) return setMessage("Choose a video, audio, image, TTF, or OTF file");
    setBusy("media");
    try {
      const duration = mediaKind === "image" || mediaKind === "font" ? project.duration : await mediaDuration(file);
      const asset = await uploadPrivateMedia(file, mediaKind);
      ...
      setMessage(mediaKind === "font" ? "Private font added to this project" : "Media added to this project");
