...
type Job = { id: string; kind: "transcribe" | "highlights" | "render" | "proxy"; state: "queued" | "running" | "done" | "error" | "cancelled"; detail: string; progress: number; artifactAssetId?: string | null; output?: { candidates?: Highlight[]; filename?: string; mediaId?: string; originalAssetId?: string; maxWidth?: number; timelineSnapshot?: "captured" | "composition" | "legacy_live"; timelineRevision?: number; timelineSha256?: string } };
...
// Browser qualification exercises long-lived editing contracts directly
// (autosave, media custody, renders, and recovery). Those journeys predate
// the workspace rail and intentionally address controls without selecting a
// visual workspace first. Keep that non-production harness surface available;
// production users receive the focused NLE workspace panels.
const CUT_STUDIO_QUALIFICATION_MODE = import.meta.env.VITE_CREATOROS_QUALIFICATION_MODE === "true";
...
export default function CutStudioPage() {
  ...
  const [creativeTimelineBusy, setCreativeTimelineBusy] = useState(false);
  const creativeBusyRef = useRef(false);
  const creativeDraftRef = useRef(false);
  const [creativeDraftDirty, setCreativeDraftDirty] = useState(false);
  const timelineAwaitingSave = useMemo(() => Boolean(project && edl && JSON.stringify(edl) !== JSON.stringify(project.edl)), [project?.edl, edl]);
  // A previous save receipt is not a receipt for the current draft. Derive this
  // during render, before the debounced save effect can update its own status.
  const visibleSaveStatus = timelineAwaitingSave && saveStatus !== "Save failed" ? "Saving…" : saveStatus;
  const renderWaitingForTimeline = creativeTimelineBusy || timelineAwaitingSave || saveStatus === "Saving…";
  ...
  const hasUnsavedTimeline = useCallback(() => Boolean(currentProjectRef.current && (creativeDraftRef.current || creativeBusyRef.current || (edlRef.current && (saveInFlight.current || JSON.stringify(currentProjectRef.current.edl) !== JSON.stringify(edlRef.current))))), []);
  ...
  const openProject = useCallback(async (id: string) => {
    ...
    try {
      ...
      const switchingProject = currentProjectRef.current?.id !== next.id;
      edlRef.current = next.edl;
      currentProjectRef.current = next;
      if (switchingProject) {
        creativeDraftRef.current = false; creativeBusyRef.current = false;
  ...
  }, [pendingProjectRefresh, busy, project, edl, saveStatus, creativeDraftDirty, creativeTimelineBusy, hasUnsavedTimeline, openProject]);
  ...
  const applyEdit = useCallback((next: CutEdl) => {
    ...
    const complete = { ...next, graphics: next.graphics ?? current.graphics, markers: next.markers ?? current.markers, compounds, multicamGroups: next.multicamGroups ?? current.multicamGroups, tracks: next.tracks ?? current.tracks, audioBuses: next.audioBuses ?? current.audioBuses };
  ...
  const uploadPrivateMedia = async (file: File, mediaKind: "video" | "audio" | "image" | "font" | "lottie" | "rive" | "code_source" | "code_lockfile") => {
    const assetKind = mediaKind === "image" ? "photo" : mediaKind === "font" ? "cut-font" : mediaKind === "lottie" ? "cut-lottie" : mediaKind === "rive" ? "cut-rive" : mediaKind === "code_source" ? "cut-code-source" : mediaKind === "code_lockfile" ? "cut-code-lockfile" : mediaKind;
    const mimeType = mediaKind === "font" ? (/\.otf$/i.test(file.name) ? "font/otf" : "font/ttf") : mediaKind === "lottie" ? "application/json" : mediaKind === "rive" ? "application/octet-stream" : mediaKind === "code_source" ? "application/zip" : mediaKind === "code_lockfile" ? (/\.json$/i.test(file.name) ? "application/json" : "text/plain") : file.type;
    const uploadFile = file.type === mimeType ? file : new File([file], file.name, { type: mimeType });
    let pendingAssetId: string | null = null;
    try {
      const intent = await (await apiRequest("POST", "/api/assets/upload-intents", { kind: assetKind, filename: uploadFile.name, mimeType: uploadFile.type, sizeBytes: uploadFile.size, visibility: "private" })).json() as { asset: { id: string }; upload: { uploadUrl: string } };
      pendingAssetId = intent.asset.id;
      const stored = await fetch(intent.upload.uploadUrl, { method: "PUT", body: uploadFile, headers: { "Content-Type": uploadFile.type } });
      ...
      body.append(mediaKind, uploadFile);
  ...
  const uploadProjectMedia = async (file: File) => {
    ...
    const mediaKind = ["package-lock.json", "npm-shrinkwrap.json", "pnpm-lock.yaml", "yarn.lock"].includes(lowerName) ? "code_lockfile" : /\.zip$/i.test(file.name) ? "code_source" : file.type.startsWith("audio/") ? "audio" : file.type.startsWith("video/") ? "video" : file.type.startsWith("image/") ? "image" : /\.(ttf|otf)$/i.test(file.name) || file.type.startsWith("font/") ? "font" : /\.json$/i.test(file.name) ? "lottie" : /\.riv$/i.test(file.name) ? "rive" : null;
    if (!mediaKind) return setMessage("Choose media, a font, Lottie/Rive animation, ZIP source capsule, or package lockfile");
    setBusy("media");
    try {
      if (mediaKind === "rive") validateCutStudioRiveBytes(await file.arrayBuffer());
      const duration = mediaKind === "lottie" ? validateCutStudioLottie(JSON.parse(await file.text()) as unknown).durationSeconds : ["rive", "image", "font", "code_source", "code_lockfile"].includes(mediaKind) ? project.duration : await mediaDuration(file);
      const asset = await uploadPrivateMedia(file, mediaKind);
      ...
      setMessage(mediaKind === "font" ? "Private font added to this project" : mediaKind === "lottie" ? "Validated Lottie animation added to this project" : mediaKind === "rive" ? "Validated Rive animation added to this project" : mediaKind === "code_source" ? "Private ZIP source capsule added to this project" : mediaKind === "code_lockfile" ? "Pinned dependency lockfile added to this project" : "Media added to this project");
  ...
  const createOrUseProxy = async (media: ProjectMedia) => {
    if (!project || media.mediaKind !== "video") return;
    setBusy(`proxy:${media.id}`); setMessage("");
    try {
      let job = await (await apiRequest("POST", `/api/cut/projects/${project.id}/media-library/${media.id}/proxy`, {})).json() as Job;
      setJobs((items) => [job, ...items.filter((item) => item.id !== job.id)]);
      for (let attempt = 0; attempt < 240 && (job.state === "queued" || job.state === "running"); attempt += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, 500));
        job = await (await apiRequest("GET", `/api/cut/jobs/${job.id}`)).json() as Job;
        setJobs((items) => items.map((item) => item.id === job.id ? job : item));
      }
      if (job.state !== "done") throw new Error(job.detail || "The editing proxy could not be created");
      const descriptor = await (await apiRequest("GET", `/api/cut/jobs/${job.id}/media`)).json() as { url: string };
      setSourceMedia(media); setSourceMediaUrl(descriptor.url); setSourceIn(0); setSourceOut(media.duration);
      if (media.assetId === project.sourceAssetId) setMediaUrl(descriptor.url);
      setMessage(`${media.name} is using a lightweight editing proxy. Final renders still use the original.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "The editing proxy could not be prepared"); }
    finally { setBusy(""); }
  };
  ...
  const startJob = async (kind: "transcribe" | "highlights" | "render", body?: unknown) => {
    if (!project) return;
    if (kind === "render" && renderWaitingForTimeline) { setMessage("Finish applying and saving timeline changes before rendering."); return; }
    setBusy(kind); setMessage("");
    try {
      const job = await (await apiRequest("POST", `/api/cut/projects/${project.id}/${kind}`, body ?? {}, kind === "render" ? { "If-Match": String(revision) } : undefined)).json() as Job;
  ...
  const addImageGraphic = (media: ProjectMedia) => {
    if (!edl || !project) return;
    if (project.mediaKind !== "video") return setMessage("Image graphics require a video project");
    if (media.mediaKind !== "image") return setMessage("Choose an image from the private project media library");
    const duration = Math.min(3_600, Math.max(1, Math.min(10, project.duration || 5)));
    const next = validateCutEdl({ version: 3, clips: edl.clips, graphics: [...(edl.graphics ?? []), {
      id: `graphic_${crypto.randomUUID().replaceAll("-", "").slice(0, 16)}`,
      kind: "image", assetId: media.assetId, text: "", timelineStart: Math.min(playhead, cutDuration(edl)), duration,
      x: .1, y: .1, width: .35, height: .35, imageFit: "contain",
    }], markers: edl.markers, compounds: edl.compounds }, Math.max(project.duration, cutDuration(edl)));
    applyEdit(next);
    setMessage(`${media.name} added as a private image graphic at the playhead`);
  };
  ...
  const takeMulticamAngle = async (group: CutMulticamGroup, angle: CutMulticamGroup["angles"][number]) => {
    if (!edl) return;
    applyEdit(switchCutMulticamAngle(edl, group.id, playhead, angle.id));
    const media = mediaLibrary.find((item) => item.assetId === (angle.assetId ?? project?.sourceAssetId));
    if (media) {
      await openSourceMedia(media);
      if (sourceMediaRef.current) sourceMediaRef.current.currentTime = Math.max(0, angle.sourceStart + playhead - group.timelineStart);
    }
    setMessage(`${angle.label} is live in the edit from ${formatTime(playhead)}.`);
  };
  ...
  if (!project || !edl) return (
    <main className="min-h-screen bg-black pb-24 text-white">
      ...
      <section className="mx-auto max-w-5xl px-4 py-8">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">{projects.map((item) => <div key={item.id} className="flex items-center gap-2 rounded-2xl border border-zinc-800 bg-zinc-950 p-2"><button type="button" aria-label={`Open ${item.name}`} onClick={() => void openProject(item.id)} className="flex min-w-0 flex-1 items-center gap-4 rounded-xl p-2 text-left hover:bg-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1d9bf0] focus-visible:ring-offset-2 focus-visible:ring-offset-black"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-zinc-900"><Film/></span><span className="min-w-0 flex-1"><span className="block truncate font-bold">{item.name}</span><span className="text-xs text-zinc-500">{formatTime(item.duration)} · {item.mediaKind}</span></span><ChevronRight className="text-zinc-500"/></button><Button variant="ghost" size="icon" className="shrink-0" aria-label={`Delete ${item.name}`} onClick={async () => { if (!window.confirm(`Delete ${item.name}? The source asset remains in your private library.`)) return; await apiRequest("DELETE", `/api/cut/projects/${item.id}`); await refreshProjects(); }}><Trash2 className="h-4 w-4 text-zinc-500"/></Button></div>)}</div>
      </section>
    </main>
  );

  const clip = edl.clips[Math.min(selectedClip, edl.clips.length - 1)];
  const updateSelectedClip = (changes: Partial<NonNullable<typeof clip>>) => {
    if (!clip) return;
    applyEdit({ ...edl, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, ...changes } : item) });
  };
  ...
  const timelineRulerTicks = Array.from({ length: 7 }, (_, index) => (timelineDuration * index) / 6);
  ...
  const latestCompletedRenderProvenance = cutRenderTimelineProvenance(latestCompletedRender?.output, revision, timelineAwaitingSave);
  const renderedTimelineJob = renderedTimelinePreview ? jobs.find((job) => job.id === renderedTimelinePreview.jobId) : undefined;
  const renderedTimelineProvenance = cutRenderTimelineProvenance(renderedTimelineJob?.output, revision, timelineAwaitingSave);
  ...
  return (
    <main className="cut-studio-shell min-h-screen pb-24 text-white">
      ...
      <div className="cut-studio-workspace mx-auto grid max-w-[1640px] gap-4 p-3 lg:grid-cols-[64px_320px_minmax(0,1fr)]">
        <div className="cut-studio-left-dock" aria-label="Project media and workspace tools">
        ...
        <section className="cut-studio-media-bin" aria-label="Project media bin">
          <div className="cut-studio-bin-header"><div><p>Project media</p><span>{mediaLibrary.length} assets</span></div><label title="Add project media"><input aria-label="Add project media from media bin" className="sr-only" type="file" accept="video/*,audio/*,image/*,.ttf,.otf,.json,.riv,.zip,.yaml,.lock,font/ttf,font/otf,application/json,application/x-rive,application/zip" disabled={busy === "media"} onChange={(event) => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ""; if (file) void uploadProjectMedia(file); }}/>{busy === "media" ? <Loader2 className="h-4 w-4 animate-spin"/> : <Plus className="h-4 w-4"/>}</label></div>
          <div className="cut-studio-bin-tabs" role="tablist" aria-label="Media bin views"><button type="button" aria-selected="true">All</button><button type="button">Video</button><button type="button">Audio</button><button type="button">Graphics</button></div>
          <div className="cut-studio-bin-assets">
            {mediaLibrary.map((media) => { const isOpen = sourceMedia?.id === media.id; const kind = media.mediaKind === "audio" ? "A" : media.mediaKind === "image" ? "I" : media.mediaKind === "font" ? "F" : media.mediaKind === "lottie" ? "L" : media.mediaKind === "rive" ? "R" : media.mediaKind.startsWith("code") ? "C" : "V"; const canOpen = !["font", "lottie", "rive", "code_source", "code_lockfile"].includes(media.mediaKind); return <button key={media.id} type="button" className={isOpen ? "is-selected" : ""} onClick={() => { if (canOpen) void openSourceMedia(media); }} disabled={!canOpen} aria-label={canOpen ? `Select ${media.name} in media bin` : `${media.name} project media`}><span className="cut-studio-bin-thumb">{kind}</span><span className="min-w-0 flex-1 text-left"><strong>{media.name}</strong><small>{media.mediaKind === "font" ? "Private font" : `${formatTime(media.duration)} · ${media.mediaKind}`}</small></span>{media.assetId === project.sourceAssetId && <i>Primary</i>}</button>; })}
          </div>
          <p className="cut-studio-bin-drop">Drag assets into the timeline or open one in Source.</p>
        </section>
        </div>
        <section ref={mainWorkspaceRef} data-active-tool={activeWorkspaceTool} data-qualification-mode={CUT_STUDIO_QUALIFICATION_MODE ? "true" : undefined} className="cut-studio-main-workspace min-w-0 space-y-4" aria-label="Editing workspace">
          ...
          <div className="cut-studio-monitor cut-studio-source overflow-hidden" aria-label="Source monitor">
            ...
            <div className="flex min-h-[220px] items-center justify-center bg-black">
              {sourceMedia && sourceMediaUrl ? sourceMedia.mediaKind === "video" ? <video ref={(node) => { sourceMediaRef.current = node; }} className="max-h-[44vh] w-full object-contain" src={sourceMediaUrl} controls/> : sourceMedia.mediaKind === "audio" ? <audio ref={(node) => { sourceMediaRef.current = node; }} className="w-[90%]" src={sourceMediaUrl} controls/> : <img alt={sourceMedia.name} className="max-h-[44vh] w-full object-contain" src={sourceMediaUrl}/> : <p className="text-xs text-zinc-600">Open an asset from Project media</p>}
            </div>
          ...
          <div className="cut-studio-monitor cut-studio-program relative flex min-h-[320px] items-center justify-center overflow-hidden" aria-label="Timeline monitor" data-rendered-timeline-provenance={timelineMonitorMode === "rendered" ? renderedTimelineProvenance : undefined}>
            ...
            {timelineMonitorMode === "rendered" && <span className={`absolute bottom-2 left-3 z-10 max-w-[calc(100%-1.5rem)] rounded px-2 py-1 text-[10px] ${renderedTimelineProvenance === "current" ? "bg-emerald-950/85 text-emerald-200" : "bg-amber-950/85 text-amber-100"}`}>Private completed render · exact compositing for this completed render · {cutRenderTimelineProvenanceLabel(renderedTimelineProvenance, renderedTimelineJob?.output?.timelineRevision)}</span>}
          ...
          <div className="cut-studio-timeline cut-studio-nle-timeline p-4">
            ...
            {(edl.multicamGroups?.length ?? 0) > 0 && <div className="mb-3 space-y-3 rounded-xl border border-[#1d9bf0]/30 bg-[#1d9bf0]/5 p-3" aria-label="Multicam angle editor">
             ...
             <div className="cut-studio-time-ruler" aria-label="Timeline time ruler">
               {timelineRulerTicks.map((tick) => <span key={tick} style={{ left: `${(tick / timelineDuration) * 100}%` }}>{formatTime(tick)}</span>)}
               <i aria-hidden="true" style={{ left: `${(playhead / timelineDuration) * 100}%` }}/>
             </div>
             <div className="cut-studio-track-canvas space-y-1 rounded-xl bg-zinc-900 p-2" onClick={(event) => { const box = event.currentTarget.getBoundingClientRect(); seek(((event.clientX - box.left) / box.width) * timelineDuration); }}>
            ...
            {edl.version === 3 && clip && clip.track !== "v1" && (clip.track ?? "").startsWith("v") && <div className="mt-4 rounded-xl border border-zinc-800 bg-black p-3"><p className="text-xs font-bold">Video compositing</p><label className="mt-3 block text-[10px] text-zinc-500">Private shape mask<select aria-label="Clip composition mask" value={clip.maskAssetId ?? ""} onChange={(event) => applyEdit({ version: 3, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, maskAssetId: event.target.value || undefined } : item) })} className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 p-2 text-xs text-white"><option value="">None</option>{mediaLibrary.filter((media) => media.mediaKind === "image").map((media) => <option key={media.id} value={media.assetId}>{media.name}</option>)}</select></label><p className="mt-2 text-[10px] leading-4 text-zinc-500">The private image controls this overlay's visible shape. Renders preserve the same mask.</p></div>}
          ...
          <div data-workspace-panel="transcript" className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-bold">Text-based edit</h2><p className="text-xs text-zinc-500">Search, correct, cut, restore, and export timed captions.</p></div><div className="flex gap-2">{project.transcript && <a className="inline-flex h-9 items-center rounded-lg border border-zinc-700 px-3 text-xs font-bold" href={`/api/cut/projects/${project.id}/captions.srt`}><FileText className="mr-1.5 h-3.5 w-3.5"/>SRT</a>}<Button size="sm" onClick={() => void transcribe()} disabled={busy === "transcribe" || transcriptJob?.state === "queued" || transcriptJob?.state === "running"}>{busy === "transcribe" || transcriptJob?.state === "queued" || transcriptJob?.state === "running" ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Captions className="mr-2 h-4 w-4"/>}{project.transcript ? "Re-transcribe" : "Transcribe"}</Button></div></div>
        ...
        <aside ref={inspectorRef} data-active-tool={activeWorkspaceTool} data-qualification-mode={CUT_STUDIO_QUALIFICATION_MODE ? "true" : undefined} className="cut-studio-inspector min-w-0 space-y-4" aria-label="Editor inspector">
          ...
          {clip && <section data-workspace-panel="edit" className="cut-studio-clip-inspector rounded-2xl border border-zinc-800 bg-zinc-950 p-4" aria-label="Selected clip inspector">
            <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#1d9bf0]">Inspector</p><h2 className="mt-1 text-sm font-bold">Clip settings</h2><p className="mt-0.5 truncate text-[10px] text-zinc-500" title={clip.label ?? "Selected timeline clip"}>Selected timeline clip</p></div><span className="rounded bg-zinc-900 px-2 py-1 font-mono text-[10px] text-zinc-400">{formatTime(clip.start)} – {formatTime(clip.end)}</span></div>
            <div className="mt-4 grid grid-cols-2 gap-3"><label className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Speed<select aria-label="Inspector speed" className="mt-1.5 w-full rounded-md border border-zinc-700 bg-black px-2 py-2 text-xs text-white" value={clip.speed ?? 1} onChange={(event) => updateSelectedClip({ speed: Number(event.target.value) })}>{[.25,.5,.75,1,1.25,1.5,2,4].map((value) => <option key={value} value={value}>{value}x</option>)}</select></label><label className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Transition<select aria-label="Inspector transition" className="mt-1.5 w-full rounded-md border border-zinc-700 bg-black px-2 py-2 text-xs text-white" value={clip.transition ?? "cut"} onChange={(event) => updateSelectedClip({ transition: event.target.value as "cut" | "fade_black" | "cross_dissolve" })}><option value="cut">Cut</option><option value="cross_dissolve">Cross dissolve</option><option value="fade_black">Fade black</option></select></label></div>
            <label className="mt-4 block text-[10px] font-bold uppercase tracking-wide text-zinc-500">Color<select aria-label="Inspector color" className="mt-1.5 w-full rounded-md border border-zinc-700 bg-black px-2 py-2 text-xs text-white" value={clip.colorPreset ?? "original"} onChange={(event) => updateSelectedClip({ colorPreset: event.target.value as "original" | "cinematic" | "vivid" | "monochrome" })}><option value="original">Original</option><option value="cinematic">Cinematic</option><option value="vivid">Vivid</option><option value="monochrome">Monochrome</option></select></label>
            <div className="mt-4 space-y-3">{[["Volume", "volume", 0, 2, .05], ["Fade in", "fadeIn", 0, 10, .1], ["Fade out", "fadeOut", 0, 10, .1]].map(([label, key, minimum, maximum, step]) => <label key={String(key)} className="block text-[10px] font-bold uppercase tracking-wide text-zinc-500"><span className="flex justify-between"><span>{label}</span><span className="font-mono text-zinc-300">{Number(clip[key as "volume" | "fadeIn" | "fadeOut"] ?? (key === "volume" ? 1 : 0)).toFixed(1)}</span></span><input aria-label={`Inspector ${String(label)}`} className="mt-2 w-full accent-[#1d9bf0]" type="range" min={Number(minimum)} max={Number(maximum)} step={Number(step)} value={clip[key as "volume" | "fadeIn" | "fadeOut"] ?? (key === "volume" ? 1 : 0)} onChange={(event) => updateSelectedClip({ [String(key)]: Number(event.target.value) })}/></label>)}</div>
            <p className="mt-4 border-t border-zinc-800 pt-3 text-[10px] leading-4 text-zinc-500">Open <strong className="text-zinc-300">Edit</strong> for transform animation, audio routing, multicam, compound, and detailed timeline controls.</p>
          </section>}
          ...
          {visibleSaveStatus && <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-3 text-sm text-zinc-300"><p role="status">Timeline state: {visibleSaveStatus}</p>{saveStatus === "Save failed" && <Button className="mt-2" size="sm" variant="outline" onClick={() => { failedSave.current = null; setSaveWake((value) => value + 1); }}>Retry saving edit</Button>}</div>}
