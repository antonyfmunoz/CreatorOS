...
import { CutStudioAudioEncoding } from "@/components/cut/CutStudioAudioEncoding";
import { CutStudioOutputFormat } from "@/components/cut/CutStudioOutputFormat";
...
type Job = { id: string; kind: "transcribe" | "highlights" | "render" | "proxy"; state: "queued" | "running" | "done" | "error" | "cancelled"; detail: string; progress: number; artifactAssetId?: string | null; output?: { candidates?: Highlight[]; filename?: string; mediaId?: string; originalAssetId?: string; maxWidth?: number } };
...
export default function CutStudioPage() {
  ...
  const [audioBitrateKbps, setAudioBitrateKbps] = useState<CutRenderRequest["audioBitrateKbps"]>();
  const [format, setFormat] = useState<NonNullable<CutRenderRequest["format"]>>("mp4");
  const [resolution, setResolution] = useState<"720p" | "1080p" | "2160p">("1080p");
  const [fps, setFps] = useState<CutRenderRequest["fps"]>(30);
  ...
  const render = async (clip?: { start: number; end: number }) => { await startJob("render", { aspect, captions, captionStyle, cleanAudio, audioPreset, masterGainDb, quality, format, audioBitrateKbps, resolution, fps, clip }); };
