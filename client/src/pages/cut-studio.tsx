...
type ProjectMedia = { id: string; assetId: string; name: string; duration: number; mediaKind: "video" | "audio" | "image" | "font" | "lottie" | "rive"; createdAt: string };
...
export default function CutStudioPage() {
  ...
  const uploadPrivateMedia = async (file: File, mediaKind: "video" | "audio" | "image" | "font" | "lottie" | "rive") => {
    const assetKind = mediaKind === "image" ? "photo" : mediaKind === "font" ? "cut-font" : mediaKind === "lottie" ? "cut-lottie" : mediaKind === "rive" ? "cut-rive" : mediaKind;
    const mimeType = mediaKind === "font" ? (/\.otf$/i.test(file.name) ? "font/otf" : "font/ttf") : mediaKind === "lottie" ? "application/json" : mediaKind === "rive" ? "application/octet-stream" : file.type;
  ...
  const uploadProjectMedia = async (file: File) => {
    if (!project) return;
    const mediaKind = file.type.startsWith("audio/") ? "audio" : file.type.startsWith("video/") ? "video" : file.type.startsWith("image/") ? "image" : /\.(ttf|otf)$/i.test(file.name) || file.type.startsWith("font/") ? "font" : /\.json$/i.test(file.name) ? "lottie" : /\.riv$/i.test(file.name) ? "rive" : null;
    if (!mediaKind) return setMessage("Choose a video, audio, image, TTF, OTF, Lottie JSON, or Rive .riv file");
    setBusy("media");
    try {
      if (mediaKind === "rive") validateCutStudioRiveBytes(await file.arrayBuffer());
      const duration = mediaKind === "lottie" ? validateCutStudioLottie(JSON.parse(await file.text()) as unknown).durationSeconds : mediaKind === "rive" || mediaKind === "image" || mediaKind === "font" ? project.duration : await mediaDuration(file);
      const asset = await uploadPrivateMedia(file, mediaKind);
      ...
      setMessage(mediaKind === "font" ? "Private font added to this project" : mediaKind === "lottie" ? "Validated Lottie animation added to this project" : mediaKind === "rive" ? "Validated Rive animation added to this project" : "Media added to this project");
    ...
    <main className="min-h-screen bg-black pb-24 text-white">
      ...
      <div className="mx-auto grid max-w-[1500px] gap-4 p-3 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="space-y-4">
          <CutStudioCreativeRuntime project={project} media={mediaLibrary} onTimelineApplied={(result) => { setEdl(result.edl); setRevision(result.revision); setProject((value) => value ? { ...value, edl: result.edl, duration: result.duration, revision: result.revision } : value); setHistory([]); setFuture([]); }}/>
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><div className="flex items-center justify-between gap-2"><div><h2 className="font-bold">Project media</h2><p className="mt-1 text-xs leading-5 text-zinc-500">Open private media in the source monitor; insert video/audio or select images, private fonts, Lottie JSON and Rive animations in compositions.</p></div><label className="inline-flex h-9 cursor-pointer items-center rounded-lg border border-zinc-700 px-3 text-xs font-bold hover:bg-zinc-900"><input aria-label="Add project media" className="sr-only" type="file" accept="video/*,audio/*,image/*,.ttf,.otf,.json,.riv,font/ttf,font/otf,application/json,application/x-rive" disabled={busy === "media"} onChange={(event) => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ""; if (file) void uploadProjectMedia(file); }}/>{busy === "media" ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin"/> : <Plus className="mr-1.5 h-3.5 w-3.5"/>}Media</label></div><div className="mt-3 space-y-2">{mediaLibrary.map((media) => { const proxy = jobs.find((job) => job.kind === "proxy" && job.output?.mediaId === media.id && job.state === "done"); return <div key={media.id} className={`flex flex-wrap items-center gap-2 rounded-xl p-2 ${sourceMedia?.id === media.id ? "border border-[#1d9bf0] bg-sky-950/30" : "bg-zinc-900"}`}><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-black text-[10px] font-bold text-zinc-500">{media.mediaKind === "audio" ? "A" : media.mediaKind === "image" ? "I" : media.mediaKind === "font" ? "F" : media.mediaKind === "lottie" ? "L" : media.mediaKind === "rive" ? "R" : "V"}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold">{media.name}</span><span className="text-[10px] text-zinc-600">{media.mediaKind === "font" ? "private font" : `${formatTime(media.duration)} · ${media.assetId === project.sourceAssetId ? "primary" : media.mediaKind}`}{proxy ? " · proxy ready" : ""}</span></span>{media.mediaKind !== "font" && media.mediaKind !== "lottie" && media.mediaKind !== "rive" && <Button aria-label={`Open ${media.name} in source monitor`} size="sm" variant="outline" disabled={busy === "source-media"} onClick={() => void openSourceMedia(media)}>Open</Button>}{media.mediaKind === "video" && <Button aria-label={`${proxy ? "Use" : "Create"} editing proxy for ${media.name}`} size="sm" variant="outline" disabled={busy === `proxy:${media.id}`} onClick={() => void createOrUseProxy(media)}>{busy === `proxy:${media.id}` ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : proxy ? "Use proxy" : "Proxy"}</Button>}{media.assetId !== project.sourceAssetId && ["video", "audio"].includes(media.mediaKind) && <Button aria-label={`Add all of ${media.name}`} size="sm" variant="outline" onClick={() => addMediaClip(media)}>Add all</Button>}</div>; })}</div>{project.mediaKind !== "video" && <p className="mt-3 rounded-lg bg-amber-950/40 px-3 py-2 text-[11px] leading-5 text-amber-300">Multitrack layers require a video project. Audio-only projects keep the fast single-track workflow.</p>}</div>
