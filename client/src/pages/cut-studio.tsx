...
import { ArrowLeft, Captions, Check, CheckCircle2, ChevronRight, Copy, Download, Film, FileText, Loader2, MessageSquare, Play, Plus, Redo2, RefreshCw, Save, Scissors, Search, Share2, Sparkles, Square, Trash2, Undo2, Upload, WandSparkles, X } from "lucide-react";
...
type ReviewVersion = { id: string; label: string; revision: number; reviewStatus: "pending" | "approved" | "changes_requested"; createdAt: string; links: ReviewLink[]; comments: ReviewComment[]; decisions: Array<{ id: string; reviewerName: string; decision: string; note?: string | null; createdAt: string }> };
...
function previewColorFilter(preset: "original" | "cinematic" | "vivid" | "monochrome" | undefined) {
  if (preset === "cinematic") return "contrast(1.08) saturate(.9) brightness(.98) sepia(.08)";
  if (preset === "vivid") return "contrast(1.08) saturate(1.25)";
  if (preset === "monochrome") return "grayscale(1)";
  return "none";
}
...
export default function CutStudioPage() {
  ...
  const openProject = useCallback(async (id: string) => {
    ...
    try {
      const [projectResponse, mediaResponse, reviewResponse] = await Promise.all([apiRequest("GET", `/api/cut/projects/${id}`), apiRequest("GET", `/api/cut/projects/${id}/media`), apiRequest("GET", `/api/cut/projects/${id}/reviews`)]);
  ...
  const applyEdit = useCallback((next: CutEdl) => {
    if (!edl) return;
    const complete = next.graphics === undefined && edl.graphics ? { ...next, graphics: edl.graphics } : next;
      ...
      <div className="mx-auto grid max-w-[1500px] gap-4 p-3 lg:grid-cols-[1fr_360px]">
        <section className="min-w-0 space-y-4">
          <div className="relative flex min-h-[280px] items-center justify-center overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950">
            {project.mediaKind === "video" ? <video ref={(node) => { mediaRef.current = node; }} className="max-h-[58vh] w-full bg-black object-contain" style={{ filter: previewColorFilter(clip?.colorPreset) }} src={mediaUrl} controls onTimeUpdate={onTime}/> : <audio ref={(node) => { mediaRef.current = node; }} className="w-[90%]" src={mediaUrl} controls onTimeUpdate={onTime}/>}
