      ...
      <div className="cut-studio-workspace mx-auto grid max-w-[1640px] gap-4 p-3 lg:grid-cols-[64px_320px_minmax(0,1fr)]">
        ...
        <aside ref={inspectorRef} data-active-tool={activeWorkspaceTool} className="cut-studio-inspector min-w-0 space-y-4" aria-label="Editor inspector">
          ...
          {clip && <section data-workspace-panel="media" className="cut-studio-clip-inspector rounded-2xl border border-zinc-800 bg-zinc-950 p-4" aria-label="Selected clip inspector">
            <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#1d9bf0]">Inspector</p><h2 className="mt-1 truncate text-sm font-bold">{clip.label ?? "Selected clip"}</h2></div><span className="rounded bg-zinc-900 px-2 py-1 font-mono text-[10px] text-zinc-400">{formatTime(clip.start)} – {formatTime(clip.end)}</span></div>
            <div className="mt-4 grid grid-cols-2 gap-3"><label className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Speed<select aria-label="Inspector clip speed" className="mt-1.5 w-full rounded-md border border-zinc-700 bg-black px-2 py-2 text-xs text-white" value={clip.speed ?? 1} onChange={(event) => updateSelectedClip({ speed: Number(event.target.value) })}>{[.25,.5,.75,1,1.25,1.5,2,4].map((value) => <option key={value} value={value}>{value}x</option>)}</select></label><label className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Transition<select aria-label="Inspector clip transition" className="mt-1.5 w-full rounded-md border border-zinc-700 bg-black px-2 py-2 text-xs text-white" value={clip.transition ?? "cut"} onChange={(event) => updateSelectedClip({ transition: event.target.value as "cut" | "fade_black" | "cross_dissolve" })}><option value="cut">Cut</option><option value="cross_dissolve">Cross dissolve</option><option value="fade_black">Fade black</option></select></label></div>
            <label className="mt-4 block text-[10px] font-bold uppercase tracking-wide text-zinc-500">Color<select aria-label="Inspector clip color" className="mt-1.5 w-full rounded-md border border-zinc-700 bg-black px-2 py-2 text-xs text-white" value={clip.colorPreset ?? "original"} onChange={(event) => updateSelectedClip({ colorPreset: event.target.value as "original" | "cinematic" | "vivid" | "monochrome" })}><option value="original">Original</option><option value="cinematic">Cinematic</option><option value="vivid">Vivid</option><option value="monochrome">Monochrome</option></select></label>
