      ...
      <div className="cut-studio-workspace mx-auto grid max-w-[1640px] gap-4 p-3 lg:grid-cols-[64px_320px_minmax(0,1fr)]">
        ...
        <aside ref={inspectorRef} data-active-tool={activeWorkspaceTool} className="cut-studio-inspector min-w-0 space-y-4" aria-label="Editor inspector">
          ...
          {clip && <section data-workspace-panel="media" className="cut-studio-clip-inspector rounded-2xl border border-zinc-800 bg-zinc-950 p-4" aria-label="Selected clip inspector">
            <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#1d9bf0]">Inspector</p><h2 className="mt-1 text-sm font-bold">Clip settings</h2><p className="mt-0.5 truncate text-[10px] text-zinc-500">{clip.label ?? "Selected timeline clip"}</p></div><span className="rounded bg-zinc-900 px-2 py-1 font-mono text-[10px] text-zinc-400">{formatTime(clip.start)} – {formatTime(clip.end)}</span></div>
