    ...
    <main className="min-h-screen bg-black pb-24 text-white">
      ...
      <div className="mx-auto grid max-w-[1500px] gap-4 p-3 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="space-y-4">
          <CutStudioCreativeRuntime project={project} media={mediaLibrary} onTimelineBusyChange={setCreativeTimelineBusy} onRenderBatchQueued={() => { void openProject(project.id); }} onTimelineApplied={(result) => { edlRef.current = result.edl; setEdl(result.edl); setRevision(result.revision); setProject((value) => value ? { ...value, edl: result.edl, duration: result.duration, revision: result.revision } : value); setHistory([]); setFuture([]); }}/>
