  ...
  if (!project || !edl) return (
    <main className="min-h-screen bg-black pb-24 text-white">
      ...
      <section className="mx-auto max-w-5xl px-4 py-8">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">{projects.map((item) => <div key={item.id} className="relative flex items-center gap-2 rounded-2xl border border-zinc-800 bg-zinc-950 p-2"><button type="button" aria-label={`${item.name} ${formatTime(item.duration)} · ${item.mediaKind}`} onClick={() => void openProject(item.id)} className="absolute inset-0 z-0 rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1d9bf0] focus-visible:ring-offset-2 focus-visible:ring-offset-black"/><div aria-hidden="true" className="pointer-events-none relative z-10 flex min-w-0 flex-1 items-center gap-4 rounded-xl p-2"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-zinc-900"><Film/></span><span className="min-w-0 flex-1"><span className="block truncate font-bold">{item.name}</span><span className="text-xs text-zinc-500">{formatTime(item.duration)} · {item.mediaKind}</span></span><ChevronRight className="text-zinc-500"/></div><Button variant="ghost" size="icon" className="relative z-20 shrink-0" aria-label={`Delete ${item.name}`} onClick={async (event) => { event.stopPropagation(); if (!window.confirm(`Delete ${item.name}? The source asset remains in your private library.`)) return; await apiRequest("DELETE", `/api/cut/projects/${item.id}`); await refreshProjects(); }}><Trash2 className="h-4 w-4 text-zinc-500"/></Button></div>)}</div>
      </section>
    </main>
  );
