...
export default function CutStudioPage() {
  ...
  const openProject = useCallback(async (id: string) => {
    ...
    try {
      ...
      setProject(next); setEdl(next.edl); setRevision(next.revision); setJobs(next.jobs ?? []); setMediaUrl(secure.url); setHistory([]); setFuture([]); setPlayhead(0); setSelectedClip(0); setHighlights([]);
    ...
    <main className="min-h-screen bg-black pb-24 text-white">
      ...
      <div className="mx-auto grid max-w-[1500px] gap-4 p-3 lg:grid-cols-[1fr_360px]">
        <section className="min-w-0 space-y-4">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><div className="flex items-center justify-between"><div><h2 className="font-bold">Text-based edit</h2><p className="text-xs text-zinc-500">Click a word to cut or restore it.</p></div><Button size="sm" onClick={() => void transcribe()} disabled={busy === "transcribe"}>{busy === "transcribe" ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Captions className="mr-2 h-4 w-4"/>}{project.transcript ? "Re-transcribe" : "Transcribe"}</Button></div>
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><h2 className="font-bold">Highlights</h2><p className="mt-1 text-xs leading-5 text-zinc-500">Score transcript moments and render platform-ready clips.</p><Button className="mt-3 w-full" variant="outline" disabled={!project.transcript} onClick={() => void requestHighlights()}><Sparkles className="mr-2 h-4 w-4"/>Find highlights</Button><div className="mt-3 space-y-2">{highlights.map((item) => <div key={item.id} className="rounded-xl bg-zinc-900 p-3"><div className="flex gap-2"><span className="rounded bg-[#1d9bf0] px-1.5 py-0.5 text-xs font-bold text-black">{item.score}</span><p className="line-clamp-2 flex-1 text-xs">{item.title}</p></div><div className="mt-2 flex items-center justify-between text-xs text-zinc-500"><span>{formatTime(item.start)}–{formatTime(item.end)}</span><button className="font-bold text-[#1d9bf0]" onClick={() => void render({ start: item.start, end: item.end })}>Render clip</button></div></div>)}</div></div>
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><h2 className="font-bold">Render</h2><label className="mt-3 block text-xs font-bold text-zinc-400">Aspect ratio<select value={aspect} onChange={(event) => setAspect(event.target.value)} className="mt-2 w-full rounded-xl border border-zinc-700 bg-black p-3 text-sm text-white"><option value="source">Source</option><option value="9:16">Vertical · 9:16</option><option value="1:1">Square · 1:1</option><option value="16:9">Landscape · 16:9</option></select></label><div className="mt-4 flex items-center justify-between text-sm"><span>Burn captions</span><Switch checked={captions} onCheckedChange={setCaptions}/></div>{captions && <label className="mt-3 block text-xs text-zinc-400">Caption style<select value={captionStyle} onChange={(event) => setCaptionStyle(Number(event.target.value))} className="mt-2 w-full rounded-xl border border-zinc-700 bg-black p-3 text-sm text-white"><option value={1}>Bold white</option><option value={2}>Creator yellow</option><option value={3}>Readable card</option></select></label>}<div className="mt-4 flex items-center justify-between text-sm"><span>Clean audio</span><Switch checked={cleanAudio} onCheckedChange={setCleanAudio}/></div><Button className="mt-4 w-full bg-[#1d9bf0] text-black hover:bg-[#1d9bf0]/90" onClick={() => void render()}><Film className="mr-2 h-4 w-4"/>Render full edit</Button></div>
