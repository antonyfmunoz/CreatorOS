...
import { cutDuration, estimateCutRenderSeconds, removeCutRange, restoreCutRange, splitCutAt, type CutEdl, type CutRenderRequest, type CutTranscript } from "@shared/cut-studio";
...
export default function CutStudioPage() {
  ...
  const openProject = useCallback(async (id: string) => {
    ...
    try {
      ...
      setProject(next); setEdl(next.edl); setRevision(next.revision); setJobs(projectJobs); setMediaUrl(secure.url); setHistory([]); setFuture([]); setPlayhead(0); setSelectedClip(0); setTranscriptDraft(next.transcript); setTranscriptSearch(""); setHighlights(projectJobs.find((job) => job.kind === "highlights" && job.state === "done")?.output?.candidates ?? []);
        ...
        <section className="min-w-0 space-y-4">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
            ...
            <div className="relative h-20 overflow-hidden rounded-xl bg-zinc-900" onClick={(event) => { const box = event.currentTarget.getBoundingClientRect(); seek(((event.clientX - box.left) / box.width) * project.duration); }}>
              {edl.clips.map((item, index) => <button key={item.id ?? `${item.start}-${item.end}`} className={`absolute top-2 h-14 rounded-md border text-xs font-bold ${selectedClip === index ? "border-white bg-[#1d9bf0] text-black" : "border-zinc-600 bg-zinc-700"}`} style={{ left: `${(item.start / project.duration) * 100}%`, width: `${Math.max(0.5, ((item.end - item.start) / project.duration) * 100)}%` }} onClick={(event) => { event.stopPropagation(); setSelectedClip(index); seek(item.start); }} aria-label={`Clip ${index + 1}`}>{index + 1}{(item.speed ?? 1) !== 1 ? ` · ${item.speed}x` : ""}</button>)}
              <span className="pointer-events-none absolute inset-y-0 w-0.5 bg-red-500" style={{ left: `${(playhead / project.duration) * 100}%` }}/>
            </div>
            {clip && <><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-zinc-400">In · {formatTime(clip.start)}<input className="mt-2 w-full accent-[#1d9bf0]" type="range" min={0} max={clip.end - .05} step="0.05" value={clip.start} onChange={(event) => applyEdit({ version: 2, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, start: Number(event.target.value) } : item) })}/></label><label className="text-xs text-zinc-400">Out · {formatTime(clip.end)}<input className="mt-2 w-full accent-[#1d9bf0]" type="range" min={clip.start + .05} max={project.duration} step="0.05" value={clip.end} onChange={(event) => applyEdit({ version: 2, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, end: Number(event.target.value) } : item) })}/></label></div><div className="mt-4 grid gap-3 sm:grid-cols-4"><label className="text-xs text-zinc-400">Speed<select aria-label="Clip speed" className="mt-1 w-full rounded-lg border border-zinc-700 bg-black p-2 text-white" value={clip.speed ?? 1} onChange={(event) => applyEdit({ version: 2, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, speed: Number(event.target.value) } : item) })}>{[.25,.5,.75,1,1.25,1.5,2,4].map((value) => <option key={value} value={value}>{value}x</option>)}</select></label>{[["Volume", "volume", 0, 2, .05], ["Fade in", "fadeIn", 0, 10, .1], ["Fade out", "fadeOut", 0, 10, .1]].map(([label, key, min, max, step]) => <label key={String(key)} className="text-xs text-zinc-400">{label} · {Number(clip[key as "volume" | "fadeIn" | "fadeOut"] ?? (key === "volume" ? 1 : 0)).toFixed(1)}<input aria-label={String(label)} className="mt-2 w-full accent-[#1d9bf0]" type="range" min={Number(min)} max={Number(max)} step={Number(step)} value={clip[key as "volume" | "fadeIn" | "fadeOut"] ?? (key === "volume" ? 1 : 0)} onChange={(event) => applyEdit({ version: 2, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, [String(key)]: Number(event.target.value) } : item) })}/></label>)}</div></>}
            <div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="destructive" disabled={edl.clips.length === 1} onClick={() => { applyEdit({ version: 2, clips: edl.clips.filter((_, index) => index !== selectedClip) }); setSelectedClip(0); }}><Trash2 className="mr-2 h-4 w-4"/>Delete clip</Button><Button size="sm" variant="outline" disabled={selectedClip === 0} onClick={() => { const clips = [...edl.clips]; [clips[selectedClip - 1], clips[selectedClip]] = [clips[selectedClip], clips[selectedClip - 1]]; applyEdit({ version: 2, clips }); setSelectedClip(selectedClip - 1); }}>Move earlier</Button><Button size="sm" variant="outline" disabled={selectedClip >= edl.clips.length - 1} onClick={() => { const clips = [...edl.clips]; [clips[selectedClip + 1], clips[selectedClip]] = [clips[selectedClip], clips[selectedClip + 1]]; applyEdit({ version: 2, clips }); setSelectedClip(selectedClip + 1); }}>Move later</Button><Button size="sm" variant="outline" onClick={() => seek(clip?.start ?? 0)}><Play className="mr-2 h-4 w-4"/>Preview</Button></div>
          </div>
