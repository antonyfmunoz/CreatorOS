...
export default function CutStudioPage() {
  ...
  const addMotionKeyframe = () => {
    ...
    const next = [...(selected.motionKeyframes ?? []).filter((item) => Math.abs(item.at - at) > .0005), { at, x: transform.x, y: transform.y }].sort((left, right) => left.at - right.at);
