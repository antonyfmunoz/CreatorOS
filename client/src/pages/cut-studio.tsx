...
import { ArrowDown, ArrowLeft, ArrowUp, Captions, Check, CheckCircle2, ChevronRight, Copy, Download, Eye, EyeOff, Film, FileText, Flag, Link2, Loader2, Lock, Magnet, MessageSquare, Play, Plus, Redo2, RefreshCw, Save, Scissors, Search, Share2, Sparkles, Square, Trash2, Undo2, Unlink2, Unlock, Upload, Volume2, VolumeX, WandSparkles, X } from "lucide-react";
...
export default function CutStudioPage() {
  ...
  const applyEdit = useCallback((next: CutEdl) => {
    ...
    const complete = { ...next, graphics: next.graphics ?? edl.graphics, markers: next.markers ?? edl.markers, compounds, tracks: next.tracks ?? edl.tracks };
  ...
  const setSelectedClipPosition = (axis: "x" | "y", value: number) => {
    if (!edl) return;
    applyEdit({ ...edl, clips: edl.clips.map((item, index) => {
      ...
      return { ...item, transform: { ...transform, [axis]: value } };
    }) });
  };
  ...
  const addMotionKeyframe = () => {
    ...
    const next = [...(selected.motionKeyframes ?? []).filter((item) => Math.abs(item.at - at) > .0005), { at, x: transform.x, y: transform.y, easing: "ease_in_out" as const }].sort((left, right) => left.at - right.at);
  ...
  const applyMotionPreset = (preset: "slide_right" | "slide_left" | "rise") => {
    if (!edl) return;
    const selected = edl.clips[selectedClip];
    if (!selected) return;
    const transform = selected.transform ?? { x: 0, y: 0, width: 1, height: 1, opacity: 1 };
    ...
    const start = preset === "slide_right" ? { x: 0, y: transform.y } : preset === "slide_left" ? { x: maximumX, y: transform.y } : { x: transform.x, y: maximumY };
    const finish = preset === "slide_right" ? { x: maximumX, y: transform.y } : preset === "slide_left" ? { x: 0, y: transform.y } : { x: transform.x, y: 0 };
    applyEdit({ ...edl, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, transform: { ...transform, ...start }, motionKeyframes: [{ at: duration, ...finish, easing: "ease_in_out" as const }] } : item) });
    setMessage(`${preset === "rise" ? "Rise" : preset === "slide_left" ? "Slide left" : "Slide right"} motion preset applied`);
  };
      ...
      <div className="mx-auto grid max-w-[1500px] gap-4 p-3 lg:grid-cols-[1fr_360px]">
        <section className="min-w-0 space-y-4">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
            ...
            {edl.version === 3 && clip && clip.track !== "v1" && (clip.track ?? "").startsWith("v") && <div className="mt-4 rounded-xl border border-zinc-800 bg-black p-3" aria-label="Clip motion keyframes"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-bold">Position animation</p><p className="mt-1 text-[10px] text-zinc-500">Set the overlay position, move the playhead, and capture a render-effective keyframe.</p></div><div className="flex gap-2"><select aria-label="Motion preset" defaultValue="" className="h-8 rounded-lg border border-zinc-700 bg-zinc-950 px-2 text-[10px] font-bold" onChange={(event) => { if (event.target.value) applyMotionPreset(event.target.value as "slide_right" | "slide_left" | "rise"); event.target.value = ""; }}><option value="" disabled>Motion preset</option><option value="slide_right">Slide right</option><option value="slide_left">Slide left</option><option value="rise">Rise</option></select><Button size="sm" variant="outline" disabled={(clip.motionKeyframes?.length ?? 0) >= 50} onClick={addMotionKeyframe}><Plus className="mr-1 h-3 w-3"/>Add keyframe</Button></div></div><div className="mt-3 grid gap-3 sm:grid-cols-2">{(["x", "y"] as const).map((axis) => { const transform = clip.transform ?? { x: 0, y: 0, width: 1, height: 1, opacity: 1 }; const maximum = Math.max(0, 1 - (axis === "x" ? transform.width : transform.height)); return <label key={axis} className="text-[11px] text-zinc-400">Position {axis.toUpperCase()} · {Math.round(transform[axis] * 100)}%<input aria-label={`Clip position ${axis.toUpperCase()}`} className="mt-2 w-full accent-[#1d9bf0]" type="range" min={0} max={maximum} step={.01} value={Math.min(transform[axis], maximum)} onChange={(event) => setSelectedClipPosition(axis, Number(event.target.value))}/></label>; })}</div>{(clip.motionKeyframes?.length ?? 0) > 0 && <div className="mt-3 flex flex-wrap gap-2">{clip.motionKeyframes!.map((keyframe) => <div key={keyframe.at} className="inline-flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-950 px-2 py-1 text-[10px] text-zinc-300"><button className="font-bold text-[#1d9bf0]" onClick={() => seek((clip.timelineStart ?? 0) + keyframe.at)}>{formatTime(keyframe.at)}</button><span>X {Math.round(keyframe.x * 100)} · Y {Math.round(keyframe.y * 100)}</span><select aria-label={`Motion easing at ${formatTime(keyframe.at)}`} value={keyframe.easing ?? "linear"} className="rounded bg-black px-1 py-0.5" onChange={(event) => applyEdit({ ...edl, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, motionKeyframes: item.motionKeyframes?.map((frame) => frame.at === keyframe.at ? { ...frame, easing: event.target.value as "linear" | "ease_in_out" } : frame) } : item) })}><option value="linear">Linear</option><option value="ease_in_out">Ease</option></select><button aria-label={`Delete motion keyframe at ${formatTime(keyframe.at)}`} onClick={() => applyEdit({ ...edl, clips: edl.clips.map((item, index) => index === selectedClip ? { ...item, motionKeyframes: item.motionKeyframes?.filter((frame) => frame.at !== keyframe.at) } : item) })}><X className="h-3 w-3 text-zinc-600"/></button></div>)}</div>}</div>}
