...
type ProjectMedia = { id: string; assetId: string; name: string; duration: number; mediaKind: "video" | "audio" | "image" | "font" | "lottie"; createdAt: string };
...
export default function CutStudioPage() {
  ...
  const uploadPrivateMedia = async (file: File, mediaKind: "video" | "audio" | "image" | "font" | "lottie") => {
    const assetKind = mediaKind === "image" ? "photo" : mediaKind === "font" ? "cut-font" : mediaKind === "lottie" ? "cut-lottie" : mediaKind;
    const mimeType = mediaKind === "font" ? (/\.otf$/i.test(file.name) ? "font/otf" : "font/ttf") : mediaKind === "lottie" ? "application/json" : file.type;
  ...
  const uploadProjectMedia = async (file: File) => {
    if (!project) return;
    const mediaKind = file.type.startsWith("audio/") ? "audio" : file.type.startsWith("video/") ? "video" : file.type.startsWith("image/") ? "image" : /\.(ttf|otf)$/i.test(file.name) || file.type.startsWith("font/") ? "font" : /\.json$/i.test(file.name) ? "lottie" : null;
    if (!mediaKind) return setMessage("Choose a video, audio, image, TTF, OTF, or Lottie JSON file");
    setBusy("media");
    try {
      const duration = mediaKind === "lottie" ? validateCutStudioLottie(JSON.parse(await file.text()) as unknown).durationSeconds : mediaKind === "image" || mediaKind === "font" ? project.duration : await mediaDuration(file);
      const asset = await uploadPrivateMedia(file, mediaKind);
      ...
      setMessage(mediaKind === "font" ? "Private font added to this project" : mediaKind === "lottie" ? "Validated Lottie animation added to this project" : "Media added to this project");
