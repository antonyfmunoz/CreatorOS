...
export default function CheckoutSuccessPage() {
  ...
  return <main className="flex min-h-[calc(100dvh-3.5rem)] items-center justify-center bg-black p-6 text-center text-white"><section className="max-w-sm"><span className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${paid ? "bg-emerald-500 text-black" : "bg-zinc-900 text-zinc-300"}`}>{paid ? <CheckCircle2 className="h-8 w-8" /> : <Clock3 className="h-8 w-8" />}</span><h1 className="mt-6 text-2xl font-bold">{paid ? "Payment complete" : "Confirming your payment"}</h1><p className="mt-3 text-sm leading-6 text-zinc-400">{paid ? "Your purchase and access are ready." : checkout.isError ? "We could not confirm this payment yet. Your order is preserved while we retry." : "Stripe is securely confirming your payment. This normally takes a moment."}</p><div className="mt-7 flex flex-col gap-3"><Button className="h-11 rounded-xl bg-white font-bold text-black hover:bg-zinc-200" onClick={() => setLocation("/orders?view=purchases")}><ShoppingBag className="mr-2 h-4 w-4" /> View my purchase</Button>{!paid && <Button variant="secondary" className="rounded-xl border border-zinc-700 bg-zinc-900 text-white hover:bg-zinc-800" onClick={() => setLocation("/marketplace")}>Continue browsing</Button>}</div></section></main>;
}
