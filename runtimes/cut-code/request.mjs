...
export function validateRequest(request) {
  ...
  if (!(request.mode === 'video' ? ['mp4', 'webm'] : request.mode === 'audio' ? ['wav', 'mp3', 'm4a'] : ['png', 'jpeg', 'webp']).includes(format)) throw new Error('Unsupported output format for this mode.');
...
export function outputContract(request) {
  ...
  const mediaType = request.mode === 'sequence' ? 'application/zip' : { png: 'image/png', jpeg: 'image/jpeg', webp: 'image/webp', mp4: 'video/mp4', webm: 'video/webm', wav: 'audio/wav', mp3: 'audio/mpeg', m4a: 'audio/mp4' }[request.format];
