...
export function validateFrameAudio(sample) {
  if (sample?.sourceLoopSeconds !== undefined) {
    ...
    if (sample.reverse || sample.sourceTimebase !== 'container' || sample.sourceSeconds >= sample.sourceLoopSeconds || sample.sourceEndSeconds === undefined || sample.sourceEndSeconds > sample.sourceLoopSeconds) throw new Error('Invalid private video loop binding.');
  }
  if (!sample || typeof sample !== 'object' || typeof sample.id !== 'string' || !/^[A-Za-z0-9:_-]{1,100}$/.test(sample.id)
    ...
    || (sample.reverse !== undefined && typeof sample.reverse !== 'boolean')
...
export class FrameAudioCollector {
  ...
  capture(frame, descriptors) {
    ...
    for (const descriptor of descriptors) {
      ...
      const expectedTime = track ? track.sourceStartSeconds + (track.reverse ? -1 : 1) * (frame - track.startFrame) * track.speed / request.fps : 0;
      ...
      if (!track || track.file !== sample.file || track.audioStream !== sample.audioStream || track.speed !== sample.speed || track.reverse !== sample.reverse || track.sourceEndSeconds !== sample.sourceEndSeconds || track.sourceTimebase !== sample.sourceTimebase || track.sourceLoopSeconds !== sample.sourceLoopSeconds
