...
export function validateFrameAudio(sample) {
  ...
  return { id: sample.id, file: sample.file, sourceSeconds: sample.sourceSeconds, speed: sample.speed, volume: sample.volume, audioStream: sample.audioStream };
}
...
export class FrameAudioCollector {
  ...
  capture(frame, descriptors) {
    ...
    for (const descriptor of descriptors) {
      ...
      if (!track || track.file !== sample.file || track.audioStream !== sample.audioStream || track.speed !== sample.speed
        ...
        track = { file: sample.file, startFrame: frame, endFrame: frame + 1, sourceStartSeconds: sample.sourceSeconds, speed: sample.speed, volume: 1, audioStream: sample.audioStream, volumeSamples: [] };
