...
const decode = (track, extra = {}) => {
  ...
  return execFileSync('ffmpeg', ['-v', 'error', '-nostdin', '-threads', '1', '-f', 'lavfi', '-i', 'sine=frequency=440:sample_rate=48000:duration=4', '-af', audioTrackFilters(plan, request.fps), '-ac', '1', '-ar', '48000', '-f', 'f32le', 'pipe:1'], { windowsHide: true, timeout: 20_000, maxBuffer: 2_000_000 });
};
...
console.log(JSON.stringify({ passed: true, tests: ['offset', 'fade', 'static-gain', 'final-hold', 'range-continuity', 'speed-independent-clock', 'exact-held-edge', 'maximum-keyframes'], baselineRms: baseline, fadedRms: rms(faded, .72, .92) }));
