...
import { db } from "./db";
import { attributeOrderConversion, emitAnalyticsEvent } from "./analytics";
...
import {
  ...
  cents,
  platformFeeBps,
} from "../shared/creator-payouts";
import { checkoutBillingTerms } from "../shared/product-catalog";
import {
  subscriptionAccessEndsAt,
  subscriptionCanCancel,
  subscriptionGrantsAccess,
  subscriptionTransitionAllowed,
} from "../shared/subscription-policy";
import {
  allocationReversalAmount,
  disputedOrderState,
  refundedOrderState,
} from "../shared/commerce-financial-policy";
import { discountedCheckoutLineAmounts } from "../shared/marketplace-maturity";
import {
  recordAffiliateOrderConversion,
  reverseAffiliateOrderCommission,
...
async function userCanRefundOrder(
  orderId: string,
  userId: number,
  ...
  const items = await db
    .select({ sellerUserId: products.userId })
    .from(orderItems)
    .innerJoin(products, eq(orderItems.productId, products.id))
    .where(eq(orderItems.orderId, orderId));
  return (
    items.length > 0 && items.every((item) => item.sellerUserId === userId)
  );
}

async function refundPaymentReference(order: typeof orders.$inferSelect) {
  if (order.providerPaymentReference?.startsWith("pi_"))
    return order.providerPaymentReference;
  ...
  if (!order.providerSubscriptionReference)
    return order.providerPaymentReference;
  const subscription = await getStripe().subscriptions.retrieve(
    order.providerSubscriptionReference,
    {
...
async function synchronizeCreatorPaymentAccount(
  account: typeof creatorPaymentAccounts.$inferSelect,
) {
  const stripeAccount = await getStripe().accounts.retrieve(
    account.stripeAccountId,
  );
  const [updated] = await db
    .update(creatorPaymentAccounts)
    .set(paymentAccountFields(stripeAccount))
    .where(eq(creatorPaymentAccounts.id, account.id))
    .returning();
  return updated ?? account;
}

async function creatorCheckoutRoute(
  orderId: string,
): Promise<CreatorCheckoutRoute | null> {
  const items = await db
    .select({ item: orderItems, product: products })
    .from(orderItems)
    .innerJoin(products, eq(orderItems.productId, products.id))
    .where(eq(orderItems.orderId, orderId));
  const creatorOffers = items.filter(
  ...
  if (
    creatorOffers.length !== items.length ||
    creatorOffers.some(({ product }) => product.userId !== sellerUserId)
  ) {
  ...
  const [savedAccount] = await db
    ...
    .where(eq(creatorPaymentAccounts.userId, sellerUserId))
    .limit(1);
  if (!savedAccount)
    throw new Error("This creator has not connected a payout account");
  if (savedAccount.accountType !== "standard") {
    throw new Error(
      "This creator must reconnect their own Stripe account before accepting payouts",
    );
  }
  const account = await synchronizeCreatorPaymentAccount(savedAccount);
  if (!account.chargesEnabled || !account.payoutsEnabled) {
    throw new Error(
      "This creator's payout account is not ready to accept payments",
    );
  }
...
async function fulfillCheckoutSession(session: Stripe.Checkout.Session) {
  ...
  const [order] = await db
    ...
    .limit(1);
  if (!order)
    throw new Error("Stripe Checkout session references an unknown order");
  if (
    order.paymentProvider !== PAYMENT_PROVIDER ||
    order.providerReference !== session.id
  ) {
    throw new Error(
      "Stripe Checkout session does not match the prepared order",
    );
  }
  if (
    session.currency !== order.currency ||
    session.amount_total !== cents(order.totalAmount)
  ) {
    throw new Error(
      "Stripe Checkout payment amount does not match the prepared order",
    );
  }
  ...
  return settleOrder({
    ...
    creatorPaymentIntentReference:
      paymentIntentReference ?? subscriptionReference ?? undefined,
  });
}
...
async function synchronizeSubscriptionAccess(
  ...
  const [order] = await db
    ...
    .limit(1);
  if (!order)
    throw new Error("Stripe subscription references an unknown order");
  if (
    order.providerSubscriptionReference &&
    order.providerSubscriptionReference !== subscription.id
  ) {
  ...
  const cancelAt = subscriptionAccessEndsAt({
    ...
    itemPeriodEnds: subscription.items.data.map(
  ...
  await db.transaction(async (tx) => {
    ...
    if (!active) {
      const linkedCommunities = await tx
        .selectDistinct({ communityId: products.communityId })
        .from(orderItems)
        .innerJoin(products, eq(orderItems.productId, products.id))
        .where(
          and(
  ...
  const payments =
    (
      invoice as Stripe.Invoice & {
        payments?: {
          data?: Array<{
            payment?: { payment_intent?: string | Stripe.PaymentIntent | null };
          }>;
        };
      }
    ).payments?.data ?? [];
...
async function recordPaidSubscriptionInvoice(invoice: Stripe.Invoice) {
  ...
  await db.transaction(async (tx) => {
    if (subscriptionGrantsAccess(order.subscriptionStatus ?? "")) {
      await tx
        .update(entitlements)
        .set({ status: "active", endsAt: null, updatedAt: new Date() })
        .where(eq(entitlements.sourceOrderId, order.id));
    }
    ...
    await tx
      .insert(creatorEarningsAllocations)
      .values({
        orderId: order.id,
  ...
  await recordAffiliateOrderConversion({
    orderId: order.id,
    sourceReference: `subscription:${invoice.id}`,
    cycleIndex: 1,
    grossRevenueCents: invoice.amount_paid,
  }).catch((error) =>
    console.error("Unable to record recurring affiliate commission", error),
  );
}
...
async function beginCommerceProviderEvent(
  event: Stripe.Event,
  payloadSha256: string,
) {
  const providerObjectReference =
  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        eq(commerceProviderEvents.providerEventId, event.id),
      ),
    )
    .limit(1);
  if (!existing)
    throw new Error("Commerce provider event idempotency state is unavailable");
  if (existing.status !== "failed")
    return { eventRow: existing, shouldProcess: false };
  const [retrying] = await db
    ...
    .where(
      and(
        eq(commerceProviderEvents.id, existing.id),
...
async function finishCommerceProviderEvent(
  eventRowId: string,
  outcome: CommerceEventOutcome,
) {
  await db
    .update(commerceProviderEvents)
    .set({
    ...
    .set({
      ...
      errorMessage:
        error instanceof Error
          ...
          : "Commerce provider event failed",
      updatedAt: new Date(),
    })
    .where(eq(commerceProviderEvents.id, eventRowId));
}

async function orderForPaymentReference(
  ...
  if (fallbackOrderId) {
    const [order] = await db
      .select()
      .from(orders)
      .where(eq(orders.id, fallbackOrderId))
      .limit(1);
  ...
  const [allocation] = await db
    ...
    .where(
      eq(creatorEarningsAllocations.paymentIntentReference, paymentReference),
    )
    .limit(1);
  ...
  if (paymentReference.startsWith("pi_")) {
    ...
    for (const session of sessions.data) {
      const [legacyOrder] = await db
        .select()
        .from(orders)
        .where(eq(orders.providerReference, session.id))
        .limit(1);
      if (legacyOrder) {
        await db
          .update(orders)
...
async function setOrderFinancialState(input: {
  ...
  const updatedOrder = {
    ...input.order,
    financialStatus: input.financialStatus,
    refundedAmount: input.refundedAmount ?? input.order.refundedAmount,
    disputedAmount: input.disputedAmount ?? input.order.disputedAmount,
  };
  if (
    ["paid", "partially_refunded", "dispute_won"].includes(
      input.financialStatus,
    )
  ) {
    await attributeOrderConversion(updatedOrder);
    await emitAnalyticsEvent({
      userId: input.order.buyerId,
      eventName: "purchase.completed",
      sessionId: `order-${input.order.id}`,
      deduplicationKey: `purchase-completed:${input.order.id}:${input.financialStatus}:${updatedOrder.refundedAmount}`,
      objectType: "order",
      objectId: input.order.id,
      properties: {
  ...
  if (["partially_refunded", "refunded"].includes(input.financialStatus)) {
    await emitAnalyticsEvent({
      userId: input.order.buyerId,
      eventName: "refund.completed",
      sessionId: `order-${input.order.id}`,
      deduplicationKey: `refund-completed:${input.order.id}:${updatedOrder.refundedAmount}`,
      objectType: "order",
      objectId: input.order.id,
      properties: { refundedAmount: updatedOrder.refundedAmount },
    });
  }
}
...
async function handleChargeRefunded(
  ...
  const order = await orderForPaymentReference(
    paymentReference,
    charge.metadata?.orderId ?? charge.transfer_group,
  );
  if (!order)
    return {
      status: "ignored",
      providerObjectReference: charge.id,
      amount: charge.amount_refunded / 100,
      currency: charge.currency,
    };
  const refundState = refundedOrderState({
    chargeAmountCents: charge.amount,
    refundedAmountCents: charge.amount_refunded,
    providerMarkedRefunded: charge.refunded,
  });
  ...
  for (const allocation of allocations) {
    await db
      ...
      .set({
        ...
        refundedAmount,
        reversedAmount: allocationReversalAmount({
          creatorNetAmount: allocation.creatorNetAmount,
          affectedAmountCents: charge.amount_refunded,
          orderAmountCents: charge.amount,
        }),
        updatedAt: new Date(),
      })
      .where(eq(creatorEarningsAllocations.id, allocation.id));
  }
  await reverseAffiliateOrderCommission({
    orderId: order.id,
    affectedAmountCents: charge.amount_refunded,
    sourceReference: `refund:${charge.id}:${charge.amount_refunded}`,
    reason: "refund",
  });
  return {
    orderId: order.id,
...
async function reconcileDisputeTransfer(input: {
  ...
  const chargeTransferReference =
    typeof input.charge.transfer === "string"
      ? input.charge.transfer
      : input.charge.transfer?.id;
  const transfer = chargeTransferReference
    ...
    : (
        ...
        const sourceTransaction =
          typeof candidate.source_transaction === "string"
  ...
  if (!transfer) {
    throw new Error(
      `The destination transfer for disputed charge ${input.charge.id} could not be resolved`,
    );
  }
  const transferReference = transfer.id;
  const reversals = await getStripe().transfers.listReversals(
    transferReference,
    { limit: 100 },
  );
  const disputeReversedCents = reversals.data
    .filter(
      (reversal) =>
        reversal.metadata?.creativesOsDisputeId === input.dispute.id,
    )
  ...
  if (input.won) {
    ...
    const alreadyRestored = priorRestorations.data.some(
      (candidate) =>
  ...
  const desiredReversalCents = cents(
    allocationReversalAmount({
  ...
  const remainingReversalCents = Math.max(
    0,
    desiredReversalCents - disputeReversedCents,
  );
  const reversibleCents = Math.min(
    remainingReversalCents,
    Math.max(0, transfer.amount - transfer.amount_reversed),
  );
...
async function handleChargeDispute(
  ...
  if (!order)
    return {
      status: "ignored",
      providerObjectReference: dispute.id,
      amount: dispute.amount / 100,
      currency: dispute.currency,
    };
  const disputeState = disputedOrderState({
    providerStatus: dispute.status,
    disputedAmountCents: dispute.amount,
    fullyRefunded: order.refundedAmount >= order.totalAmount,
  });
  await setOrderFinancialState({
    order,
    financialStatus: disputeState.financialStatus,
    disputedAmount: disputeState.disputedAmount,
    accessActive: disputeState.accessActive,
  });
  const allocations = await db
    .select()
    .from(creatorEarningsAllocations)
  ...
  for (const allocation of allocations) {
    try {
      await reconcileDisputeTransfer({
        charge,
        dispute,
        order,
        allocation,
      ...
      .set({
        ...
        reversedAmount: disputeState.won
          ...
          : Math.max(
              ...
              allocationReversalAmount({
                creatorNetAmount: allocation.creatorNetAmount,
                affectedAmountCents: dispute.amount,
                orderAmountCents: cents(order.totalAmount),
              }),
            ),
        updatedAt: new Date(),
  ...
  if (!disputeState.won)
    await reverseAffiliateOrderCommission({
      orderId: order.id,
      affectedAmountCents: dispute.amount,
      sourceReference: `dispute:${dispute.id}:${dispute.status}`,
      reason: "dispute",
    });
  return {
    orderId: order.id,
...
async function retryFailedCommerceProviderEvents() {
  ...
  for (const eventRow of failedEvents) {
    ...
    try {
      const dispute = await getStripe().disputes.retrieve(
        claimed.providerObjectReference,
      );
...
export function scheduleStripeCommerceRecovery() {
  const run = () =>
    retryFailedCommerceProviderEvents().catch((error) => {
...
async function handleConnectedAccountUpdated(
  ...
  await db
    .update(creatorPaymentAccounts)
    .set(paymentAccountFields(account))
    .where(eq(creatorPaymentAccounts.stripeAccountId, account.id));
  return {
    providerObjectReference: account.id,
...
async function handlePayoutEvent(
  ...
  if (!connectedAccountId)
    return {
      status: "ignored",
      providerObjectReference: payout.id,
  ...
  const [account] = await db
    .select()
    .from(creatorPaymentAccounts)
    .where(eq(creatorPaymentAccounts.stripeAccountId, connectedAccountId))
    .limit(1);
  if (!account)
    return {
      status: "ignored",
...
async function processCommerceProviderEvent(
  event: Stripe.Event,
): Promise<CommerceEventOutcome> {
  if (
    ...
    if (session.payment_status !== "paid")
      return { status: "ignored", providerObjectReference: session.id };
    await fulfillCheckoutSession(session);
    return {
      orderId: session.metadata?.orderId ?? session.client_reference_id,
      providerObjectReference: session.id,
  ...
  if (
    ...
    return {
      ...
      providerObjectReference: subscription.id,
    };
  }
  if (event.type === "invoice.paid") {
    ...
    const subscriptionReference = invoiceSubscriptionReference(invoice);
    const [order] = subscriptionReference
      ? await db
          .select()
          .from(orders)
          .where(
            eq(orders.providerSubscriptionReference, subscriptionReference),
          )
          .limit(1)
      : [];
    return {
      orderId: order?.id,
      providerObjectReference: invoice.id,
  ...
  if (event.type === "account.updated")
    return handleConnectedAccountUpdated(event.data.object as Stripe.Account);
  if (
    [
      "payout.created",
      "payout.updated",
...
export function registerStripeWebhook(app: Express) {
  app.post("/api/stripe/webhook", async (req: Request, res: Response) => {
    ...
    try {
      ...
      await failCommerceProviderEvent(started.eventRow.id, error).catch(
        () => undefined,
      );
      ...
      return res
        .status(500)
        .json({ message: "Unable to fulfill Stripe payment" });
    }
  });
}
...
export function registerStripeRoutes(app: Express) {
  ...
  app.post(
    "/api/creator-payments/account/refresh",
    attachUser,
    async (req, res) => {
      if (!isStripeConfigured())
        return res
          .status(503)
          .json({ message: "Stripe Connect is not configured yet" });
      try {
        const [account] = await db
          .select()
          .from(creatorPaymentAccounts)
        ...
        if (!account)
          return res
            .status(404)
            .json({ message: "No payout account is connected" });
        ...
        res.json({
          ...
          disabledReason: refreshed.disabledReason,
          requirementsCurrentlyDue: refreshed.requirementsCurrentlyDue,
          requirementsPastDue: refreshed.requirementsPastDue,
        ...
        res
          .status(502)
          .json({ message: "Unable to refresh payout account status" });
      }
    },
  );
  ...
  app.post("/api/creator-payments/onboarding", attachUser, async (req, res) => {
    ...
    try {
      ...
      if (!paymentAccount) {
        ...
        const stripeAccount = await getStripe().accounts.create({
          type: "standard",
        });
        const [created] = await db
      ...
      if (!paymentAccount)
        throw new Error("Unable to create a creator payout account");
      ...
      res
        .status(502)
        .json({ message: "Unable to start Stripe account connection" });
    }
  });
  ...
  app.get("/api/creator-payments/earnings", attachUser, async (req, res) => {
    ...
    const totals = allocations.reduce(
      (result, allocation) => {
        if (
          ["payment_required", "canceled", "refunded", "dispute_lost"].includes(
            allocation.status,
          )
        )
          return result;
        const retainedRatio =
          allocation.grossAmount > 0
            ? Math.max(
                0,
                1 - allocation.refundedAmount / allocation.grossAmount,
              )
            : 0;
        const creatorNet = Math.max(
          0,
          allocation.creatorNetAmount - allocation.reversedAmount,
        );
        result.grossAmount += Math.max(
          0,
          allocation.grossAmount - allocation.refundedAmount,
        );
        result.platformFeeAmount += Math.max(
          0,
          allocation.platformFeeAmount * retainedRatio,
        );
  ...
  app.post("/api/orders/:id/checkout", attachUser, async (req, res) => {
    if (process.env.CREATOROS_DEMO_MODE === "true") {
      return res
        .status(501)
        .json({ message: "Stripe Checkout is unavailable in demo mode" });
    }
    if (!isStripeConfigured()) {
      return res
        .status(503)
        .json({ message: "Secure checkout is not configured yet" });
    }
    ...
    try {
      ...
      if (order.status !== "payment_required")
        return res
          .status(409)
          .json({ message: "This order is not ready for payment" });
      const items = await db
        .select()
        .from(orderItems)
        .where(eq(orderItems.orderId, order.id));
      if (!items.length)
        return res.status(409).json({ message: "This order has no items" });

      let billingTerms;
      try {
        billingTerms = checkoutBillingTerms(
          items.map((item) => ({
      ...
      const sessionParams: Stripe.Checkout.SessionCreateParams = {
        ...
        line_items: items.map((item, index) => ({
          ...
          price_data: {
            ...
            unit_amount: checkoutLineAmounts[index],
            product_data: {
              name:
                item.quantity > 1
            ...
            ...(recurring
              ? {
                  recurring: {
                    interval: (billingTerms.billingInterval ??
      ...
      if (recurring) {
        sessionParams.subscription_data = {
          metadata: { orderId: order.id, buyerId: String(order.buyerId) },
          ...(order.trialDays > 0
            ? { trial_period_days: order.trialDays }
            : {}),
          ...(creatorRoute
            ? {
      ...
      await db.transaction(async (tx) => {
        await tx
          ...
          .where(
            and(eq(orders.id, order.id), eq(orders.status, "payment_required")),
          );
        if (creatorRoute) {
          await tx
            .insert(creatorEarningsAllocations)
      ...
      const message =
        error instanceof Error && error.message.includes("creator")
          ? error.message
  ...
  app.post(
    "/api/orders/:id/subscription/cancel",
    attachUser,
    async (req, res) => {
      if (!isStripeConfigured()) {
        return res
          .status(503)
          .json({ message: "Subscription management is not configured yet" });
      }
      try {
        const order = await getBuyerOrder(req.params.id, req.dbUser!.id);
        if (!order) return res.status(404).json({ message: "Order not found" });
        if (order.status !== "paid" || !order.providerSubscriptionReference) {
          return res
            .status(409)
            .json({
              message: "This order does not have an active subscription",
            });
        }
        if (
          !subscriptionCanCancel(
            order.subscriptionStatus,
            order.subscriptionCancelAtPeriodEnd,
          )
        ) {
          return res
            .status(409)
            .json({ message: "This subscription is already inactive" });
        }
        ...
        res
          .status(502)
          .json({ message: "Unable to cancel renewal. Please try again." });
      }
    },
  );
  ...
  app.post("/api/orders/:id/refund", attachUser, async (req, res) => {
    if (!isStripeConfigured())
      return res
        .status(503)
        .json({ message: "Refund management is not configured yet" });
    const idempotencyKey = req.header("idempotency-key")?.trim();
    if (!idempotencyKey || idempotencyKey.length > 255) {
      return res
        .status(400)
        .json({ message: "A valid Idempotency-Key header is required" });
    }
    try {
      const [order] = await db
        .select()
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "You are not authorized to refund this sale" });
      }
      if (
        order.status !== "paid" ||
        ["refunded", "dispute_lost"].includes(order.financialStatus)
      ) {
        return res
          .status(409)
          .json({ message: "This order is not refundable" });
      }
      const remainingCents = cents(
        Math.max(0, order.totalAmount - order.refundedAmount),
      );
      ...
      if (
        ...
        requestedAmount <= 0 ||
        requestedAmount > remainingCents
      ) {
        return res
          .status(400)
          .json({ message: "Refund amount exceeds the remaining paid amount" });
      }
      ...
      if (!paymentIntent?.startsWith("pi_"))
        return res
          ...
          .json({
            message:
              "The Stripe payment reference is unavailable for this order",
          });
      await db
        .update(orders)
        .set({ providerPaymentReference: paymentIntent, updatedAt: new Date() })
        .where(eq(orders.id, order.id));
      ...
      const providerPayment = await getStripe().paymentIntents.retrieve(
        paymentIntent,
      ...
      const charge =
        typeof latestCharge === "string"
          ? await getStripe().charges.retrieve(latestCharge)
          : latestCharge;
      ...
      const refund = await getStripe().refunds.create(
        {
          ...
          metadata: {
            orderId: order.id,
            initiatedByUserId: String(req.dbUser!.id),
          },
        },
        { idempotencyKey },
      );
      ...
      res
        .status(502)
        .json({ message: "Unable to issue the refund. Please try again." });
    }
  });
