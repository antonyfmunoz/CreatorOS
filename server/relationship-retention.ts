...
export async function cleanupRelationshipHubRetention() {
  ...
  const redactedEvents = await db.update(relationshipProviderEvents).set({ normalizedPayload: { redacted: true }, rawStorageKey: null, errorMessage: null }).where(and(inArray(relationshipProviderEvents.status, ["processed", "dead_letter"]), lt(relationshipProviderEvents.receivedAt, daysAgo(providerPayloadDays)), sql`${relationshipProviderEvents.normalizedPayload} <> '{"redacted":true}'::json`)).returning({ id: relationshipProviderEvents.id });
  const removedAudits = await db.delete(relationshipAuditEvents).where(lt(relationshipAuditEvents.createdAt, daysAgo(auditDays))).returning({ id: relationshipAuditEvents.id });
  return { voiceAssetsRemoved, expiredSuggestions: expiredSuggestions.length, expiredMemories: expiredMemories.length, redactedProviderEvents: redactedEvents.length, removedAuditEvents: removedAudits.length };
}
