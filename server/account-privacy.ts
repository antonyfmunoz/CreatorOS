...
export async function buildAccountExport(user: User) {
  ...
  const [
    ...
    Promise.all([
      ...
      exportRows(sql`select * from foundation_instruments where owner_user_id = ${userId} order by created_at`),
      exportRows(sql`select revision.* from foundation_instrument_revisions revision join foundation_instruments instrument on instrument.id = revision.instrument_id where instrument.owner_user_id = ${userId} order by revision.created_at`),
      exportRows(sql`select event.* from foundation_instrument_events event join foundation_instruments instrument on instrument.id = event.instrument_id where instrument.owner_user_id = ${userId} order by event.created_at`),
      exportRows(sql`select submission.* from foundation_form_submissions submission join foundation_instruments instrument on instrument.id = submission.form_instrument_id where instrument.owner_user_id = ${userId} order by submission.created_at`),
      exportRows(sql`select * from design_projects where owner_user_id = ${userId} order by created_at`),
      exportRows(sql`select version.* from design_versions version join design_projects project on project.id = version.project_id where project.owner_user_id = ${userId} order by version.created_at`),
      exportRows(sql`select event.* from design_project_events event join design_projects project on project.id = event.project_id where project.owner_user_id = ${userId} order by event.created_at`),
      exportRows(sql`select * from design_templates where owner_user_id = ${userId} order by created_at`),
      exportRows(sql`select collaboration.* from design_collaborators collaboration join design_projects project on project.id = collaboration.project_id where collaboration.user_id = ${userId} or project.owner_user_id = ${userId} order by collaboration.created_at`),
      exportRows(sql`select link.id, link.version_id, link.label, link.expires_at, link.revoked_at, link.created_at from design_review_links link join design_versions version on version.id = link.version_id join design_projects project on project.id = version.project_id where project.owner_user_id = ${userId} order by link.created_at`),
  ...
  const payload = {
    ...
    creatorStudio: {
      ...
      foundationInstruments: creatorStudio[27], foundationInstrumentRevisions: creatorStudio[28],
      foundationInstrumentEvents: creatorStudio[29], foundationFormSubmissions: creatorStudio[30],
      designProjects: creatorStudio[31], designVersions: creatorStudio[32], designEvents: creatorStudio[33],
