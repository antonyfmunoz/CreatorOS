...
async function projectAccess(userId: number, projectId: string, write = false) {
  ...
  const [collaborator] = await db.select().from(designCollaborators).where(and(eq(designCollaborators.projectId, project.id), eq(designCollaborators.userId, userId))).limit(1);
  if (!collaborator || (write && collaborator.role !== "editor")) return null;
  return { project, role: collaborator.role };
}

async function activeReview(token: string) {
  const [row] = await db.select({ link: designReviewLinks, version: designVersions, project: designProjects }).from(designReviewLinks).innerJoin(designVersions, eq(designReviewLinks.versionId, designVersions.id)).innerJoin(designProjects, eq(designVersions.projectId, designProjects.id)).where(and(eq(designReviewLinks.tokenHash, hashToken(token)), isNull(designReviewLinks.revokedAt), gt(designReviewLinks.expiresAt, new Date()))).limit(1);
  return row ?? null;
}

function scaleDocument(document: DesignDocument, width: number, height: number, mode: "fit" | "fill" | "stretch") {
  const source = document.pages[0]; const sx = width / source.width; const sy = height / source.height; const uniform = mode === "fill" ? Math.max(sx, sy) : Math.min(sx, sy); const xScale = mode === "stretch" ? sx : uniform; const yScale = mode === "stretch" ? sy : uniform; const dx = (width - source.width * xScale) / 2; const dy = (height - source.height * yScale) / 2;
  return { version: 1 as const, pages: document.pages.map((page, index) => ({ ...page, id: `page-${index + 1}`, width, height, elements: page.elements.map((element) => ({ ...element, x: element.x * xScale + dx, y: element.y * yScale + dy, width: element.width * xScale, height: element.height * yScale, ...(element.type === "text" ? { fontSize: element.fontSize * Math.min(xScale, yScale) } : {}) })) })) };
}

async function imageData(document: DesignDocument, ownerUserId: number, directory: string) {
  const ids = Array.from(new Set(document.pages.flatMap((page) => page.elements.filter((element): element is Extract<DesignElement, { type: "image" }> => element.type === "image").map((element) => element.assetId))));
  if (!ids.length) return new Map<string, string>();
  const rows = await db.select().from(assets).where(and(inArray(assets.id, ids), eq(assets.ownerUserId, ownerUserId), eq(assets.status, "ready")));
  if (rows.length !== ids.length) throw Object.assign(new Error("Every design image must be a ready owned Media Cloud asset"), { status: 404 });
  const result = new Map<string, string>();
  for (const asset of rows) {
    if (!asset.mimeType?.startsWith("image/") || asset.storageProvider === "remote-import") throw Object.assign(new Error("Design images must be managed image assets"), { status: 400 });
    await assertAssetUsageAllowed(asset.id, "editing");
    const target = path.join(directory, `${asset.id}${path.extname(asset.originalFilename ?? "") || ".image"}`);
    await materializeStoredAsset(asset.storageKey, asset.visibility === "private" ? "private" : "public", target);
    result.set(asset.id, `data:${asset.mimeType};base64,${(await fs.readFile(target)).toString("base64")}`);
  }
  return result;
}

function elementSvg(element: DesignElement, images: Map<string, string>) {
  const transform = `rotate(${element.rotation} ${element.x + element.width / 2} ${element.y + element.height / 2})`;
  if (element.type === "shape") {
    if (element.shape === "ellipse") return `<ellipse cx="${element.x + element.width / 2}" cy="${element.y + element.height / 2}" rx="${element.width / 2}" ry="${element.height / 2}" fill="${element.fill}" stroke="${element.stroke ?? "none"}" stroke-width="${element.strokeWidth}" opacity="${element.opacity}" transform="${transform}"/>`;
    if (element.shape === "line") return `<line x1="${element.x}" y1="${element.y}" x2="${element.x + element.width}" y2="${element.y + element.height}" stroke="${element.stroke ?? element.fill}" stroke-width="${Math.max(1, element.strokeWidth)}" opacity="${element.opacity}" transform="${transform}"/>`;
    return `<rect x="${element.x}" y="${element.y}" width="${element.width}" height="${element.height}" rx="${element.radius}" fill="${element.fill}" stroke="${element.stroke ?? "none"}" stroke-width="${element.strokeWidth}" opacity="${element.opacity}" transform="${transform}"/>`;
  }
  if (element.type === "image") return `<image href="${images.get(element.assetId) ?? ""}" x="${element.x}" y="${element.y}" width="${element.width}" height="${element.height}" preserveAspectRatio="${element.fit === "contain" ? "xMidYMid meet" : element.fit === "cover" ? "xMidYMid slice" : "none"}" opacity="${element.opacity}" transform="${transform}"/>`;
  const anchor = element.align === "center" ? "middle" : element.align === "right" ? "end" : "start"; const x = element.align === "center" ? element.x + element.width / 2 : element.align === "right" ? element.x + element.width : element.x; const lines = element.text.split(/\r?\n/).slice(0, 100);
  return `<text x="${x}" y="${element.y + element.fontSize}" fill="${element.fill}" font-size="${element.fontSize}" font-family="${escapeXml(element.fontFamily)}" font-weight="${element.fontWeight}" text-anchor="${anchor}" opacity="${element.opacity}" transform="${transform}">${lines.map((line, index) => `<tspan x="${x}" dy="${index ? element.fontSize * 1.2 : 0}">${escapeXml(line)}</tspan>`).join("")}</text>`;
}

async function renderPage(document: DesignDocument, pageId: string, ownerUserId: number, directory: string) {
  const page = document.pages.find((candidate) => candidate.id === pageId); if (!page) throw Object.assign(new Error("Design page not found"), { status: 404 }); const images = await imageData(document, ownerUserId, directory); const body = [...page.elements].sort((a, b) => a.zIndex - b.zIndex).map((element) => elementSvg(element, images)).join(""); return { page, svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${page.width}" height="${page.height}" viewBox="0 0 ${page.width} ${page.height}"><rect width="100%" height="100%" fill="${page.background}"/>${body}</svg>` };
}
