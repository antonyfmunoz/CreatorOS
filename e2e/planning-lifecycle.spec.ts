...
test("production planning syncs sources and enforces dependencies before scheduling", async ({ page }) => {
  ...
  const parent = await (await page.request.post("/api/planning/items", { data: { title: `Parent ${stamp}`, kind: "campaign", status: "idea" } })).json() as { id: string };
  const child = await (await page.request.post("/api/planning/items", { data: { title: `Child ${stamp}`, kind: "content", status: "idea", parentWorkItemId: parent.id } })).json() as { id: string; version: number };
  const detail = await (await page.request.get(`/api/planning/items/${child.id}`)).json() as { parentWorkItemId: string; events: Array<{ eventType: string }> };
  expect(detail.parentWorkItemId).toBe(parent.id);
  expect(detail.events.map((event) => event.eventType)).toContain("task.created");
  expect((await page.request.patch(`/api/planning/items/${child.id}`, { data: { title: `Child revised ${stamp}`, version: child.version + 1 } })).status()).toBe(409);
  expect((await page.request.patch(`/api/planning/items/${child.id}`, { data: { title: `Child revised ${stamp}`, version: child.version } })).status()).toBe(200);
  ...
  await expect(page.getByText(`Child revised ${stamp}`).first()).toBeVisible();
  await page.getByRole("button", { name: "Calendar" }).click();
  await expect(page.getByText(`Parent ${stamp}`).first()).toBeVisible();
});
